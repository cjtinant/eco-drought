...
gage_poss <- whatNWISsites(bBox = c(-103.5, 42.5, -99.5, 44.2), 
...
# 3. join gage metadata & drop the partial metadata 
...
# cleans the metadata that was imported
...
gage_meta <- gage_meta_poss %>% 
  ...
  select(-c(gw_file_cd, nat_aqfr_cd, aqfr_cd)) %>% # aquifer data, so NA 
...
# 1. create a list of active gages & get metadata
# add an end date to remove provisional data & gaps in vals 
...
gage_nm_cont_top <-  tibble(sta = c("bad_mid", "bat_bhr", "brsf_co", "blp_bel", 
                                 "bev_buf", "bev_pri","che_red", "che_sce", 
                                 "che_was", "hat_edg","hor_oel", "lcr_bel", 
                                 "che_buf", "lwr_vet"),  
                  site_no = c("06441000", "06406500", "06440200", "06447230",
                              "06402500", "06402430", "06403700", "06408650", 
                              "06423500", "06400000", "06400875", "06449000",
                              "06402600", "06449100"))  

# need to spilt metadata into chunks to get from USGS
gage_nm_cont_top_1 <- gage_nm_cont_top %>% 
  slice(1:6)  

gage_nm_cont_top_2 <- gage_nm_cont_top %>% 
  slice(7:8)  

gage_nm_cont_top_3 <- gage_nm_cont_top %>% 
  slice(9:14)  

# get metadata
gage_meta_cont_top_1 <- gage_nm_cont_top_1 %>% 
  split(.$site_no)  %>% 
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta_cont_top_2 <- gage_nm_cont_top_2 %>% 
  split(.$site_no)  %>% 
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta_cont_top_3 <- gage_nm_cont_top_3 %>% 
  split(.$site_no)  %>% 
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

# fix data & join metadata 
gage_meta_cont_top_2 <- gage_meta_cont_top_2 %>% 
  mutate(inventory_dt = as.character(inventory_dt))

gage_meta_cont_top <- bind_rows(gage_meta_cont_top_1, gage_meta_cont_top_2, 
                                gage_meta_cont_top_3)

gage_meta_cont_top <- full_join(gage_nm_cont_top, gage_meta_cont_top, 
                                by = "site_no")  

rm(gage_nm_cont_top_1, gage_nm_cont_top_2, gage_nm_cont_top_3)  
rm(gage_meta_cont_top_1, gage_meta_cont_top_2, gage_meta_cont_top_3) 
rm(gage_nm_cont_top) 

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

gage_nm_cont_bot <-  tibble(sta = c("lwr_mar", "lwr_ros", "lwr_whi", "rap_far", 
                                     "whi_kad", "whi_ogl", "whi_slm", "whi_sta",
                                     "whi_int", "wkc_wok", "whi_roc",
                                     "whi_whi", "wcc_ogl"), 
                  site_no = c("06447500", "06449500", "06450500", "06421500", 
                              "06447000", "06446000", "06445700", "06445685", 
                              "06446500", "06446100", "06446200",  
                              "06447450", "06445980"))



                                 ""
#gage_nm_cont_bot <- gage_nm_cont_bot %>% 
  slice(1:14)

gage_meta_cont_bot <- gage_nm_cont_bot %>% 
  split(.$site_no)  %>% 
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta_cont_bot <- full_join(gage_nm_cont_bot, gage_meta_cont_bot, 
                                by = "site_no")  


gage_meta_cont <- bind_rows(gage_meta_cont_top, gage_meta_cont_bot)
...
# 2. get flow data from USGS using purrr:map & add short-names to dataframe
# continuous gages 
...
gage_cont_top <- full_join(gage_cont_top_nm, gage_cont_top, by = "site_no")  
gage_cont_bot <- full_join(gage_cont_bot_nm, gage_cont_bot, by = "site_no") 
gage_disc <- full_join(gage_disc_nm, gage_disc, by = "site_no") 

# adding these separately removes some bad data at the start: 
...
# 3. export daily flows to data folder 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
...
gage_summary <- rbind(gage_summary_cont, gage_summary_disc) 

rm(gage_summary_cont, gage_summary_disc) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 

# 4. create summaries to check data 
gage_nm <- rbind(gage_cont_bot_nm, gage_cont_top_nm, gage_disc_nm) %>% 
  rename(sta_nm = sta) 

rm(gage_cont_bot_nm, gage_cont_top_nm, gage_disc_nm) 

gage_summary <- full_join(gage_nm, gage_summary, by = "site_no") 
  
gage_summary <- gage_summary %>% 
  select(sta_nm, sta, everything) 
```


```{r import_metadata, eval=FALSE} 
# downloads the metadata from USGS 
# need to download in groups & coerce to fix chars & intigers in some vars

# 1. get  metadata from USGS using purrr:map
gage_summary1 <- gage_summary %>% 
  slice(1:20) 

gage_summary2 <- gage_summary %>% 
  slice(21:23) 

gage_summary3 <- gage_summary %>% 
  slice(24:29) 

gage_summary4 <- gage_summary %>% 
  slice(30:50) 

gage_summary5 <- gage_summary %>% 
  slice(51:55) 

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

gage_meta1 <- gage_summary1 %>% 
  split(.$site_no)  %>% # from base R
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta2 <- gage_summary2 %>% 
  split(.$site_no)  %>% # from base R
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta3 <- gage_summary3 %>% 
  split(.$site_no)  %>% # from base R
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta4 <- gage_summary4 %>% 
  split(.$site_no)  %>% # from base R
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

gage_meta5 <- gage_summary5 %>% 
  split(.$site_no)  %>% # from base R
  map_dfr(~ readNWISsite(siteNumbers = .$site_no)) 

# bind metadata into a single tibble 
gage_meta <- rbind(gage_meta1, gage_meta2, 
                   gage_meta3, gage_meta4, gage_meta5) 

# clean up 
rm(gage_meta1, gage_meta2, gage_meta3, gage_meta4, gage_meta5) 

rm(gage_summary1, gage_summary2, gage_summary3, 
   gage_summary4, gage_summary5, gage_nm) 

gage_summary <- gage_summary %>% 
  select(-sta_nm)

# 2. check the short names
gage_meta <- full_join(gage_summary, gage_meta, by = "site_no") 

gage_meta <- gage_meta %>% 
  select(sta, station_nm, everything())
...
```{r clean_active_disc_sites}
# prepare for reducing the gages 
gage_cont_disc <- bind_rows(gage_cont, gage_disc) 

gage_cont_disc <- gage_cont_disc %>% 
  filter(waterYear > 1990) 

gage_summary <- semi_join(gage_summary, gage_cont_discby = c("site_no"))  

test <- readNWISsite(siteNumber="06421500")
