...
gage_meta_na_fix <- readNWISsite(gage_meta_na$site_no) %>% 
...
# 1. censor zero flows to 0.01 cfs
...
gage_yr_incomp_Q30 <- gage_dv_incomp_Q30 %>% 
  group_by(sta, waterYear) %>% 
  summarize(num_year = first(num_year), 
            Q_mean = mean(Q, na.rm = TRUE), 
            Q7_mean = mean(Q7, na.rm = TRUE), 
            Q30_mean = mean(Q30, na.rm = TRUE)) %>% 
  ungroup() 

# check manually 
#
# Name     Result                   Note
# bev_abf  could use the mean data  Ephemeral & not changing much  
# che_red  drop the year            event at start of water year 
# 
``` 
