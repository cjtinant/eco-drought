...
gage_incomp <- left_join(gage_incomp, gage_meta, by = "sta") 
gage_incomp <- gage_incomp %>% 
  select(1:5) %>% 
  mutate(type = ifelse(waterYear == max_yr, "last yr",
                       ifelse(waterYear == min_yr, "first yr",
                              "other"))) %>% 
  filter(type == "other") %>% 
  filter(sta != "lcr_abv") %>% 
  filter(sta != "ros_ros") %>% 
...
#gage_incomp
# these are stations with missing data not first or last year
# A tibble: 4 x 6
#  sta     waterYear i_count min_yr max_yr type 
#  <chr>       <int>   <int>  <dbl>  <dbl> <chr>
#1 lcr_abv      1996     172   1938   2016 other
#2 che_buf      2007     192   1969   2017 other
#3 lwr_abv      2000     274   1982   2004 other
#4 whi_slm      1991     301   1962   1997 other
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
gage_incomp <- full_join(gage_incomp, gage_res,  
                         by = c("sta", "min_yr", "max_yr"))
...
#   <chr>       <int>   <int>  <dbl>  <dbl>
 #1 whi_slm      1991     301   1962   1997
 #2 lwr_abv      2000     274   1982   2004
 #3 lwr_abv      2003     316   1982   2004
 #4 che_buf      2007     192   1969   2017
 #5 blp_bel        NA      NA   1992   2017
 #6 brsf_co        NA      NA   1989   2017
 #7 lcr_abv        NA      NA   1938   2016
 #8 lcr_bel        NA      NA   1939   2017
 #9 lcr_vet        NA      NA   1959   2017
#10 lwr_mar        NA      NA   1938   2017
#11 lwr_whi        NA      NA   1950   2017
#12 wcc_ogl        NA      NA   1966   1999
#13 whi_int        NA      NA   1929   2017
#14 whi_kad        NA      NA   1942   2017
#15 whi_ogl        NA      NA   1943   2017
#16 whi_sta        NA      NA   1988   2017
#17 wkc_wok        NA      NA   1992   1997
...
```{r subset-new}
# This code chunk creates groups of stations for clustering.
# Want to create a first group with stations up to 1997 (wkc_wok)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   Original Gages; gage_full       = 61 gages & 741,215 obs  
#   Gages w/o 1990+ or missing data = 19 gages & 103,465 (dropped) 
#   Gages remaining; gage           = 42 gages & 637,750 obs 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   Gages max_yr < 1997             = 7 gages & 82,585 obs (dropped) 


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Steps:
# 1. filter the gages with max year < 1997

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# 1. filter the gages with max year < 1997----

# filter gages & observations
dum_meta <- gage_meta %>% 
  filter(max_yr < 1997) # 7 gages not on reservation
dummy    <- gage %>% 
  filter(max_yr < 1997) # 82,595 observations

# update dropped gages & observations
gage_meta_drop <- bind_rows(gage_meta_drop, dum_meta) 
gage_drop      <- bind_rows(gage_drop, dummy)
rm(dum_meta, dummy)

# update gages & gage metadata
gage_meta <- gage_meta %>% 
  filter(max_yr >= 1997) # 35 gages remaining
gage      <- gage %>% 
  filter(max_yr >= 1997) # 555,155 observations remaining
