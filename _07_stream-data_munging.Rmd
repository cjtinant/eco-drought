...
#  5. Use the EGRET input data (below) to increase serial completeness
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# find incomplete data
gage_incomp <- gage_fin3 %>% 
...
gage_incomp <- inner_join(gage_incomp, gage_meta, by = "sta") %>% 
  select(-c(4:12))

# EGRET table for gage_met3 stations
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#  Remove lcr_abv for lack of data 
...
# make bat_key serially complete  
chk_sta <- gage_fin3 %>% 
  filter(sta == "bat_key" & 
           waterYear < 1962) %>% 
  distinct(waterYear)
...
chk_sta <- anti_join(gage_fin3, gage_fin4) 
