...
# check results - should be 61 stations
check_sta <- gage_full %>% 
  distinct(sta)
...
gage_met_lt_1990 <- gage_meta_full %>% 
  filter(max_yr < 1990) 

#gage_lt_1990 <- gage_full %>% 
#    filter(max_yr < 1990) 
...
# So, we should work with the 44 stations with data post-1990
...
# join the stations to get a sense of 
...
#   Gages- original     = 61 
#   Gages w/o 1990+ obs = 17 (dropped)
#   Gages w/o 1996+ obs =  7 (dropped)
#   Gages 1992-1996 obs = 32 (1st clustering)
#   Gages post-1996 obs =  3 (2nd clustering; with additional gages)
...
gage_meta_post97 <- gage_meta %>% 
  filter(min_yr > 1993) %>% 
  arrange(count_yr) %>%
  as.tibble() 


# update list of dropped stations & clean up
gage_meta_drop <- gage_meta %>% 
  filter(max_yr < 1996) %>% 
  arrange(count_yr) %>%
  as.tibble() 

gage_meta_drop <- bind_rows(gage_met_lt_1990, gage_meta_drop)
rm(gage_met_lt_1990)
...
gage_fin_92_97 <- gage_full %>% 
  filter(min_yr < 1993 & 
           max_yr > 1995
         ) %>% 
  as.tibble() 

gage_drp_92_97 <- gage_full %>% 
  filter(min_yr > 1992 |
           max_yr < 1996
         ) %>% 
  as.tibble() 
...
# check work
chk_sta <- bind_rows(gage_met_92_97, gage_mdp_92_97) # count is ok
chk_sta <- bind_rows(gage_fin_92_97, gage_drp_92_97) # count is ok
...
export(gage_fin5, "data/gage_92_97_lg.csv") 
export(gage_met5, "data/gage_meta_92_97.csv") 
# Note that gage_92_97.csv is too big for git push 
...
export(gage_fin6, "data/gage_92_97.csv")
```
