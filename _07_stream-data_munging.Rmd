...
# 2. check for stations with very incomplete data 
#      * remove cheyenne river at angustora 
# 2. remove very short records (< 5 years) 
# 3. check end dates
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# create set of all possible recorded streamflows & arrange
gage_full  <- bind_rows(gage_cont, gage_disc)  
gage_full  <- bind_rows(gage_full, gage_othr) 
...
# 2. calculate year count, drop vars & join
gage_yr <- gage_full %>% 
  group_by(sta) %>% 
...
# 2. view reservation gages
# based on this look, I will use 1992-1997 as set of stations to do 
# grouping...
gage_fin1 <- gage_full %>% 
  filter(state_cd == 46)
...
# 2. remove gages with very short records of < 5 yrs
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# This is the top of the metadata tibble: 61 x 13
#   count_yr sta    site_no station_nm        dec_lat_va dec_long_va 
#      <dbl> <chr>    <int> <chr>                  <dbl>       <dbl>  
# 1        1 ant_g… 6458000 Antelope Creek n…       42.7       -102.  
# 2        2 fre_f… 6403500 FRENCH CR NEAR F…       43.7       -103.  
# 3        3 hor_a… 6400870 HORSEHEAD CR NEA…       43.1       -103.
# 4        6 bea_e… 6458500 BEAR C NR ELI NE…       42.9       -102.  
# 5        6 brnf_… 6440500 NORTH FORK BAD R…       44.0       -102.  
# 6        6 wkc_w… 6446100 WOUNDED KNEE CRE…       43.1       -102.  
# 7        7 bev_a… 6402470 BEAVER CREEK ABO…       43.5       -103.   # 8        7 whi_r… 6446200 WHITE R NEAR ROC…       43.5       -102.   
# 9        8 nio_c… 6457000 NIOBRARA RIVER N…       42.5       -102.  
#10       10 nio_c… 6459000 NIOBRARA R NEAR …       42.8       -101.  
...
gage_dmet1 <- gage_meta %>%
  filter(sta == "ant_gor" |
           sta == "fre_fai" | 
           sta == "hor_aoe" 
         ) 

gage_imet1 <- gage_meta %>%
  filter(sta != "ant_gor" & 
           sta != "fre_fai" & 
           sta != "hor_aoe" 
         ) %>% 
  select(min_yr, max_yr, sta, station_nm, everything()) %>%
  arrange(max_yr)
...
table <- gage_int1 %>% 
  group_by(sta) %>% 
  summarize(endYear = max(waterYear)) %>% 
  arrange(endYear) %>% 
  ungroup() %>% 
  as.tibble()

# 3. look for years with not full data
gage_incomp <- gage_full %>% 
  group_by(sta, waterYear) %>% 
  summarize(i_count = n()) %>% 
  filter(i_count <= 364) %>% 
  arrange(desc(i_count), waterYear) 

# 2.1 remove che_ang - which is run ~ 180 days/yr 
gage_incomp <- gage_incomp %>% 
  filter(sta != "che_ang")

gage_full_1 <- gage_full %>% 
  filter(sta != "che_ang") 

che_ang <- gage_full %>% 
  filter(sta == "che_ang") 

# 2.2 remove bad_mid after 1979
check_sta <- gage_full_1 %>% 
  group_by(sta, waterYear) %>% 
  summarize(i_count = n()) %>% 
  filter(sta == "bad_mid") %>% 
  arrange(waterYear) 

gage_full_2 <- gage_full_1 %>% 
  filter(sta != "bad_mid" | 
           waterYear < 1979)
...
gage_incomp <- gage_full_2 %>% 
  group_by(sta, waterYear) %>% 
  summarize(i_count = n(), 
            min_yr = min(waterYear), 
            max_yr = max(waterYear)) %>% 
  filter(i_count <= 330) %>% 
  filter(sta != "che_ang") %>%
  arrange(sta, waterYear, desc(i_count)) 
...
# che_ang   discharge data jumps from 1978-10-17 to 1979-01-18
#            discharge data jumps from 1979-08-08 to 1980-02-01
#            discharge data jumps from 1980-08-04 to 1981-02-01
#            discharge data jumps from 1981-08-03 to 1982-02-01
#            discharge data jumps from 1982-08-02 to 1982-12-17
#            discharge data jumps from 1983-08-02 to 1984-02-01
#            discharge data jumps from 1984-07-31 to 1985-02-01
#            discharge data jumps from 1985-07-31 to 1986-02-01
#            discharge data jumps from 1986-07-31 to 1987-02-01
#            discharge data jumps from 1987-07-31 to 1988-02-01
#            discharge data jumps from 1988-07-31 to 1989-02-01
#            discharge data jumps from 1989-07-31 to 1990-01-31
#            discharge data jumps from 1990-08-08 to 1991-02-01
#            discharge data jumps from 1991-07-31 to 1992-02-01
#            discharge data jumps from 1992-07-31 to 1993-02-01
#            discharge data jumps from 1993-07-31 to 1994-02-01
#            discharge data jumps from 1994-07-31 to 1995-02-02
#            discharge data jumps from 1995-07-31 to 1996-02-01
#            discharge data jumps from 1996-07-31 to 1997-02-01
#            discharge data jumps from 1997-07-31 to 1997-10-01
#            discharge data jumps from 1998-09-30 to 1999-02-01
#            discharge data jumps from 1999-07-31 to 2000-02-01
#            discharge data jumps from 2000-07-31 to 2000-10-01
#            discharge data jumps from 2001-09-30 to 2002-02-01
#            discharge data jumps from 2002-07-31 to 2003-02-01
#            discharge data jumps from 2003-07-31 to 2004-02-01
#            discharge data jumps from 2004-07-31 to 2005-02-01
#            discharge data jumps from 2005-07-31 to 2006-02-01
#            discharge data jumps from 2006-07-31 to 2007-02-01
#            discharge data jumps from 2007-07-31 to 2008-02-01
#            discharge data jumps from 2008-07-31 to 2009-02-01
#            discharge data jumps from 2009-07-31 to 2010-02-01
#            discharge data jumps from 2010-07-31 to 2011-02-01
#            discharge data jumps from 2011-07-31 to 2012-02-01
#            discharge data jumps from 2012-07-31 to 2013-02-01
#            discharge data jumps from 2013-07-31 to 2014-02-01
#            discharge data jumps from 2014-07-31 to 2015-02-01
#            discharge data jumps from 2015-07-31 to 2016-02-01
#            discharge data jumps from 2016-07-31 to 2017-02-01 
# bat_key   discharge data jumps from 1947-07-31 to 1961-10-02
# nio_gor   discharge data jumps from 1928-12-05 to 1929-03-01
#            discharge data jumps from 1932-06-30 to 1932-07-27
#            discharge data jumps from 1932-07-27 to 1932-08-17
#            discharge data jumps from 1932-08-17 to 1932-09-14
#            discharge data jumps from 1932-09-14 to 1945-10-01
