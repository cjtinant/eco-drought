...
```{r munge_streamflow}
# remove ice - starts 2017-10-23
gage <- gage %>%
  filter(code != "P Ice")

# remove provisional values 
gage <- gage %>%
  filter(code != "P") # starts 2016-10-26

gage <- gage %>%
  filter(code != "P e") # starts 2016-10-05
  
gage <- gage %>%
  filter(code != "P <") # starts 2016-10-05

code <- gage %>% 
  distinct(code)

gage <- gage %>%
  arrange(desc(Date)) # need to remove abo
