...
# Issues
an intermediate variable called names needs to be changed
...
#  mutate(site_no = as.character(site_no)) %>%
#  as.vector()
# Note that dataRetrieval::zeroPad pads with leading zeros

# not sure about this red line - seems like an artifact...
...
gage_fin <- bind_rows(gage_prior, gage_2017) %>%
  arrange(desc(date)) 
rm(gage_2017, gage_int, gage_list, gage_prior, ice, provis)  
...
names <- bind_cols(abrev, names)

gage_fin <- full_join(names, gage_fin, by = "site_no")
rm(names, abrev)
...
discharge <- discharge %>% 
  spread(key = site_no, value = discharge) 

code <- gage_fin %>%
    select(site_no, date, code) 

code <- code %>% 
  spread(key = site_no, value = code) 
```
