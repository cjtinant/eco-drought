...
# import and check gage data (1989-01 to 2017-09-15)----    
...
#sri_trend_ecoreg <- sri_seas %>%  
#  group_by(ecoreg, date) %>% 
#  summarise(trend_mad = mad(trend),
#            trend_med    = med(trend) 
##  ) %>%  
#  ungroup() %>% 
...
# check these----
#bat_bhr - ?? started in 1988!  -- should not show up!  
#brsf_co -- ?? started in 1988! -- should not show up!  
#lon_riv -- should work now -- accidently dropped  - need to rerun calcs  -- ok  
#nio_spa -- should work now -- accidently dropped  - need to rerun calcs  -- ok  
# hor_oel -- discordant - dropped
# bad_fpi -- Discordant - dropped
...
gage_meta <- import("data/gage_meta_lmom.csv") %>%  
 rename(gage = sta) %>%      # NEED TO FIX DUPLICATE COLS   
  rename(years = years.x) %>%   
  select(-c(ecoreg.x, years.y, ecoreg.y)) %>%  
  filter(isD == FALSE)  


sci_ts <- anti_join(sci_gage, gage_meta) %>% 
  distinct(gage)
