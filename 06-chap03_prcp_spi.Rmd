...
# Black Hills Plateau (bhp)  
gage_bhp <- sri_input %>%  
  filter(sta == "bat_her")    
  
bhp <- as.double(gage_bhp$log_q1_mon)   
  
# Keya Paha Tablelands (kpt)
gage_kpt <- sri_input %>%  
  filter(sta == "key_key")   
  
kpt <- as.double(gage_kpt$log_q1_mon)   
  
# Pierre Shale Plains (psp)  
gage_psp <- sri_input %>%  
  filter(sta == "brsf_co")    
  
psp <- as.double(gage_psp$log_q1_mon)   
  
# Pine Ridge Escarpment (pre)
gage_pre <- sri_input %>%  
  filter(sta == "whi_ogl")   
  
pre <- as.double(gage_pre$log_q1_mon)   
  
# Sand Hills (snd)  
gage_snd <- sri_input %>%  
  filter(sta == "lwr_vet")     
  
snd <- as.double(gage_snd$log_q1_mon)   
  
# White River Badlands (bad)  
gage_bad <- sri_input %>%  
  filter(sta == "whi_kad")   
  
bad <- as.double(gage_bad$log_q1_mon)   
...
# set up the station for sci & make sci list vars   
sta      <- kpt  
sci_1mo  <- sci.fun(sta, 1)         # 11-mon fail                          
sci_2mo  <- sci.fun(sta, 2)         # 10-mon fail                        
sci_3mo  <- sci.fun(sta, 3)         # 10-mon fail                        
sci_4mo  <- sci.fun(sta, 4)    
sci_6mo  <- sci.fun(sta, 6)   
sci_9mo  <- sci.fun(sta, 9)         # 2-, 8-mon fail  
sci_12mo <- sci.fun(sta, 12)        # 8-, 10-mon fail  
...
# set up the station for sci & make sci list vars     
sta      <- psp     
sci_1mo  <- sci.fun(sta, 1)       # 4-, 6-, 12-mon fail               
sci_2mo  <- sci.fun(sta, 2)       # 4-, 7-mon fail                       
sci_3mo  <- sci.fun(sta, 3)       # 1-, 2-mon fail                           
sci_4mo  <- sci.fun(sta, 4)       # 3-, 8-mon fail  
sci_6mo  <- sci.fun(sta, 6)       # 3-, 8-mon fail    
sci_9mo  <- sci.fun(sta, 9)   
sci_12mo <- sci.fun(sta, 12)      # 1-, 11-mon fail  
...
# set up the station for sci & make sci list vars   
sta      <- pre  
sci_1mo  <- sci.fun(sta, 1)       # 9-, 11-mon fail                       
sci_2mo  <- sci.fun(sta, 2)       # 11-mon fail                       
sci_3mo  <- sci.fun(sta, 3)       # 12-mon fail                       
sci_4mo  <- sci.fun(sta, 4)       #  2-mon fail    
sci_6mo  <- sci.fun(sta, 6)       # 4-mon, 5-mon fail  
sci_9mo  <- sci.fun(sta, 9)    
sci_12mo <- sci.fun(sta, 12)  
...
sci_1mo  <- sci.fun(sta, 1)                              
sci_2mo  <- sci.fun(sta, 2)                             
sci_3mo  <- sci.fun(sta, 3)         # 4-mon fail                    
sci_4mo  <- sci.fun(sta, 4)   
sci_6mo  <- sci.fun(sta, 6)   
sci_9mo  <- sci.fun(sta, 9)   
sci_12mo <- sci.fun(sta, 12)  
...
sci_4mo  <- sci.fun(sta, 4)    
sci_6mo  <- sci.fun(sta, 6)   
sci_9mo  <- sci.fun(sta, 9)      # 6-mon fail  
sci_12mo <- sci.fun(sta, 12)  
...
# check pierre shale seasonality 
psp_seas <- sri_seas %>% 
  filter(ecoreg == "Pierre Shale Plains") %>% 
  mutate(mon = month(date)) %>% 
  group_by(mon) %>% 
  summarize(mon_dep = mean(season))
