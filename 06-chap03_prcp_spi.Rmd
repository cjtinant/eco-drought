...
```{r 3_plot_monthly_precip, eval=FALSE}  
...
mclust_aug <- mclust_aug %>%  
  ...
  unite("final_group", ecoreg2, group, 
        sep = "_", 
        remove = FALSE) %>%  
  # create final groups  
  mutate(final_group = case_when(  
    final_group == "bad_2" ~ "Tertiary Clays",  
    final_group == "pre_2" ~ "Tertiary Clays",  
    final_group == "bhp_3" ~ "Black Hills Plateau",  
    final_group == "kpt_3" ~ "Keya Paha Tablelands",  
    final_group == "kpt_4" ~ "Keya Paha Tablelands",  
    final_group == "snd_3" ~ "Sand Hills",  
    final_group == "snd_4" ~ "Sand Hills",  
    final_group == "snd_1" ~ "GW control",  
    final_group == "bhp_1" ~ "GW control",   
...
mutate(final_group = factor(final_group)) %>% 
  mutate(final_group = fct_relevel(final_group,  
                                   "GW control",  
                                   "Tertiary Clays",  
                                   "Pierre Shale High",  
                                   "Black Hills Plateau",  
...
# 6 create final group summaries====  
final_group <- mclust_aug %>%  
  # summarise by final group  
  ...
  gather(key = drop, val = ecoreg,  
         -c(final_group, L1, L_CV, L_skew, L_kurtosis)) %>%  
  select(-drop) %>%  
  distinct() %>%  
  arrange(L_skew) %>%   
  # relevel ecoregion for plotting  
...
fin_grp_plot <- mclust_aug %>%  
  ggplot(aes(L_skew, L_kurtosis)) +  
  # set theme, facets, labels #  
  theme_bw() +  
  facet_wrap(vars(ecoreg)) +  
  ggtitle("Final L-moment groups for streamflow gages") + 
  xlab("L-skew") +  
  ylab("L-kurtosis") +  
  theme_bw() +  
  facet_wrap(vars(ecoreg)) + 
  geom_point(data = final_group, 
             shape = 3,  
             size = 4,  
             aes(color = final_group)) +   
  geom_point(aes(color = final_group,  
                 size = L1)) +  
  scale_size(range = c(1, 4), guide = "none") +     
  scale_color_grey(name = "Final groups")  

# plot the plots above as a grid & save====  
cowplot::plot_grid(init_grp_plot, fin_grp_plot,  
                   ncol = 1,   
                   align = "v")    
  
cowplot::ggsave2("figure/Mclust_lmoms.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
# 1.1 SCI calcs - bha_bat -- set up station for sci & make sci list vars====  
sta      <- bhp_bat_v   
sci_1mo  <- sci.fun(sta, 1)                             
sci_2mo  <- sci.fun(sta, 2)                             
sci_3mo  <- sci.fun(sta, 3)                           
...
# 1.2 SCI calcs - bhp_bev -- set up station for sci & make sci list vars====    
sta      <- bhp_bev_v  
sci_1mo  <- sci.fun(sta, 1)                             
sci_2mo  <- sci.fun(sta, 2)                             
sci_3mo  <- sci.fun(sta, 3)                             
...
# 1.3 SCI calcs - bha_fal -- set up station for sci & make sci list vars====   
sta      <- bhp_fal_v   
sci_1mo  <- sci.fun(sta, 1)                             
...
# 1.4 SCI calcs - bha_frn -- set up station for sci & make sci list vars====  
sta      <- bhp_frn_v   
...
# 2.0 calculate SCI_kpt----  
# 2.1 SCI calcs - kpt_key -- set up station for sci & make sci list vars====   
sta      <- kpt_key_v   
sci_1mo  <- sci.fun(sta, 1)                     
sci_2mo  <- sci.fun(sta, 2)                   
sci_3mo  <- sci.fun(sta, 3)           
...
# 2.2 SCI calcs - kpt_wew -- set up station for sci & make sci list vars==== 
sta      <- kpt_wew_v  
sci_1mo  <- sci.fun(sta, 1)                       
sci_2mo  <- sci.fun(sta, 2)                        
sci_3mo  <- sci.fun(sta, 3)           
...
# 3.0 calculate SCI_pre---- 
# 3.1 SCI calcs - pre_ogl -- set up station for sci & make sci list vars====   
sta      <- pre_ogl_v   
sci_1mo  <- sci.fun(sta, 1)     # MLE fail month 9 (SEP)                    
...
# 3.2 SCI calcs - pre_sta -- set up station for sci & make sci list vars====   
sta      <- pre_sta_v  
sci_1mo  <- sci.fun(sta, 1)    # MLE fail month 9 (SEP)                   
sci_2mo  <- sci.fun(sta, 2)    # MLE fail month 10  (SEP-OCT)               
sci_3mo  <- sci.fun(sta, 3)             
...
# 4.0 calculate SCI_psp---- 
# 4.1 SCI calcs - psp_che -- set up station for sci & make sci list vars====   
sta      <- psp_bat_v   
...
# 4.2 SCI calcs - psp_brs -- set up station for sci & make sci list vars====   
sta      <- psp_brs_v  
...
# 4.3 SCI calcs - psp_che -- set up station for sci & make sci list vars====   
sta      <- psp_che_v  
sci_1mo  <- sci.fun(sta, 1)                       
sci_2mo  <- sci.fun(sta, 2)                        
sci_3mo  <- sci.fun(sta, 3)                 
...
# 4.4 SCI calcs - psp_elk -- set up station for sci & make sci list vars====   
sta      <- psp_elk_v  
...
# 4.5 SCI calcs - psp_hat -- set up station for sci & make sci list vars====   
sta      <- psp_hat_v  
...
# 4.6 SCI calcs - psp_whi -- set up station for sci & make sci list vars====   
sta      <- psp_whi_v  
...
# 5.0 calculate SCI_snd---- 
# 5.1 SCI calcs - snd_lcr -- set up station for sci & make sci list vars====    
sta      <- snd_lcr_v   
...
# 5.2 SCI calcs - snd_lon -- set up station for sci & make sci list vars====   
sta      <- snd_lon_v  
sci_1mo  <- sci.fun(sta, 1)                      
sci_2mo  <- sci.fun(sta, 2)                       
sci_3mo  <- sci.fun(sta, 3)       
...
# 5.3 SCI calcs - snd_mar -- set up station for sci & make sci list vars====  
sta      <- snd_mar_v   
sci_1mo  <- sci.fun(sta, 1)                      
sci_2mo  <- sci.fun(sta, 2)                       
sci_3mo  <- sci.fun(sta, 3)       
...
# 5.4 SCI calcs - snd_nio -- set up station for sci & make sci list vars====  
sta      <- snd_nio_v  
sci_1mo  <- sci.fun(sta, 1)                      
...
# 5.5 SCI calcs - snd_ros -- set up station for sci & make sci list vars====  
sta      <- snd_ros_v  
sci_1mo  <- sci.fun(sta, 1)                        
sci_2mo  <- sci.fun(sta, 2)                    
...
# 5.6 SCI calcs - snd_vet -- set up station for sci & make sci list vars====  
sta      <- snd_vet_v   
sci_1mo  <- sci.fun(sta, 1)                      
sci_2mo  <- sci.fun(sta, 2)                             
...
# 5.7 SCI calcs - snd_whi -- set up station for sci & make sci list vars====  
sta      <- snd_whi_v  
sci_1mo  <- sci.fun(sta, 1)                      
sci_2mo  <- sci.fun(sta, 2)                            
sci_3mo  <- sci.fun(sta, 3)       
...
# 6.0 calculate SCI-bad----  
# 6.1 SCI calcs - bad_whi -- set up station for sci & make sci list vars====  
sta      <- bad_whi_v    
sci_1mo  <- sci.fun(sta, 1)                              
sci_2mo  <- sci.fun(sta, 2)                             
sci_3mo  <- sci.fun(sta, 3)                            
...
sci_bhp <- bind_rows(sci_bhp_bat,  
  ...
  gather(key = type, val = value,  
         -c(sta, date, ecoreg, log_q1_mon))   

# join gw control SCI vals====  
sci_gwc <- bind_rows(sci_bhp_bev,  
                     sci_snd_lcr) %>%    
  gather(key = type, val = value,  
         -c(sta, date, ecoreg, log_q1_mon)) 
...
# join pierre shale high SCI vals====  
sci_psh <- bind_rows(sci_psp_bat,  
                     sci_psp_brs,  
                     sci_psp_che,  
                     sci_psp_elk,  
                     sci_psp_hat,  
