...
gage_lmom <- full_join(gage_meta, gage_ecoreg,  
                       by = "sta")  
# arrange variables  
gage_lmom <- gage_lmom %>%  
  select(site_no,  
...
gage_lmom <- semi_join(gage_lmom, gage_mon,  
                  by = "sta")  

# 3. import final monthly precip data & prepare for bootstrapping====   
...
# L-moments 
```{r 4_lmoment-ratios-streamflow, eval=FALSE}
...
lmom_gage <- full_join(gage_lmom, lmom_gage,    # lmom_gage_meta  
                       by = "sta") %>%  
  select(-c(site_no,  
            dec_lat_va,  
            dec_long_va)  
...
# 5. check and remove discordant gages - will put back later ====    
