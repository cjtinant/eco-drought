...
sci_sta_test <- sci_sta %>%  
  group_by(sta) %>%  
  arrange(date) %>% # checking the algorithm  
  slice(1:26) %>%   # checking the algorithm    
...
sci_sta_lags <- sci_sta_test %>%  
  group_by(sta, spi_length) %>%               # this is a key step!  
    tq_mutate(  
        select     = spi_val,  
        mutate_fun = lag.xts,  
        k          = k,  
...
sci_gage_test <- sci_gage %>%  
  group_by(gage) %>%  
  arrange(date) %>% # checking the algorithm  
  slice(1:26) %>%   # checking the algorithm    
  gather(key   = spi_length,   # use these for join below  
         value = sci_val,   
         -c(group,   
            date,   
            gage,   
...
# join sta & gage====  
sci_all <- full_join(sci_sta_lags, sci_gage_test,  
                     by = c("date", 
                            "group",  
                            "spi_length")) %>%   
  select(sta,  
...
# The goal is to get the la and each lag side-by-side so we can do a correlation. 
...
sci_sta_autocorrelations <- sci_sta_lags %>%  
    gather(key = "lag",  
           value = "lag_value",  
           -c(sta, gage, group, date, spi_1mo, sci_1mo)) %>%  
