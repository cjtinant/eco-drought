...
gage_mon_pres <- import("data/gage_mon_full.csv") %>%  
  mutate(Date = ymd(Date)) %>%  
  filter(between(Date, as.Date("1988-09-15"), as.Date("2018-09-15")))   
  
gage_meta <-  import("data/gage_meta.csv")   
  
gage_eco <- gage_mon_pres %>%  
  select(sta, ecoreg) %>%  
  distinct()  
...
export(gage_mon_pres, "data/gage_mon_pres.csv")  
...
gage_mon <- import("data/gage_mon_pres.csv")  
