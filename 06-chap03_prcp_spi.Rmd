...
# import working data and metadata====    
...
gage_meta <- import("data/gage_meta_seas.csv") %>%  
 rename(gage = sta) %>%  
  filter(isD == FALSE)  
...
yr_ck <- bind_rows(yr_ck_sta, yr_ck_gage)
...
# apply groups & ecoregions====   
# create groups of stations by region  
sta_meta_ts <- sta_meta %>%  
  select(sta, group)  

gage_meta_ts <- gage_meta %>%  
  rename(gage = sta) %>%  
  select(gage, dec_lat_va, dec_long_va, ecoreg) %>%   
  mutate(north_group = case_when(   
    dec_lat_va > 43.5 ~ "N",   
    TRUE ~ "S")   
  ) %>%   
  ...
  mutate(group = str_c(north_group, east_group, sep = "")) %>%  
  dplyr::select(-c(north_group, east_group, dec_lat_va, dec_long_va)) %>%  
  arrange(group)    

gage_ts <- semi_join(gage_meta_ts, sci_gage) 
...
test <- sci_gage %>% 
  ...
  group_by(count) %>% 
  summarise(yrs = n())  
meta_ts <- full_join(sta_meta_ts, gage_meta_ts,  
                     by = "group") %>%  
  select(group, everything()) 

rm(sta_meta_ts, gage_meta_ts)

sci_sta <- full_join(meta_ts, sci_sta, 
                     by = "sta")  

sci_gage <- full_join(meta_ts, sci_gage,  
                      by = c("gage", "ecoreg"))   

sci_ts <- full_join(sci_sta, sci_gage, 
                    by = c("group", "sta", "gage", "ecoreg", "date"))  
```
