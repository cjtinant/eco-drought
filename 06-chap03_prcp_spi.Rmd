...
#sta_meta_orig <- left_join(sta_meta_orig, sta_meta_fin, 
#                           by = "sta")  
#export(sta_meta_plus, "data/sta_meta_plus.csv") -?delete? 
# export(sta_dv_alt, "data/sta_dv_alt.csv")  - delete?  
...
Many solutions for clustering time series are available with R:
NoThomas Girke’s blog post, 
NoRafael Irizarry and Michael Love’s book, 
Time-series Andrew B. Collier’s blog post, 
Peter Laurinec’s blog post, 
Dylan Glotzer’s lecture 
Ana Rita Marques’s module.
...
ts.plot(ts_sim_classic_run, 
        xlab = "Distance [x100m]", 
        ylab = "Differential pace [min/km]", 
        main = "Example of classic run", 
        ylim=c(0,25)
        )
...
ts.plot(ts_sim_wall_run,  
        xlab = "Distance [x100m]", 
        ylab = "Differential pace [min/km]", 
        main = "Example of wall run", ylim=c(0,25) 
        )
```
...
# classic run
ts_sim_classic_run <- abs(arima.sim(n = 420, 
                                    mean = 0.001, 
                                    model = 
                                      list(order = c(1, 0, 0), 
                                           ar = 0.9))) + 
...
# wall run
ts_sim_part1 <- abs(arima.sim(n = 210, 
                              model = 
                                list(order = c(1, 0, 0), 
                                     ar = 0.9))) 
+ pace_min    

ts_sim_part2 <- ts(arima.sim(n = 210, 
                             model = 
                               list(order = c(1, 0, 0), 
                                    ar = 0.9)) + 
                     pace_wall, 
...
ts_sim_wall_run<- pmin(ts_sim_wall_run[,1], 
                       ts_sim_wall_run[,2], 
                       na.rm = TRUE)
...
Even if I’m a big fan of ggplot2 possibilities, some packages offer efficient ways to compute and plot data. For heatmaps I’m using the gplots package which displays time series with dendrograms is a single function. An overlook of all the heatmap possibilities can be found here

```{r}
dtw_dist <- function(x){proxy::dist(x, method="DTW")}

ts_clust <- ts_sim_df %>%
  dtwclust::as.matrix() #%>%  

ts_clust <- tibble(ts_clust)
  gplots::heatmap.2 (
    # dendrogram control
    distfun = dtw_dist,
    hclustfun = hclust,
