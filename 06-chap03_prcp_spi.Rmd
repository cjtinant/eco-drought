...
coef_table <- mclust_aug %>%   
  ...
  mutate(final_group = as.character(final_group)) %>%  
  mutate(ecoreg = as.character(ecoreg)) %>%  
...
CONSIDER ADDING STATION TO THE EXCEEDANCE PLOT  
```{r plot_exceedances}
...
# plot the plots above as a grid & save====  
cowplot::plot_grid(exceed_yr_plot, exceed_mon_plot,  
                   ncol = 1,   
                   align = "v")    

cowplot::ggsave2("figure/sci_exceedances.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
# 3 get quantiles====  
sri_summary <- sri_seas %>%  
  select(group, sci_1mo, season, trend, remainder) %>%  
  pivot_longer(-group) %>%   
  group_by(group, name) %>%  
...
sri_season <- full_join(sri_seas, sri_summary,  
                        by = "group")  
...
cowplot::plot_grid(sri_obs, sri_seas_plot, sri_trend_plot, sri_remain,  
                   ncol = 1,   
                   align = "v")  
...
# 3 update fal_hot grouping and export results====  
#spi_seas <- import("data/spi_seas.csv")  
mclust_fin <- mclust_aug %>%  
  mutate(final_group = as.character(final_group)) %>%  
  mutate(final_group = case_when(  
    group == "GW control" ~ "GW loss",  
    sta   == "fal_hot" ~ "GW gain",  
    TRUE ~ final_group))  

mclust_group <- mclust_fin %>%  
  select(sta, final_group)  

sri_final <- full_join(sri_seas, mclust_group,  
                       by = "sta")  

export(mclust_fin, "data/mclust_final.csv")  
export(sri_final, "data/sri_final.csv")  
...
# 2 drop season & trend variables from SPI and SRI dataframes====  
spi_corr <- spi_seas %>%  
  select(-c(prcp, 
            season:anomaly)) %>% 
...
sri_corr <- full_join(sri_final, gage_location,  
  ...
  select(-c(log_q1_mon,  
            ...
            group,  
            season:anomaly)) %>%  
  select(sta, final_group, everything()) %>%  
  rename(gage = sta) %>%  
...
ggplot(test, aes(spi_2mo, sri_1mo)) + 
  geom_point()
```
