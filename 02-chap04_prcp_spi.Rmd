...
# remove 'young' stations  
  filter(mindate < "1989-01-01") %>%                    # n = 434  
...
# Get stations for SPI calculations  
```{r prepare-for-spi}
...
sta_input <- left_join(sta_input, sta_meta_orig,    # join to meta orig 
                  by = c("id" = "sta_id"))  
...
```{r simplify_workspace}     

# prepare final spi values   
spi_fin <- bind_rows(spi_cot,    
                     spi_gor,  
                     spi_mis,  
                     spi_oel,  
                     spi_oni,  
...
sta_meta_fin <- semi_join(sta_meta_orig, spi_all,  
                          by = "sta")  

export(spi_fin, "data/spi_fin.csv")  
export(sta_meta_fin, "data/sta_meta_fin.csv")  

# clean up workspace   
rm(dateMax, distrib, first_mon, p_zero, p_zero_cm, scale, sci.limit, sta,  
   time_scale, warn_me, spi.fun, station)  
rm(spi_cot, spi_gor, spi_mis, spi_oel, spi_oni, spi_rap)  
...
spi_fin <- import("data/spi_fin.csv") %>% 
  mutate(date = ymd(date))  

# data is from 1954 to 2018 = 65 years 
# split into two chuncks & test before after 
spi_fin <- spi_fin %>%   
  mutate(yr = year(date)) %>%   
...
#  8 OEL_before   384     32
#  9 ONI_after    396     33
# 10 ONI_before   384     32
# 11 RAP_after    396     33
# 12 RAP_before   384     32

# bootstrap confidence interval of prcp  
prcp_dabest <- spi_fin %>%  
  dabest(x = group,             # grouping variable 
         y = prcp,              # measurement variable  
         idx = list(c("COT_before", "GOR_before", "OEL_before",  
                      "MIS_before", "ONI_before", "RAP_before"), 
                    c("COT_after", "GOR_after", "OEL_after", 
                      "MIS_after", "ONI_after", "RAP_after")), 
         paired = TRUE, 
         id.column = group  
  )  

plot.dabest(prcp_dabest, color.column = period)
...
```{r bootstrap_precip, eval=FALSE}

# Construct bootstrap confidence intervals of precip to 
#   identify difference in means 
#
# Notes on dabest
...
# TEST using 4,680 obs; 20,000 reps for precip 
...
# prepare to deconvolute prcp into trend, seasonal & random comps----  
...
# 1.3 create inputs====  
decomp_input <- bind_cols(spi_freq, spi_trend)  
rm(spi_freq, spi_trend)  

# decompose SPI into seasonal, trend, & remainder----  
# spi_01mo====  
TARGET <- "spi_1mo"  
spi01  <- spi_fin %>% 
  select(sta, date, spi_1mo) %>% 
  decomp_fun() %>% 
  rename(season_01mo    = season) %>% 
  rename(trend_01mo     = trend) %>% 
  rename(remain_01mo    = remainder) %>% 
  rename(hival_01mo     = remainder_l1) %>% 
  rename(loval_01mo     = remainder_l2) %>% 
  rename(anom_01mo      = anomaly) %>% 
  group_by(date) %>% 
  mutate(spi01_mean     = mean(spi_1mo)) %>%  
  mutate(seas01_mean    = mean(season_01mo)) %>%  
  mutate(trend01_mean   = mean(trend_01mo)) %>%  
  mutate(remain01_mean   = mean(remain_01mo)) %>%  
  ungroup() %>% 
# order stations for plotting 
  mutate(sta = fct_relevel(sta, "RAP", "COT", "ONI", "OEL", "GOR", "MIS"))  
...
# Filter out wyoming & sd counties  
counties_sd <- subset(counties, region %in%  
   c("south dakota")) %>%  
  filter(   
    subregion == "butte" |  
    subregion == "meade" |  
