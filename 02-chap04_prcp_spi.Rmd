...
```{r download_daily-precip, eval=FALSE}
...
#export(sta_dv_orig, file = "data/sta_dv_orig_1971.csv")   
#export(sta_meta_orig, file = "data/sta_meta_orig.csv")   
  
```  

```{r check_missingness_dv, eval=FALSE}
...
#export(sta_dv_plus, file = "data/sta_dv_plus_1971.csv")   
export(sta_meta_plus, file = "data/sta_meta_plus.csv")   

```

```{r plot-stations-initial, eval=FALSE}
...
```{r fill_missing_precip_data-nw, eval=FALSE}  
...
# export daily data   
export(sta_filled, file = "data/sta_filled.csv")  
```  
...
sta_meta_plus <- sta_meta_plus %>%  
  mutate(sta = case_when(  
    ...
    str_detect(name, "^FORT MEADE")   ~ "FOR",      
    str_detect(name, "^GORDON")       ~ "GOR",     
    str_detect(name, "^HOT SPRINGS")  ~ "HOR",  
...
sta_meta_plus <- full_join(sta_meta_plus, lmom_sta,  
                            by = "sta")  
...
sta_meta_plus$group <- factor(sta_meta_plus $group, 
                     levels = c("NW", "NC", "NE", "SW", "SC", "SE")
                     )  

# plot the theoretical distributions, and sample vals 
ggplot(sta_meta_plus, (aes(L_skew, L_kurtosis))) + 
...
rm(lmom_theo, lmom_sta, lmom_sta_nm)  
...
# Note; station downloads are of different lengths in order to converge 
#   the SPI vals below...  
...
```{r prepare_for_spi_NW-rap}

# prepare to download data 
sta_meta_input <- sta_meta_plus %>% 
  filter(group == "NW" 
  ) 
...
sta_input <- left_join(sta_input, sta_meta_plus, 
                  by = c("id" = "sta_id")) 
...
# prepare for input and clean up 
...
# get alternate site data and check
sta_alt <- sta_input %>%    
  filter(str_detect(name, "^ORAL")) %>%        
  filter(!is.na(prcp))                             
...
# get alternate site data   
sta_alt <- sta_meta_orig %>%  
  filter(str_detect(name, "^VALENTINE")) %>%  
  select(sta_id) %>%  
  split(.$sta_id) %>%  
  map_dfr(~meteo_tidy_ghcnd(stationid = .$sta_id,   
...
sta_fil <- bind_rows(sta_alt, sta_fil)  %>%                      # n = 25,933   
  arrange(name) %>%  
  distinct(date, .keep_all = TRUE)  
...
rm(dateMin, sta_alt, sta_fil, sta_input_len, sta_meta_input,  
   sta_miss_yr, sta_short_name, sta_input)  
```   

```{r prepare_for_spi_SE-mis}
...
spi_24mo <- spi.fun(sta, 24)  

# transformSCI applies the transformation identified by fitSCI   
...
spi_rap <- bind_cols(sta_rap,     
  ...
  rename(spi_24mo = value7) %>%  
  drop_na() %>%  
# drop the incomplete years     
  filter(date != "1950-12-15")  

# remove list calls 
rm(spi_1mo, spi_2mo, spi_3mo, spi_4mo, spi_6mo, spi_9mo, spi_12mo, spi_24mo)  
rm(sta_rap, rap)  
```
...
# make spi list vars          
spi_1mo  <- spi.fun(sta, 1)                            # ml fail for mon 10    
spi_2mo  <- spi.fun(sta, 2)                             
spi_3mo  <- spi.fun(sta, 3)                            
spi_4mo  <- spi.fun(sta, 4)    
spi_6mo  <- spi.fun(sta, 6)   
...
# make spi list vars          
spi_1mo  <- spi.fun(sta, 1)                            # ml fail for mon 6     
spi_2mo  <- spi.fun(sta, 2)                             
spi_3mo  <- spi.fun(sta, 3)                            
spi_4mo  <- spi.fun(sta, 4)   
spi_6mo  <- spi.fun(sta, 6)   
...
cot_1mo  <- spi.fun(cot, 1)   
cot_2mo  <- spi.fun(cot, 2)   
cot_3mo  <- spi.fun(cot, 3)   
cot_4mo  <- spi.fun(cot, 4)   
cot_6mo  <- spi.fun(cot, 6)   
cot_9mo  <- spi.fun(cot, 9)   
cot_12mo <- spi.fun(cot, 12)  
cot_24mo <- spi.fun(cot, 24)  

spi_cot_1mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_1mo) %>%  
  enframe(name = NULL)  

spi_cot_2mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_2mo) %>%  
  enframe(name = NULL)  

spi_cot_3mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_3mo) %>%  
  enframe(name = NULL)  

spi_cot_4mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_4mo) %>%  
  enframe(name = NULL)  

spi_cot_6mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_6mo) %>%  
  enframe(name = NULL)  

spi_cot_9mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_9mo) %>%  
  enframe(name = NULL)  

spi_cot_12mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_12mo) %>%  
  enframe(name = NULL)  

spi_cot_24mo <- transformSCI(cot,  
                        first.mon = first_mon,  
                        obj = cot_24mo) %>%  
  enframe(name = NULL)  

# remove list calls 
rm(cot_1mo, cot_2mo, cot_3mo, cot_4mo, cot_6mo, cot_9mo, cot_12mo, cot_24mo)  
...
spi_cot <- bind_cols(sta_cot,   
  ...
  rename(spi_cot_1mo = value)   %>%  
  rename(spi_cot_2mo = value1)  %>%  
  rename(spi_cot_3mo = value2)  %>%  
  rename(spi_cot_4mo = value3)  %>%  
  rename(spi_cot_6mo = value4)  %>%  
  rename(spi_cot_9mo = value5)  %>%  
  rename(spi_cot_12mo = value6) %>%  
  rename(spi_cot_24mo = value7) %>%  
  drop_na()             
...
# make spi list vars          
rap_1mo  <- spi.fun(rap, 1)   
rap_2mo  <- spi.fun(rap, 2)   
rap_3mo  <- spi.fun(rap, 3)   
rap_4mo  <- spi.fun(rap, 4)   
rap_6mo  <- spi.fun(rap, 6)   
rap_9mo  <- spi.fun(rap, 9)   
rap_12mo <- spi.fun(rap, 12)  
rap_24mo <- spi.fun(rap, 24)  

# transformSCI applies the transformation identified by fitSCI   
spi_rap_1mo  <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_1mo) %>%  
  enframe(name = NULL)  

spi_rap_2mo  <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_2mo) %>%  
  enframe(name = NULL)  

spi_rap_3mo  <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_3mo) %>%  
  enframe(name = NULL)  

spi_rap_4mo  <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_4mo) %>%  
  enframe(name = NULL)  

spi_rap_6mo  <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_6mo) %>%  
  enframe(name = NULL)  

spi_rap_9mo  <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_9mo) %>%  
  enframe(name = NULL)  

spi_rap_12mo <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_12mo) %>%  
  enframe(name = NULL)  

spi_rap_24mo <- transformSCI(rap,  
                        first.mon = first_mon,  
                        obj = rap_24mo) %>%  
  enframe(name = NULL)  

# remove list calls 
rm(rap_1mo, rap_2mo, rap_3mo, rap_4mo, rap_6mo, rap_9mo, rap_12mo, rap_24mo)  
...
spi_rap <- bind_cols(sta_rap,   
  ...
  rename(spi_rap_1mo = value)   %>%  
  rename(spi_rap_2mo = value1)  %>%  
  rename(spi_rap_3mo = value2)  %>%  
  rename(spi_rap_4mo = value3)  %>%  
  rename(spi_rap_6mo = value4)  %>%  
  rename(spi_rap_9mo = value5)  %>%  
  rename(spi_rap_12mo = value6) %>%  
  rename(spi_rap_24mo = value7) %>%  
  drop_na()             

rm(spi_rap_1mo, spi_rap_2mo, spi_rap_3mo, spi_rap_4mo, 
   spi_rap_6mo, spi_rap_9mo, spi_rap_12mo, spi_rap_24mo)  
...
```{r spi_rap_1-month, message=FALSE}   
  
# 1-month SPI -- RAP  
#~~~~~~~~~~~~~~~~~~~   
# calculate SPI as a list & extract results  
spi.rap.l <- fitSCI(rap,   
                    first.mon      = first_mon,  
                    distr          = distrib,  
                    time.scale     = time_scale,   
                    p0             = p_zero,    
...
```{r spi_cot_1-month, message=FALSE}  
    
# 1-month SPI -- COT  
#~~~~~~~~~~~~~~~~~~~  
# calculate SPI as a list & extract results   
spi.cot.l <- fitSCI(cot,    
                    first.mon      = first_mon,  
                    distr          = distrib,  
                    time.scale     = time_scale,   
                    p0             = p_zero,    
...
# create a matrix with SPI values - used below  
spi.cot <- transformSCI(cot, first.mon = first_mon,  
                        obj = spi.cot.l)  
...
```{r spi_oni_1-month, message=FALSE}  
  
# 1-month SPI -- ONI  
#~~~~~~~~~~~~~~~~~~~  
# calculate SPI as a list & extract results   
spi.oni.l <- fitSCI(oni,   
                    first.mon      = first_mon,  
                    distr          = distrib,  
                    time.scale     = time_scale,   
                    p0             = p_zero,    
...
# create a matrix with SPI values - used below  
spi.oni <- transformSCI(oni, first.mon = first_mon, 
                        obj = spi.oni.l) 

# change to tibble, prepare for join
spi_oni <- as.tibble(spi.oni) %>% 
  rename(spi_oni = value)

spi_oni <- bind_cols(sta_oni, spi_oni) # bind date, depth, SPI value

```

```{r spi_oel_1-month, message=FALSE}  

# Oelrichs SPI 
#~~~~~~~~~~~~~~~~~~~   
# calculate SPI as a list & extract results   
spi.oel.l <- fitSCI(oel,   
                    first.mon      = first_mon,  
                    distr          = distrib,  
                    time.scale     = time_scale,   
                    p0             = p_zero,    
...
```{r spi_gor_1-month, message=FALSE}  

# GORDON SPI 
#~~~~~~~~~~~~~~~~~~~ 
# calculate SPI as a list & extract results
spi.gor.l <- fitSCI(gor,   
                    first.mon      = first_mon,  
                    distr          = distrib,  
                    time.scale     = time_scale,   
                    p0             = p_zero,    
...
# create a matrix with SPI values - used below
spi.gor <- transformSCI(gor, first.mon = first_mon, 
                        obj = spi.gor.l) 

# change to tibble, prepare for join
spi_gor <- as.tibble(spi.gor) %>% 
  rename(spi_gor = value) 

spi_gor <- bind_cols(sta_gor, spi_gor) # bind date, depth, SPI value


```

```{r spi_mis_1-month, message=FALSE}  

# MISSION SPI 
#~~~~~~~~~~~~~~~~~~~ 
# calculate SPI as a list & extract results
spi.mis.l <- fitSCI(mis,   
                    first.mon      = first_mon,  
                    distr          = distrib,  
                    time.scale     = time_scale,   
                    p0             = p_zero,    
...
# create a matrix with SPI values - used below
spi.mis <- transformSCI(mis, first.mon = first_mon, 
                        obj = spi.mis.l) 

# change to tibble, prepare for join
spi_mis <- as.tibble(spi.mis) %>% 
  rename(spi_mis = value) 

spi_mis <- bind_cols(sta_mis, spi_mis) # bind date, depth, SPI value
...
```{r prepare_for_spi_cot-old}
# import precipitation data---- 
sta_meta_orig    <- import("data/sta_meta_orig.csv")   

sta_meta_input <- sta_meta_orig %>% 
  filter(str_detect(name, "^COTTON") | 
         str_detect(name, "^INTER")   
  )  
...
first.mon = first_mon_cot, distr = distrib, 
                   time.scale = time_scale, p0 = p_zero, 
                   p0.center.mass = p_zero_cm, scaling = scale, 
                   warn	= warn_me
...
spi.cot.l <-sta_mon %>% 
  filter(sta == "COT") %>% 
  as_tibble() %>% 
  mutate(depth_mm = as.double(depth_mm))  %>% 
  fitSCI(.$depth_mm, 
         time.scale = 1) 
