...
export(sta_dv_orig, file = "data/sta_dv_orig.csv")   
...
#  mutate(dur_year = maxdate - mindate) %>% 
...
sta_meta    <- import("data/sta_meta.csv") 
...
sta_cot <- sta_mon %>% 
  arrange(date) %>% 
  select(date, depth_mm) 





sta_int <- sta_mon %>% arrange(date) %>% select(date, int) 
sta_ora <- sta_mon %>% arrange(date) %>% select(date, ora) 
sta_oel <- sta_mon %>% arrange(date) %>% select(date, oel) 
sta_rap <- sta_mon %>% arrange(date) %>% select(date, rap) 
