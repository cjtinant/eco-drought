...
# keep only the GHCND stations  
  filter(str_detect(id, "^GHCND")) %>%                  # n = 53      
  filter(!str_detect(name, "^MAGPIE")) %>%              # n = 52  
  filter(elevation < 1200)                              # n = 42  
...
# prepare to download data 
sta_meta_input <- sta_meta_orig %>%   # need to use original stations for fill
  filter(group == "SC" ) 
...
sta_input <- left_join(sta_input, sta_meta_plus,    # join to meta orig 
                  by = c("id" = "sta_id"))  
