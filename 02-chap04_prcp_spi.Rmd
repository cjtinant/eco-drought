...
# remove 'young' stations 
  filter(mindate < "1989-01-01") %>%                 # n = 180       
# remove 'dead' stations 
  filter(maxdate > "2018-01-01") %>%                 # n = 31
# remove poor data coverage stations   
  filter(datacoverage == 1.0) %>%                    # n = 25 
# keep only the GHCND stations  
  filter(str_detect(id, "^G")) %>%                   # n = 23 
# remove mountain stations 
  filter(elevation < 1500) #%>%                      # n = 20  
...
ggplot(data = sta_meta_orig, aes(x = longitude, y = latitude)) + 
# add stations  
  geom_point(
