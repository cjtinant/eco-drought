...
```{r select-plot_final_stations}

# select final stations -- based on central location 
...
# join daily data-plus to metadata  
sta_dv_plus <- right_join(sta_meta_plus, sta_dv_plus,  
                   by = c("sta_id" = "id"))  %>% 
  select(-c(type, datacoverage, elevationUnit)) %>% 
...
# get north-west 
sta_nw <-  sta_dv_plus %>% 
   filter(str_detect(name, "^RAPID CITY R"))       # NC 

sta_nw_len <- sta_nw %>% 
  group_by(name, sta_id) %>% 
  summarise(count = n()) 

# get and fill north-central 
sta_nc <-  sta_dv_plus %>% 
   filter(str_detect(name, "^COTTONWOOD"))       # NC 
...
sta_nc_alt <-  sta_dv_plus %>% 
   filter(str_detect(name, "^INTERIOR"))          # NC-alt 

sta_nc_fill <- anti_join(sta_nc,sta_nc_alt, by = "date") 
...
# split the data into dv and fill data 
sta_dv <- sta_dv_plus %>% 
    filter(str_detect(name, "^RAPID CITY R") |   # NW         
         str_detect(name, "^COTTONWOOD")   |     # NC 
         str_detect(name, "^ONIDA")        |     # NE
...
sta_dv_alt <- anti_join(sta_dv_plus, sta_dv, 
                        by = c("sta_id")  
                        )  
