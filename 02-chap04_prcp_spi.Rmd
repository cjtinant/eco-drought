...
#  The string "GHCND:" doesn't appear in calls to get Global Historical  
#   Climatology Network (GHCN) Daily Data  -- split the 'id' string  
    separate(  
             col = id,  
             sep = ":",   
             into = c("type", "sta_id")   
...
export(sta_meta_plus, "data/sta_meta_orig.csv")   
export(sta_meta_fin, "data/sta_meta_fin.csv")  

export(sta_dv_orig, "data/sta_dv_orig.csv")   
export(sta_dv_plus, "data/sta_dv_orig.csv")   
...
# deconvolute prcp into trend, seasonal & random comps----  
...
# prepare for deconvoluting the spi====  
spi_fin <- spi_fin %>%   
...
# find mean values 
  group_by(date) %>% 
  mutate(prcp_mean     = mean(prcp)) %>%   
  mutate(seas_mean    = mean(season)) %>%  
  mutate(trend_mean   = mean(trend)) %>%  
  mutate(remain_mean   = mean(remainder)) %>%  
  ungroup() %>%  
# order stations for plotting 
...
# clean up  
rm(FREQ, TARGET, TREND, decomp_fun)  
