...
```{r prepare-to-download-station-data, eval=FALSE}
# import precipitation metadata---- 
sta_meta_orig <- import("data/sta_meta_orig.csv")  
dateMax       <- "2018-12-31"    
```  

```{r prepare_for_spi_NW-rap, eval=FALSE} 
...
```{r prepare_for_spi_NC-cot, eval=FALSE}  
...
```{r prepare_for_spi_NE-oni, eval=FALSE}
...
```{r prepare_for_spi_SW-oel, eval=FALSE}
...
```{r prepare_for_spi_SC-gor, eval=FALSE}
...
```{r prepare_for_spi_SE-mis, eval=FALSE}
...
spi_rap <- bind_cols(sta_rap,     
  ...
  drop_na() %>%  
  # drop the incomplete years (manually checked)    
...
```{r simplify_workspace_for_SPI, eval=FALSE}     
