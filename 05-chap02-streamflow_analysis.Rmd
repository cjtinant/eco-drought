...
# 3. plot clusters by ecoregion====  
mclust_eco_plot  <- mclust_aug %>%  
  ggplot(aes(PC1_mon, PC2_mon)) +  
  # set theme and facets #  
  theme_bw() +  
  facet_wrap(vars(ecoreg)) +  
  geom_jitter(aes(color = flow_type,  
        shape = ecoreg,  
        alpha = .uncertainty),  
    show.legend = FALSE, 
    size = 2) +  
...
# 4. plot & save using patchwork====  
mclust_flow_plot / mclust_eco_plot +  
  plot_annotation(tag_levels = 'A')  

ggsave("figure/mclust_plot_a.png",   
       units = "in",    
       width = 7,  
       height = 9)  
...
```{r PC-explanatory-plot}

# 1.prepare plotting variable  
mclust_aug <- mclust_aug %>%  
  mutate(q30_q1_mon = q30_q1_mon + 0.005)  

# 1. plot pc1 vs 7-day flow====  
pc1_plot <- mclust_aug %>%  
  ggplot(aes(q7_mon, PC1_mon)) +  
  # set theme and facets #  
  theme_bw() +  
  facet_wrap(~ ecoreg) +  
  # plot points #  
  ...
  geom_smooth(method = "lm",  
              ...
              size = 1) +  
  # set axis scale & text #  
  scale_x_log10("7-day mean daily flow averaged by month",  
                limits = c(0.005, 2000),  
...
# 3. plot pc1 vs 7-day flow====  
pc2_plot <- mclust_aug %>%  
  ggplot(aes(q30_q1_mon, PC2_mon)) +  
  # set theme and facets #  
  theme_bw() +  
  facet_wrap(~ ecoreg) +  
  # plot points #  
  ...
  geom_smooth(method = "lm",  
              ...
              size = 1) +  
  # set axis scale & text #  
  scale_x_log10("Difference in q30 and q1 mean daily flow averaged by month",  
                limits = c(0.005, 2000),  
                breaks = trans_breaks("log10", function(x) 10^x),  
                labels = trans_format("log10", math_format(10^.x))) +  
  ylab("PC2 axis") +  
...
ggsave("figure/mclust_diff_plot.png",   
       units = "in",    
       width = 7,  
       height = 9)  
...
# some alternate approaches for plotting   
...
```{r patchwork_overview, eval=FALSE}
...
env_vars_pierre_shale <- env_vars %>%  
...
```{r 08-visually_explore_spatial_data_fun}  
...
env_vars <- env_vars %>%  
  select(-c(vpd07_sq, kvert_ln))  

```     

```{r 12-export_ecoreg_table}
...
```{r 13-add-response-var_spatial-data} 
...
```{r 14-prepare-glm-models}  

# import gaged data  
gaged <- import("data/glm_input.csv") %>%  
  filter(sta != "fal_hot")  # bad fit!  Check down the line  ??? 
...
```{r 15-build-glm-models-all_data}
...
ridge_pc1 <- train_pc1 %>%   
  ...
  train(    
    ...
    preProcess = c("center", "scale"),   
    trControl = reg.ctrl,     
    tuneGrid = expand.grid(alpha = 0, lambda = lambda) ) # tuning values df  
...
```{r 16-build-glm-models-without-ecoreg-data}  
...
model_plot_pc1 <- error_vals_pc1 %>%  
  ...
  geom_boxplot(aes(x = factor(model_text),  
                   ...
                   ymax = `Max`),   
               stat = "identity") +    
# add text of number of training data  
...
```{r 21-get_glm_observations_and_predictions}
...
```{r 22-prepare_glm_observations_predictions_plot}
...
```{r 23-plot_glm_observations_predictions}
...
obs_pred_plot_pc1 <- fit_model_pc1 %>%  
  ...
  geom_smooth(method = "lm",  
              color = "black") +  
# add text of number of test data  
  geom_text(data = n_test,   
            x = -2.5,  
            y = 2.7,     
            aes(label= paste("n =", n_test, sep = " ")),    
            size = 3,    
            color = "gray20" ) +   
...
```{r 23b-plot_glm_residuals, eval=FALSE}
...
```{r 24-export-table_glm-coeffs_pc1}
...
```{r 25-export-table_glm-coeffs_wsd}
...
```{r 26-export-table_glm-coeffs_pc2} 
...
#   function adopted from https://stackoverflow.com/a/52557631/590437
...
```{r 30-prepare-for-glm-ecoreg-model}
...
```{r 33-get_glm_ecoreg_obs_&_preds}
...
```{r 35-plot_flow-duration-curve-density_ecoreg}
...
```{r 36-export-table_glm-coeffs_ecoreg}  
...
coef_table <- coefs_ecoreg %>%  
  mutate(coeff = case_when( 
    ...
    coeff == "cat_area" ~ "area",  
    coeff == "mon_spring" ~ "spring",  
    coeff == "slop_med" ~ "slope",  
