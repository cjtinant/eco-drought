...
```{r 23-prepare-for-glm-ecoreg-model}
...
gaged_ecoreg <- gaged_pc1 %>%  
  select(-c(prcp_sq,                   # drop unimportant predictors  
            slop_ln  
            ))  %>%  
...
#                                    alpha = 1, lambda = lambda)  # for lasso  
...
```{r 24b-select-glm-model_ecoreg, eval=FALSE}
...
```{r 26-export-table_glm-coeffs_ecoreg}  
...
```{r 29-plot_flow-duration-curve-density_ecoreg} 
  
# flow duration curve plot -ecoreg  
freq_plot_eco <- fit_ecoreg %>%  
# change the order of facets  
  mutate(ecoreg = factor(ecoreg,  
                         levels = c('pshale', 'escarp', 'wrbadl',  
                                    'bhplat', 'kptabl', 'sndhil')  
...
ggsave("figure/freq_plot_eco_new.png",  
       width = 7,  
       height = 7,  
       units = "in"  
       )  
