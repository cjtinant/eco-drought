...
wsd_summary <- import("data/wsd_summary.csv") %>%   
  mutate(ecoreg = case_when(   
    sta == "bat_bhr" ~ "Black Hills Transition",  # this was a judgement call:  
...
wsd_summary <- wsd_summary %>%  
  mutate(ecoreg = case_when(  
    ...
    sta == "POT1" ~ "Keya Paha Tablelands",  
    sta == "RED1" ~ "Keya Paha Tablelands",  
    sta == "WCC1" ~ "Pine Ridge Escarpment",  
    sta == "WCC2" ~ "Pine Ridge Escarpment",  
    sta == "WCC3" ~ "Pine Ridge Escarpment",  
    sta == "WHR1" ~ "Pine Ridge Escarpment",  
    sta == "WHR2" ~ "Pine Ridge Escarpment",  
    sta == "WHR3" ~ "White River Badlands",  
    sta == "WHR4" ~ "White River Badlands",  
    sta == "WHR5" ~ "White River Badlands",  
    sta == "WOK1" ~ "Sand Hills Fringe",  
    sta == "WOK2" ~ "Keya Paha Tablelands",  #  functionally bet SH & Escarp  
    sta == "WOK3" ~ "Pine Ridge Escarpment",  
    sta == "WOK4" ~ "Pine Ridge Escarpment",  
    sta == "WOL1" ~ "Sand Hills Fringe",  
...
gage_dep_fill <- right_join(gage_dep, gages_fill,
                        by = "sta")
...
blp_bel_ck <- gage_dep %>%
  filter(sta == "blp_bel") %>%
  filter(between(date,
                 as.Date("2017-08-09"), as.Date("2017-08-11")))
...
gages_in <- right_join(gage_dep, gages_in,
                        by = "sta")
...
gage_comp <- gage_dep_fill %>%  
  group_by(sta, waterYear) %>%  
  mutate(count = n())  %>%  
  ungroup() %>%  
  filter(count > 364)  

# unit test -- new gage_comp should be 342,243 + 365 = 342,244  
gage_ck <- gage_nms  
