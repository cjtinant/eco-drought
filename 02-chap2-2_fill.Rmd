...
# 2.  create lead and lag stations for same clusters - NA for diff clusters 
...
# 3.  gather the pairs to prepare to join the q1-q30 vals 
...
# 4.  join the q1-q30 vals for the stations 
gage_pairs <- full_join(gage_pairs, output, 
                        by = "sta") 

# 5.  join the q1-q30 vals for the station pairs & calculate diffs 
gage_pairs <- full_join(gage_pairs, output, 
                        by = c("sta_pair" = "sta")) %>% 
...
# 6.  join the year lengths for sta & sta_pair 
...
ggplot(gage_ck, aes(date, log(q30_depth))) +
  geom_line(aes(color = sta)) +
  facet_grid(sta ~ .) 
...
saveRDS(params, "data/mod_params_run09")   # need to update the file
#write_csv(gage_dv, "data/mod_dvs_run05")
...
yr_len <- yr_len %>% 
  mutate(min_yr = case_when(
    sta == "che_buf" ~ 2007,    # check for runs 1-8 ?2008?
    sta == "che_pla" ~ 2001,
    sta == "wcc_ogl" ~ 1988,
    TRUE ~ min_yr
  )) %>% 
