...
# for run 24 used 1990-2012  
# for run 36 used 1989-2017 instead of 1990-2017
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
miss_min <- yr_min %>% 
  as_tibble() %>% 
...
# 5.1 add lead Q30 information to summary table -updated 
...
# 5.2 add lag information to summary table & clean up - updated 
...
gage_miss <- gage_miss %>% 
  mutate(sta_fill = if_else(name.run == "run_08" & sta == "che_buf",
                            "che_red",
                            sta_fill)
        ) %>% 
  mutate(sta_fill = if_else(name.run == "run_013" & sta == "che_buf",
                            "hat_edg",
                            sta_fill)
         ) %>%
  mutate(sta_fill = if_else(name.run == "run_017" & sta == "che_red",
                            "hor_oel",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_018" & sta == "che_sce",
                            "hor_oel",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_020" & sta == "whi_int",
                            "hor_oel",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_025" & sta == "whi_whi",
                            "hat_edg",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_026" & sta == "whi_int",
                            "hat_edg",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_028" & sta == "che_sce",
                            "hat_edg",
                            sta_fill)
         ) %>% 
 mutate(sta_fill = if_else(name.run == "run_031" & sta == "che_red",
                            "elk_elm",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_031" & sta == "whi_whi",
                            "bad_fpi",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_032" & sta == "che_sce",
                            "elk_elm",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_032" & sta == "whi_int",
                            "bad_fpi",
                            sta_fill)
         ) %>% 
 mutate(sta_fill = if_else(name.run == "run_035" & sta == "che_red",
                            "blc_wan",
                            sta_fill)
         ) %>% 
  ...
  mutate(sta_fill = if_else(name.run == "run_035" & sta == "blc_wan",
                            "che_red",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_035" & sta == "whi_int",
                            "che_buf",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_037" & sta == "whi_slm",
                            "whi_int",
                            sta_fill)
         ) %>% 
  mutate(sta_fill = if_else(name.run == "run_038" & sta == "che_sce", 
                              "whi_slm", 
                            sta_fill)
         ) %>% 
...
# write_csv(gage_dv, "data/gage_fill_q30.csv") 
write_csv(vals_eigen,  "data/vals_eigen_q30.csv") 
write_csv(vals_pca, "data/vals_pca_q30.csv") 
write_csv(vals_eigen,  "data/vals_lambda_q30.csv") 
write_csv(vals_miss,   "data/vals_fill_q30.csv") 
