...
rm(bev_pri_1991, gage_dv_incomp, gage_dv_raw, lwr_aro_2003, whi_slm_1991,
   wy_1991, wy_2003, yr_dv_incomp, yr_incomp, wkc_wok) 
...
# 1. split complete and incomplete years 
gage_dv_comp <-  gage_dv %>% 
  filter(incomp_yr == "N") 

gage_dv_incomp <- gage_dv %>% 
  filter(incomp_yr == "Y") 

yr_sum_incomp <- gage_dv_incomp %>%                          
  group_by(sta, water_year) %>%                  
  summarise(days_record = n()) %>%                   
  ungroup()   

yr_sum_comp <- gage_dv_comp %>%                          
  group_by(sta, water_year) %>%                  
  summarise(days_record = n()) %>%                   
  ungroup()   
...
gage_othr_yr <- anti_join(gage_dv, yr_sum_incomp, 
                          by = "sta")     

# 3. calculate missing values based on monthly averages 
gage_miss_yr <- gage_miss_yr %>% 
  mutate(month = month(date)) %>% 
  group_by(sta, month) %>% 
  mutate(q_log = log10(q)) %>% 
  mutate(q7_log = log10(q7)) %>% 
  mutate(q30_log = log10(q30)) %>% 
  mutate(q_new = case_when( 
    is.na(q_log) ~ round(10^mean(q_log, na.rm = TRUE), digits =2), 
    TRUE ~ q 
    )) %>% 
  mutate(q7_new = case_when( 
    is.na(q7_log) ~ round(10^mean(q7_log, na.rm = TRUE), digits =2), 
    TRUE ~ q7 
    )) %>% 
  mutate(q30_new = case_when( 
    is.na(q30_log) ~ round(10^mean(q30_log, na.rm = TRUE), digits =2),  
    TRUE ~ q30 
    )) %>% 
  ungroup() 

# 4. check results 
sum_gage_yr <- gage_miss_yr %>% 
  group_by(sta) %>% 
  summarise(q_log = round(mean(q_log, na.rm = TRUE), digits =2), 
            q7_log  = round(mean(q7_log, na.rm = TRUE), digits =2), 
            q30_log = round(mean(q30_log, na.rm = TRUE), digits =2), 
            q       = round(mean(q, na.rm = TRUE),digits =2), 
            q7      = round(mean(q, na.rm = TRUE), digits =2), 
            q30     = round(mean(q, na.rm = TRUE), digits =2), 
            q_new   = round(mean(q_new, na.rm = TRUE), digits =2), 
            q7_new  = round(mean(q7_new, na.rm = TRUE), digits =2), 
            q30_new = round(mean(q30_new, na.rm = TRUE),  digits =2) 
  ) %>% 
  ungroup() 

# 5. update missing year gages & bind to complete gages 
gage_miss_yr <- gage_miss_yr %>% 
  select(sta:incomp_yr, 
         q = q_new, 
         q7 = q7_new, 
         q30 = q30_new) %>% 
  group_by(sta, water_year) %>% 
...
gage_dv2 <- bind_rows(gage_othr_yr, gage_miss_yr) %>% 
  select(-c(incomp_yr, days_yr)) 

# 6. check values 
sum_gage_yr <- gage_dv2 %>% 
  group_by(sta) %>% 
  summarise(q       = round(mean(q, na.rm = TRUE),digits =2), 
            q7      = round(mean(q, na.rm = TRUE), digits =2), 
            q30     = round(mean(q, na.rm = TRUE), digits =2), 
            min_yr  = min(water_year), 
            max_yr  = max(water_year),  
            diff_yr = 2 + max_yr - min_yr 
  ) %>% 
  ungroup() 

# 7. summarize & clean-up 
gage_dv <- gage_dv2  %>% 
  filter(sta != "nio_mar")                    # only 5 years of record        
...
yr_len <- yr_sum %>% 
  group_by(sta) %>% 
  summarise(years_record = n() + 1) %>%                   
  ungroup() 

yr_sum <- right_join(sum_gage_yr, yr_len, 
                      by = "sta") %>% 
  mutate(gap_yr = case_when( 
    diff_yr == years_record ~ "N", 
    TRUE ~ "Y" 
  )) %>% 
  select(-diff_yr) %>% 
  arrange(years_record) %>% 
  arrange(gap_yr)  

rm(gage_dv2, gage_dv_comp, gage_dv_incomp, gage_miss_yr, gage_othr_yr, 
   yr_sum_comp, yr_sum_incomp, yr_len, sum_gage_yr) 
```

```{r convert_to_flow_depth} 
...
yr_min <- as_vector(yr_sum %>% 
                      select(min_yr) %>% 
                      slice(1)
                    )  

yr_max <- as_vector(yr_sum %>%  
                      select(max_yr) %>%
                      slice(1)
                    ) 
...
library(factoextra) 
fviz_mclust(mod1, "classification", geom = "point", ellipse.type = "t", 
            ellipse.level = 0.8, pointsize = 0.5,  palatte = "aaas")

```

```{r}
#jsonedit(mod1, mode = "view", elementId = "mod1")


# 1. prepare to collect output
input <- input %>%  
  rownames_to_column() %>%  
  mutate(rowname = as.integer(rowname)) 

cluster <- enframe(mod1$classification, value = "clust", name = "rowname") 
...
vals_eigen <- read_csv("data/vals_eigen_pc.csv") %>% 
  filter(name != "none") %>% 
  mutate(PC = as.character(PC))
attr(vals_eigen, 'spec') <- NULL

vals_pca <- read_csv("data/vals_pca_pc.csv") %>% 
    filter(PC != "0") %>% 
  mutate(PC = as.character(PC)) 
attr(vals_pca, 'spec') <- NULL

vals_lambda <- read_csv("data/vals_lambda_pc.csv") 
attr(vals_lambda, 'spec') <- NULL

vals_fill <- read_csv("data/vals_fill_pc.csv") %>% 
    filter(sta != "sta") 
attr(vals_fill, 'spec') <- NULL
...
ggplot(vals_eigen, aes(PC, cumulative)) +
  geom_boxplot() 

ggplot(vals_pca, aes(var, value)) +
  geom_boxplot() + 
