...
# 5. prepare counter for model runs 
...
# 2. prepare model summary outputs & clean up
...
# 6. save data 
# append data for save 
#clust_sums <- bind_rows(clust_sums, clust_sum) 
#mod_sums   <- bind_rows(mod_sums, mod_sum)
#outputs    <- bind_rows(outputs, output)

#saveRDS(params, "data/params_run03")

rm(mod_nm, num_clust, num_obs, df,bic, loglik, X, clust_sum, cluster, mod_sum, 
   params, mod, BIC)
```

```{r extrapolate_data}
# 3.0  prepare to join lead and lag stations by creating lead and lag  
#      cluster columns 
gage_pairs <- output %>% 
...
# 3.1  create lead and lag stations for same clusters - NA for diff clusters 
...
# 3.3 gather the pairs to prepare to join the q1-q30 vals 
...
# 3.4 join the q1-q30 vals for the stations 
gage_pairs <- full_join(gage_pairs, output, 
                        by = "sta") 

# 3.5 join the q1-q30 vals for the station pairs & calculate diffs 
gage_pairs <- full_join(gage_pairs, output, 
                        by = c("sta_pair" = "sta")) %>% 
...
```{r calculate_estimated_dvs}
# 1. find the best fit lag pair 
miss_lag <- gage_pairs %>% 
...
# 5. calculate q, q7, q30 for the filled vals 
...
rm(miss_lag, miss_lead, gage_pairs, input, output)
```

```{r check_answers_save_dvs}
# check the fill vals 
print(gage_fill$sta)
...
# join dv data 
#gage_dv_orig <- gage_dv 

gage_dv <- bind_rows(gage_dv, gage_fill_dv) 

rm(gage_fill, clust_dv)
```
