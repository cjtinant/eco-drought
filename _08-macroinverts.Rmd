...
<!--  
Input data:  
Macroinvertebrate data is from spreadsheets created by CJ Tinant and STEM  
research assistants in collaboration with the Oglala Sioux Tribe (OST)  
...
2.0 --  import and clean data saved from Excel worksheets as csv files  
        importing from 'xlsx': 2015/04/09_OB  
2.1 -- rename duplicates  
2.2 -- check count (check for samples with lt 80 count)  
2.3 -- check by duplicate name  
2.4 -- keep original sample names  
3.0 -- organize taxonomy data  
3.1 -- remove odd characters  
3.2 -- organize genus data  
3.3 -- organize family data  
3.4 -- organize order data  
3.5 -- organize class data   
3.6 -- manual check and join data  
4.0 -- fix year and month in data  
...
# should be 12 samples with count less than 80 (e.g. 100 +/- 80)  
count_ck <- scratch %>%  
...
# check_vars n = 11  
######## ?? should be n = 13 -- but below is n = 9  ??
# 1   lwr1s7,  
# 2,3   lwr2t6 (2 samples) = lwr2t6,  
# 4   lwr3r6 = lwr3r6,  
# 5   me	ra7,          # didn't include -- fix name below  
# 6   mer4t8 = mer4t8,  
# 7   por2d7,  
# 8   por3s6,  
# 9   wcc2a6	7,  
...
check_samples <- scratch %>%  
  select(-c(order, family, notes)) %>%  
  names() %>%   
  tibble(samples = .)  
...
```{r 07_fix_taxa_names_genus_year_data}
...
# add misnamed samples to check_samples  
...
check_samples <- bind_cols(check_samples, scratch)  


```

```{r 13_data_check-manual}
...
# 12. lost dog creek split====  #### changed from LDC#######
lod <- macros %>%  
...
join5 <- full_join(ean, lod,   #### CHANGED FROM 'ldc'  
                  by = c("class", "order", "family", "genus"))  
...
```{r 26_join_dates_validate_names}
