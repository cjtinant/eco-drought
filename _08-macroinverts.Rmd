...
# organize taxa by taxonomy  
# 1.   remove difficult characters==== 
macros <- scratch %>%  
  rename(genus = notes) %>%  
  arrange(genus) %>%  
...
# 2    split into genus and no genus -- and clean non-year specific genera====  
macros_noGen <- macros %>%  
  filter(is.na(genus))  
...
# 3    add finished genus to other set of data genus ====
...
# 4.0  fix individual year data in genus====  
# 4.1  Procladius sp.'96====  
scratch <- macros_Gen %>% 
...
macros_new <- bind_rows(scratch)  

# 4.2  Stienelmis sp. '96====  
scratch <- macros_Gen %>% 
...
# 4.13 Tipula sp.b5; Subfamily Limoniinaer6====  
scratch <- macros_Gen %>% 
...
# 1 make last taxon fix====  
macros <- macros %>%  
  mutate(genus = case_when(  
    str_detect(family, "Polymitarcyidae") ~ "Ephoron sp.",  
    TRUE ~ genus)) 

# 2 clean up duplicated names created by the name cleaning above====   
...
# 3 unit test to check if clean-up is ok====  
test <- macros_cleaned %>%  
  select(class:genus, mer1d8, bea1d7) %>%  
  arrange(mer1d8, bea1d7)

test2 <- macros_cleaned %>%  
  select(class:genus, mer1d8, bea1d7) %>%  
  arrange(mer1d8, bea1d7)

# 3a looks ok -- the test and test 2 have identical non-NA observations  
macros <- macros_cleaned  

rm(test, test2, macros_cleaned)  

# 4 arrange site names, dates, notes====  
macros_wide <- macros  
...
# 5 create year variable====   
# make a look-up table for year  
yr_look_up <- macros %>%  
  select(yr_cd) %>%  
  distinct() %>%  
  arrange(yr_cd) %>%  
...
export(site_hist, "site_hist_ck.csv")  
...
```{r import_macros-prior2000}  

# 2 import and clean data saved from Excel worksheets as .csv  
...
BLC2_prior <- import("data/BLC2_prior2000.csv")  %>%  
  clean_fun()  
