...
# usethis::edit_r_environ("project") # used to increase memory allocation 
#   for dabest()
...
# Fix error in rlang::is_quosure(index_quo) 
gage_mon <- bind_rows(gage_mon1, gage_mon2) %>% 
  as_tibble()   

rm(gage_mon1, gage_mon2, decomp_input) 
...
# Unpaired mean difference of: 
#   Pine Ridge Escarpment (n=4195) minus Pierre Shale Plains (n=1261)
#   0.385 [95CI  0.304; 0.465]

#   White River Badlands (n=4195) minus Pierre Shale Plains (n=941)
#   0.75 [95CI  0.65; 0.851]

#   Black Hills Plateau (n=4195) minus Pierre Shale Plains (n=2501)
#   1.62 [95CI  1.55; 1.68]

#   Keya Paha Tablelands (n=4195) minus Pierre Shale Plains (n=1466)
#   1.78 [95CI  1.71; 1.84]

#   Sand Hills (n=4195) minus Pierre Shale Plains (n=3663)
#   2.62 [95CI  2.56; 2.67]




# calculate 95% CI for PC2 - 'control' is Pierre Shale Plains 
...
export(ci_input_pc1, "data/dabest_pc1.rdata")  


```

```{r bootstrap-swarmplot-pc1}
plot(ci_input_pc1, rawplot.type = "swarmplot")   
...
export(ci_input_pc2, "data/dabest_pc2.rdata")  
#DABEST (Data Analysis with Bootstrap Estimation) v0.2.2
#=======================================================
...
#Unpaired mean difference of 
#Pine Ridge Escarpment (n=4195) minus Pierre Shale Plains (n=1261)
# -0.132 [95CI  -0.15; -0.114]

# White River Badlands (n=4195) minus Pierre Shale Plains (n=941)
# 0.0399 [95CI  0.0152; 0.0646]

Unpaired mean difference of Black Hills Plateau (n=4195) minus Pierre Shale Plains (n=2501)
 -0.147 [95CI  -0.16; -0.133]

Unpaired mean difference of Keya Paha Tablelands (n=4195) minus Pierre Shale Plains (n=1466)
 -0.173 [95CI  -0.188; -0.159]

Unpaired mean difference of Sand Hills (n=4195) minus Pierre Shale Plains (n=3663)
 -0.0326 [95CI  -0.0463; -0.0191]
...
```{r bootstrap-swarmplot-pc2}
plot(ci_input_pc2, rawplot.type = "swarmplot")   
...
clust_input <- gage_mon %>% 
  select(sta, Date, waterYear, axis, remainder) %>% 
  spread(axis, remainder) 

X <- clust_input %>%                # this gets rid of categorical data for Mclust 
  select(-c(sta, Date, waterYear))  
...
mod_tidy <- tidy(mod) 
mod_tidy2 <- augment(mod, clust_input) 


mod_tidy3 <- mod_tidy2 %>% 
  group_by(sta, .class, waterYear) %>% 
  summarise(count = n(), 
            PC1_mon = mean(PC1_mon), 
            PC2_mon = mean(PC2_mon), 
            uncertainty = mean(.uncertainty)
            ) %>% 
  ungroup() 

mod_tidy4 <- full_join(mod_tidy3, eco_join)

mod_tidy5 <- full_join(mod_tidy3, mod_tidy4, 
                       by = "sta") %>% 
...
ggplot(mod_tidy4, aes(PC1_mon, PC2_mon)) + 
  facet_wrap(~ ecoreg) + 
  geom_jitter(aes(color = .class), 
              size = 1)  +
#  geom_point(data = mod_tidy5, 
 #            aes(PC1_mon, PC2_mon) 
 #            ) +  
  theme_bw()  
...
gage_mon <- gage_mon %>% 
  group_by(ecoreg, Date, axis) %>% 
  mutate(obs_eco  = mean(observed)) %>%   
  mutate(seas_eco  = mean(season)) %>% 
  mutate(trend_eco = mean(trend)) %>% 
  mutate(remain_eco = mean(remainder)) %>% 
  ungroup() %>% 
  group_by(ecoreg, axis) %>% 
  mutate(obs_eco_mean = mean(observed)  
        ) %>% 
  arrange(obs_eco_mean) %>% 
  ungroup() 

# prepare for plotting the PC Axes 
pca_plot <- gage_mon %>% 
  select(1:10) %>% 
  spread(axis, PCval) %>% 
#  mutate(q1_q30_rat = q1_mon / q30_mon) %>% 
  group_by(sta) %>%  
  mutate(PC1_sta = mean(PC1_mon)) %>% 
  mutate(PC2_sta = mean(PC2_mon)) %>% 
  mutate(q7_sta = mean(q7_mon)) %>% 
  mutate(q30_q1_sta = mean(q30_q1_mon)) %>% 
  ungroup() %>% 
  group_by(ecoreg) %>% 
  mutate(PC1_ecoreg = mean(PC1_mon)) %>% 
  mutate(PC2_ecoreg = mean(PC2_mon)) %>% 
  mutate(q7_ecoreg = mean(q7_mon)) %>% 
  mutate(q30_q1_ecoreg = mean(q30_q1_mon)) %>% 
  ungroup() %>%  
  arrange(PC1_ecoreg) %>% 
  mutate(ecoreg = fct_reorder( 
    as.factor(ecoreg), PC1_ecoreg)  # sets up the ordering by pc-axis 
    ) %>% 
  arrange(PC1_sta) %>%   
  mutate(sta = fct_reorder( 
    as.factor(sta), PC1_sta)  # sets up the ordering by pc-axis 
    ) %>% 
  mutate()
...
pc1_pc2_plot <- pca_plot %>% 
  ggplot(aes(PC1_mon, PC2_mon)) + 
  facet_wrap(~ ecoreg) + 
  geom_point(size = 0.5, color = "gray" 
#          aes(color = as.factor(sta)) 
          ) +  
  geom_jitter(aes(PC1_sta, PC2_sta),  # station averages 
        shape = 3,                   # cross  
        size = 1.5, 
    colour = "black") +  
  geom_point(aes(PC1_ecoreg, PC2_ecoreg),  # ecoregion averages 
...
pc1_q7_plot <-pca_plot %>% 
  ggplot(aes(y =PC1_mon, x = q7_mon))  + 
...
#      aes(color = as.factor(sta)), 
  ...
  geom_smooth(method = "lm", 
              ...
              size = 0.7) + 
  facet_wrap( ~ ecoreg) + 
  geom_jitter(aes(q7_sta, PC1_sta),  # station averages 
        shape = 3,                   # cross 
        size = 1.5, 
    colour = "black") +  
  geom_point(aes(q7_ecoreg, PC1_ecoreg),  # ecoregion averages 
             shape = 15,                   # filled square 
             size = 1.5, 
    colour = "black") +  
  theme_bw() + 
...
pc2_q_diff_plot <- pca_plot %>% 
  ggplot(aes(y =PC2_mon, x = q30_q1_mon))  + 
  geom_jitter(size = 0.7, 
#      aes(color = as.factor(sta)), 
              color = "gray" 
              ) + 
  geom_smooth(method = "lm", 
              color = "black", 
              size = 0.7) + 
  geom_jitter(aes(q30_q1_sta, PC2_sta),  # station averages 
        shape = 3,                   # cross 
        size = 1.5, 
    colour = "black") +  
  geom_point(aes(q30_q1_ecoreg, PC2_ecoreg),  # ecoregion averages 
             shape = 15,                   # filled square 
             size = 1.5, 
    colour = "black") +  
  facet_wrap( ~ ecoreg) + 
...
# PC1 - remainder plot 
pc1_remainder <- mon_plot_pc1 %>% 
  ggplot(aes(Date, remainder)) + 
  facet_wrap(~ ecoreg) + 
  geom_line(size =  0.5, 
            colour = "grey"
            ) + 
  geom_jitter(aes(Date, remainder), 
              data = anom, 
              shape = 4, 
              size = 0.7) + 
...
ggsave2("figure/pc1-deconv.png", 
        units = "in", 
        width = 7, 
        height = 9)


rm(pc1_obs, pc1_seas, pc1_trend, pc1_remainder) 
```
