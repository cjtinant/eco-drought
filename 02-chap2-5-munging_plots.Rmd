...
mod_tidy_mon <- augment(mod, clust_input)  

#mod_tidy_mon <- full_join(mod_tidy_mon, eco_join, by = "sta")
...
mod_plot <- mod_tidy_mon %>% 
  rename(q1 = q1_mon) %>% 
  rename(q7 = q7_mon) %>% 
  rename(q30 = q30_mon) %>% 
  gather(key = axis, mon, -c(sta, Date, waterYear, .class, 
                                     .uncertainty, ecoreg)) %>% 
  group_by(sta, axis) %>% 
  mutate(station = mean(mon)  # find station means 
        ) %>% 
  ungroup() %>%    
  group_by(ecoreg, axis) %>%   
  mutate(eco  = mean(mon)) %>%   # find ecoregion means 
   ungroup()  %>% 
  nest(mon, station, eco, .key = 'value_col') %>%
  spread(key = axis, value = value_col) %>%       # spread ecoreg, sta, obs 
  unnest(q1, q7, q30, .sep = '_') %>% 
  arrange(q7_station) %>% 
  mutate(sta = fct_reorder( 
    as.factor(sta), q7_station)  # sets up the ordering by pc-axis 
    ) %>% 
  arrange(q7_eco) %>%  
  mutate(ecoreg = fct_reorder( 
    as.factor(ecoreg), q7_eco)  # sets up the ordering by pc-axis 
    )  
...
plot_class <- factoextra::fviz_mclust(mod, 
                          "classification", 
                          geom = "point", 
                          ellipse.type = "t", 
                          ellipse.level = 0.7, 
...
ggplot(mod_plot, aes(q1_mon, q30_mon)) + 
  facet_wrap(~ ecoreg) + 
