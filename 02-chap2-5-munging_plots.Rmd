...
ggplot(aes(mon_PC1, mon_PC2)) + 
  ...
  geom_jitter(
    aes(shape = ecoreg, color = ecoreg), 
              size = 2)  + 
# map the title of the geom for the legend  
  scale_shape_discrete(name="Ecoregion") + 
  scale_color_discrete(name="Ecoregion") +
  geom_point(
    aes(PC1_flow_eco, PC2_flow_eco, 
      shape = ecoreg),
   show.legend=FALSE, 
    color = "black", 
              size = 3)  + 
...
rm(mclust_flow_plot, mclust_eco_plot) 
```
...
#test2 <- full_join(mod_tidy_mon, test,  by = ".class") 
#test3 <- test2 %>% 
#  group_by(.class, ecoreg) %>% 
#  mutate(q1_flow = mean(q1_mon)) %>% 
#  mutate(q7_flow = mean(q7_mon)) %>%  
#  mutate(q30_flow = mean(q30_mon))  %>% 
#  mutate(PC1_flow = mean(mon_PC1)) %>% 
#  mutate(PC2_flow = mean(mon_PC2)) %>%  
#  ungroup()  
...
#    color = "gray", 
              size = 1)  + 
  geom_point(
    aes(q1_flow, q7_flow, shape = ecoreg), 
    color = "black", 
              size = 1)  +   
  scale_x_log10() + 
  scale_y_log10() + 
...
test2 %>%  
ggplot(aes(q1_mon, q30_mon)) + 
  facet_wrap(~ flow_type) + 
  geom_jitter(
    color = "gray", 
              size = 1)  + 
  geom_point(
    aes(q1_flow, q30_flow), 
    color = "black", 
              size = 1)  +   
  scale_x_log10() + 
  scale_y_log10() + 
...
#        size = 1.5, 
#    colour = "black") +  
#  geom_point(aes(ecoreg_PC1, ecoreg_PC2, shape = ecoreg, legend = FALSE),  # ecoregion averages 
#             shape = 15,                   # filled square 
#             size = 2, 
 #   colour = "black") +  
  geom_point(
    aes(PC1_flow, PC2_flow, 
        shape = ecoreg),
    show.legend=FALSE, 
    color = "black", 
              size = 1)  + 
#  geom_point(
#    aes(q7_flow, q30_flow), 
#    color = "black", 
#              size = 1)  +   
