...
gaged <- full_join(gage_dv, env_vars, 
  ...
  select(sta, .fittedPC1:.fittedPC2, ecoreg:vpdan_sq)
...
r2_vals_train <- bind_rows(r2_val_pc1, r2_val_pc2)

# join the r2 vals & params to the RMSE  
model_fits_train <- full_join(rmse_vals_train, r2_vals_train,  # r2 vals
                            by = c("model", "axis"))

model_fits_train <- model_fits_train %>% 
  select(model, rmse = mean, axis:rsquare) %>% 
  mutate(split = "train") 
# join the train and test data  
model_fits <- bind_rows(model_fits_train, model_fits_test) 
...
model_fits <- model_fits %>% 
  mutate(numer = (1-rsquare) * (n - 1)) %>% 
  mutate(denom = (n - num_param - 1)) %>%   
  mutate(quot = numer / denom) %>% 
  mutate(r2adj = 1 - quot) #%>% 
#  mutate(r2adj = if_else(quot < 0, 0, r2adj)) 

#rm(r2_call_pc1, r2_call_pc2, r2_val_pc1, r2_val_pc2, 
   r2_vals_train, model_fits_test_pc1, 
   model_fits_test_pc2, model_fits_train, model_fits_test, 
   num_param, n_train, n_test, num_obs)  


# rmse_vals_train, train_pc1, train_pc2, test_pc1, test_pc2


# training data -------------------------------------------------------------- 
# get median Rsquared from training data 
r2_call_pc1 <- resamples(models_pc1) %>%  
  summary(metric = c("Rsquared")) 

r2_call_pc2 <- resamples(models_pc2) %>%  
  summary(metric = c("Rsquared")) 

# change the lists into tibbles 
r2_val_pc1 <- as_tibble( 
  as.data.frame(
    pluck(r2_call_pc1, 'statistics')
    ), rownames = "model"
  ) %>% 
  select(model, rsquare = Rsquared.Median) %>% 
  mutate(axis = "pc1") 
  
r2_val_pc2 <- as_tibble( 
  as.data.frame(
    pluck(r2_call_pc2, 'statistics')
    ), rownames = "model"
  ) %>% 
  select(model, rsquare = Rsquared.Median) %>% 
  mutate(axis = "pc2") 

# bind the pc1 & pc2 data, clean up
r2_vals_train <- bind_rows(r2_val_pc1, r2_val_pc2)

rm(r2_call_pc1, r2_call_pc2, r2_val_pc1, r2_val_pc2) 


model_fits_test <- bind_rows(model_fits_test_pc1, model_fits_test_pc2) %>% 
  mutate(split = "test") 
...
#  gather(model, value, -observed) %>% 
#  group_by(model) %>% 
#  mutate(rsquare = cor(observed, value)^2) %>%   # find r-square 
#  mutate(rmse = sqrt(mean((value - observed)^2))) %>% 
#  summarise(rsquare = first(rsquare), 
#            rmse    = first(rmse), 
#            mae     = first(mae) ) %>% 
#  mutate(axis = "pc2") %>% 
...
# get mae summary  
#mae_summary <- mae_vals %>% 
#  select(model, MAE = med, axis)
...
# use the test data to calculate RMSE and MAE -- get R2 later
#listviewer::jsonedit(model_fit_test_pc1) 

model_fits_test_pc2 <- fit_model_pc2 %>% 
  filter(split == "test") %>%   
  select(-split) %>% 
  map_dfc(~postResample(., fit_model_pc2$observed)) %>% 
...
rm(observed_train, preds_train, observed_test, preds_test, 
   model_fit_train, model_fit_test, mae_vals, r2_call, r2_vals, 
   fit_test, fit_train) 

rm(fit_model_pc1, fit_model_pc2) 
...
# join train and test and 
#fit_model_pc1 <- bind_rows(fit_train_pc1, fit_test_pc1) %>% 
#  mutate(split = as_factor(split)) %>% 
#  mutate(split = fct_relevel(.$split, "train", "test"))  

#fit_model_pc2 <- bind_rows(fit_train_pc2, fit_test_pc2) %>% 
#  mutate(split = as_factor(split)) %>% 
#  mutate(split = fct_relevel(.$split, "train", "test"))  
```
