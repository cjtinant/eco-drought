...
# function to decompose into seasonal, trend, remainder & finds anomolies ----
...
# Decompose a time series into trend, seasonal, and random components 


# 1. make a tibble for decomposing monthly averages 
#    by scaling daily flow values into monthly mean flows 
...
gage_mon <- gage_dv %>% 
  ...
  # drop q_vars to focus on PC vals 
#  select(-starts_with("q")) %>%              
  gather(key = axis, value = PCval, 
         -c(sta, Date, waterYear, ecoreg, q1_mon, 
            q7_mon, q30_mon, q1_q30_mon) 
         )  %>% 
  as_tibble() 


# 3. calculate parameters to remove season & trend from observed vals -------- 
# find the average monthly frequency & enframe 
mon_freq <- gage_mon  %>% 
...
# find the average monthly trend 
mon_trend <- gage_mon  %>% 
      select(-c(waterYear,ecoreg)) %>%  
  arrange(Date) %>% 
  unite(sta_axis, c(sta, axis), sep = ".") %>% 
  ...
  spread(axis, trend) %>% 
  summarise(trend = median(PC1_mon)) 
...
# map_dfr causes issues with indexing; using a manual split and combine 
gage_mon1 <- gage_mon %>% 
  filter(axis == "PC1_mon") 
...
mon_plot <- gage_mon %>% 
  ...
  mutate(obs_ave  = mean(observed)) %>%   
  mutate(seas_ave  = mean(season)) %>% 
  mutate(trend_ave = mean(trend)) %>% 
  mutate(remain_ave = mean(remainder)) %>% 
  ungroup() 
