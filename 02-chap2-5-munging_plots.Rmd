...
library("mathpix")                # support for 'Mathpix' API--Image to 'LaTeX' 
```
...
```{r convert_to_depth, message=FALSE}
...
# 1. calculate contributing drainage area in square kilometers 
...
gage_dv <- left_join(gage_dv, gage_contrib_area, by = "sta") %>% 
  select(site_no, sta, Date, Q, Q7, Q30, contrib_drain_km2) %>% 
  gather(type, value, 
         -c(site_no, sta, Date, contrib_drain_km2)
         ) %>% 
  mutate(value = value/contrib_drain_km2) %>% 
  spread(type, value) %>% 
  clean_names() %>% 
  select(-c(site_no, contrib_drain_km2)) 
