...
# drop ecoreg from the list
ecoreg_list <- lapply(ecoreg_list, function(x) select(x, -ecoreg2)) 
...
#listviewer::jsonedit(model_list) 

rm(cores, lambda, seed, reg.ctrl)
```

```{r tidy-glm-models-ecoreg}
# get & tidy model coefficients for each of the GLMs 
# having challenges with extracting elements from lists....
pshale <- model_list %>% 
  pluck("pshale")
...
#rm(train_pshale, train_escarp, train_wrbad, 
#   train_bhplat, train_kptabl, train_sandhill) 
train_ecoreg <- c("pshale", "escarp", "wrbad", "bhplat", "kptabl", "sandhill") 

preds_train_ecoreg <- map2_dfc(models_ecoreg, train_ecoreg, 
                               predict, newdata = train_ecoreg)    
...
```{r prepare-glm-model-inputs_ecoreg-delete}  
...
train_ecoreg <- bind_rows(train_pshale, 
                          train_escarp, 
                          train_wrbad
                          )
```

```{r build_lasso_models_ecoreg-delete}
