...
```{r explore_spatial_data} 

# this chunk visually explores the spatial data 
...
env_vars <- env_vars %>% 
 # mutate(cat_area_ln = 1/area_linv) %>%  
  select(sta_id, area_linv:vpdan_sq)  
...
# this code chunk calculates mean pc axes by station to prepare for regression 

# get mean pc-vals by station & join to expanatory vars  
pc_sta <- gage_dv %>% 
  group_by(sta) %>%  
  summarise( 
    pc1_sta = mean(.fittedPC1), 
    pc2_sta = mean(.fittedPC2)      
  ) 

gaged <- full_join(env_vars, pc_sta, 
                   by = c("sta_id" = "sta")) %>% 
  rename(sta = sta_id) %>% 
...
ridge_pc1 <- train( 
  pc1_sta ~.,             # x = 'medv', y = the rest of the columns, from   
...
set.seed(seed)              # need to set a seed each time you call a rand num 
ridge_pc2 <- train( 
  pc2_sta ~.,             # x = 'medv', y = the rest of the columns, from   
...
mae_call_pc1 <- resamples(models_pc1) %>%   
  summary(metric = c("MAE"))  

mae_call_pc2 <- resamples(models_pc2) %>%   
  summary(metric = c("MAE"))  

mae_vals_pc1 <- as_tibble(  
  as.data.frame( 
    pluck(mae_call_pc1, 'statistics') 
    ), rownames = "model" 
  ) %>% 
  mutate(axis = "pc1")

mae_vals_pc2 <- as_tibble(  
  as.data.frame( 
    pluck(mae_call_pc2, 'statistics') 
    ), rownames = "model" 
  ) %>% 
  mutate(axis = "pc2")

# join the mae vals together and fix the names 
mae_vals <- bind_rows(mae_vals_pc1, mae_vals_pc2) 

names(mae_vals) <- c(
  "model", "min", "Q1", "med", "mean", "Q3", "max", "na_ct", "axis") 

rm(mae_call_pc1, mae_call_pc2, mae_vals_pc1, mae_vals_pc2)
...
```{r prepare-export-table_glm-coeffs, eval=FALSE} 
...
# get observations 
obs_train_pc1    <- tibble(observed = train_pc1$pc1_sta) 
obs_train_pc2    <- tibble(observed = train_pc2$pc2_sta) 

# get predictions 
preds_train_pc1 <- map_dfc(models_pc1, predict, newdata = train_pc1) 
preds_train_pc2 <- map_dfc(models_pc2, predict, newdata = train_pc2) 

# combine observations & predictiont 
fit_train_pc1 <- bind_cols(obs_train_pc1, preds_train_pc1) %>% 
  mutate(axis = "pc1")
fit_train_pc2 <- bind_cols(obs_train_pc2, preds_train_pc2) %>% 
  mutate(axis = "pc2")

fit_train <- bind_rows(fit_train_pc1, fit_train_pc2)

rm(obs_train_pc1, obs_train_pc2, 
   preds_train_pc1, preds_train_pc2, 
   fit_train_pc1, fit_train_pc2)

#  prepare to plot test observations and predictions 
obs_test_pc1   <- tibble(observed = test_pc1$pc1_sta) 
preds_test <- map_dfc(models_pc1, predict, newdata = test_pc1) 
