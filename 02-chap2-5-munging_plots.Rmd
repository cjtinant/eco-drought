...
# get separate training and test df from the index (used in a code-chunk below)
...
try <- fit_train_ecoreg %>% 
  gather(ecoreg_mod, value, -c(observed, ecoreg, split)) %>% 
  filter(ecoreg == ecoreg_mod) %>% 
  select(-ecoreg_mod) %>% 
  group_by(ecoreg) %>% 
  mutate(null = mean(observed)) %>% 
  ungroup() 
...
preds_test_ecoreg <- map_dfc(model_list, predict, newdata = test_ecoreg)  
...
fit_model_ecoreg <-  bind_rows(fit_test_ecoreg, fit_train_ecoreg)  
 
# clean-up global environment 
rm(obs_train_ecoreg,  preds_train_ecoreg, obs_test_ecoreg, 
   preds_test_ecoreg, fit_test_ecoreg, fit_train_ecoreg)  


rm(train_bhplat, train_kptabl, train_pshale, 
   train_escarp, train_sndhil, train_wrbadl, 
   test_bhplat, test_kptabl, test_pshale, 
   test_escarp, test_sndhil, test_wrbadl)  
