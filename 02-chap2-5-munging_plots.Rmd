...
5) filter no data gages for 1980- 2018 water years 
...
```{r filter_clean_gage_meta, eval=FALSE}
...
# filter gages with short flow records----------------------------------------
...
# recreating past work -- have a difference of 19 stations!!!! 
gage_dv_meta <- import("data/gage_dv_meta.csv")

sta_names <- gage_dv_meta %>% 
  select(1:2)  %>%
    mutate(site_no = as.character(site_no)) %>% 
  mutate(site_no = zeroPad(site_no, 8)) 

gage_meta <- full_join(sta_names, gage_meta, 
                       by = "site_no")

gage_dropped <- gage_meta %>% 
  filter(is.na(sta))
```


# 1. check for incomplete days of record & years of record  
yr_incomp <- gage_dv %>%                          
