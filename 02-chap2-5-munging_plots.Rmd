...
# checking if these can be removed without harm 
rm(eco_join, ecoreg) 
``` 
...
# add number of params to error vals 
...
rm(preds_test_dv, preds_train_dv, obs_train_dv, 
   obs_test_dv, test_dv, train_dv, fit_test_dv, fit_train_dv)
...
```{r plot_glm_observations_predictions_dv_in-progress}
...
# %>%               # get residuals 
  gather(type, value, -c(Date, ecoreg, model, MAE, residual)) %>% 
  mutate(type = case_when( 
    type == "fitted" ~ model, 
    TRUE ~ type)) %>% 
  select(-model) %>% 
  mutate(my_rank = 1- percent_rank(value)) %>%
  mutate(my_rank = if_else(my_rank == 1, 0.9999, my_rank)) 

#try2 <- try %>% 
#  group_by(Date, split, model) %>% 
#  summarise(fitted = mean(fitted), 
#            obserced = mean(observed)) %>% 
#  ungroup() 
...
ecoreg_list <- lapply(ecoreg_list, function(x) select(x, -c(ecoreg2, Date)))  
...
# -- this fits the right models to the right ecoregions -- 
...
# clean-up global environment 
rm(fit_train_ecoreg, fit_test_ecoreg, train_ecoreg, test_ecoreg, 
   obs_train_ecoreg,  preds_train_ecoreg, obs_test_ecoreg, 
   preds_test_ecoreg, fit_test_ecoreg, fit_train_ecoreg, 
   train_bhplat, train_kptabl, train_pshale, train_escarp, 
