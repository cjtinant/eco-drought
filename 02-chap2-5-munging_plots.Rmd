...
```{r EDA_02-transforms_spatial-data} 

print(lambda) 

# transform variables in the following way:  
## variable    lambda_val- gaged      transform 
## ------------------------------------------ 
...
# get the watershed summaries with the stations attached 
#gaged <- wsd_summary %>% 
#  filter(type == "gaged") 

# keep just the uncorrelated and transformed variables 
gaged <- right_join(wsd_summary, env_vars, 
                    by = c("sta_id", "lw_ratio", "drain_dens", 
                           "prcp_mean", "t07_mean", "vpd_ann", 
                           "vpd_07", "cat_out", "cat_rel", 
                           "slop_med", "TWI_mean", "perc_cov", 
                           "fc_mean", "ksat_mean", "kvert_mean") 
                    ) %>% 
  select(sta_id, area_linv:vpdan_sq) %>% 
  select(-c(ksat_ln, vpd07_sq))  
...
pca_PC1 <- prcomp(resp_vars_PC1[,c(2:4)], 
              center = TRUE, scale. = TRUE) 

tidy(pca, "pcs") 

resp_vars_PC1 <- augment(pca, data = resp_vars_PC1) %>% 
