...
gaged <- full_join(gage_dv, env_vars, 
  ...
  select(sta, q1_depth, .fittedPC1:.fittedPC2, ecoreg:vpdan_sq) 
...
ridge_dv <- train(  
  ...
  data = train_dv,        #   the dataset 'train.data' 
...
rmse_vals_dv <- full_join(rmse_vals_dv, num_param, 
                              by = "model")
...
rmse_call_dv <- resamples(models_dv) %>%   
  summary(metric = c("RMSE", "MAE"))  

# change list to tibble 
rmse_vals_dv <- as_tibble(   
  as.data.frame( 
    pluck(rmse_call_dv, 'statistics')  
    ), rownames = "model" 
  )   %>% 
...
ggplot(rmse_vals_dv) + 
  ...
  geom_boxplot(aes(x = as.factor(model_text), 
                   group = model, 
      lower = `1st`, 
      upper = `3rd`, 
      middle = `Median`, 
  ...
  ylim(0, 1.0) +  
  xlab("") + 
  ylab("Log10 of mean daily flow per unit area") +   
  theme_bw() 
...
# clean-up global environment 
rm(obs_train_dv,  preds_train_dv, obs_test_dv, 
   preds_test_dv, fit_test_dv, fit_train_dv)  
...
fit_model_dv_plot <- fit_model_dv %>%  
  select(-c(ridge, elastic)) %>% 
  mutate(obs_antilog = 10^observed) %>% 
  mutate(fit_antilog = 10^lasso)    

ggplot(fit_model_dv_plot, aes(obs_antilog, fit_antilog)) +  
...
```{r get_glm_model-fits_dv}

# get train data model statistics --------------------------------------------  
# dv model fits 
model_fits_train_dv <- fit_model_dv %>%  
  filter(split == "train") %>%    
...
# dv model fits 
model_fits_test_dv <- fit_model_dv %>% 
  filter(split == "test") %>%    
  gather(model, value, -c(observed, split)) %>% 
  group_by(model) %>% 
  mutate(rmse = rmse(.$observed, .$value)) %>% 
...
model_fits <- bind_rows(model_fits_train_dv, model_fits_test_dv) %>%  
...
rm(model_fits_train_dv, model_fits_test_dv, 
...
```{r prepare-glm-model-inputs_ecoreg}  

# clean up model data from above 
rm(train_dv, test_dv, elastic_dv, lasso_dv, ridge_dv, models_dv,
   rmse_vals_dv, fit_model_dv, fit_model_dv_plot, model_fits)  
...
gaged_ecoreg <- gaged %>% 
  select(-c(prcp_sq, crel_sqr, vpdan_sq, t07_sq, TWI_sq)) 

# Split the data into training and test sets by ecoreg ----------------------- 
# get train and test one by one -- a bit klugy but works... 

# pierre shale --------------------------------------------------------------- 
dv_pshale <- gaged_ecoreg %>% 
  filter(ecoreg == "Pierre Shale Plains") 

set.seed(seed) 
index_pshale <- dv_pshale %>% 
     select(sta) %>% 
     unique() %>% 
     mutate(len = length(sta)) %>% 
     sample_n(0.8*.$len) %>% 
     select(-len) 

train_pshale <- index_pshale %>%  
   semi_join(dv_pshale, ., 
             by = "sta") %>% 
  select(-c(sta, q1_depth, .fittedPC1, .fittedPC2, ecoreg))   

test_pshale <- anti_join(dv_pshale, train_pshale)  

# pine ridge escarpment  ----------------------------------------------------- 
dv_escarp <- gaged_ecoreg %>% 
  filter(ecoreg == "Pine Ridge Escarpment")  

set.seed(seed) 
index_escarp <- dv_escarp %>% 
     select(sta) %>% 
     unique() %>% 
     mutate(len = length(sta)) %>% 
     sample_n(0.8*.$len) %>% 
...
train_escarp <- index_escarp %>%  
   semi_join(dv_escarp, ., 
             by = "sta") %>% 
  select(-c(sta, q1_depth, .fittedPC1, .fittedPC2, ecoreg))   

test_escarp <- anti_join(dv_escarp, train_escarp)  

# white river badlands ------------------------------------------------------ 
dv_wrbad <- gaged_ecoreg %>% 
  filter(ecoreg == "White River Badlands") 

set.seed(seed) 
index_wrbad <- dv_wrbad %>% 
     select(sta) %>% 
     unique() %>% 
     mutate(len = length(sta)) %>% 
     sample_n(0.8*.$len) %>% 
...
train_wrbad <- index_wrbad %>%  
   semi_join(dv_wrbad, ., 
             by = "sta") %>% 
  select(-c(sta, q1_depth, .fittedPC1, .fittedPC2, ecoreg))   

test_wrbad <- anti_join(dv_wrbad, train_wrbad)  

# black hills plateau ------------------------------------------------------- 
dv_bhplat <- gaged_ecoreg %>% 
  filter(ecoreg == "Black Hills Plateau") 

set.seed(seed) 
index_bhplat <- dv_bhplat %>% 
     select(sta) %>% 
     unique() %>% 
     mutate(len = length(sta)) %>% 
     sample_n(0.8*.$len) %>% 
...
train_bhplat <- index_bhplat %>%  
   semi_join(dv_bhplat, ., 
             by = "sta") %>% 
  select(-c(sta, q1_depth, .fittedPC1, .fittedPC2, ecoreg))   

test_bhplat <- anti_join(dv_bhplat, train_bhplat)  

# keya paha tablelands  ------------------------------------------------------ 
dv_kptabl <- gaged_ecoreg %>% 
  filter(ecoreg == "Keya Paha Tablelands") 

set.seed(seed) 
index_kptabl <- dv_kptabl %>% 
     select(sta) %>% 
     unique() %>% 
     mutate(len = length(sta)) %>% 
     sample_n(0.8*.$len) %>% 
...
train_kptabl <- index_kptabl %>%  
   semi_join(dv_kptabl, ., 
             by = "sta") %>% 
  select(-c(sta, q1_depth, .fittedPC1, .fittedPC2, ecoreg))   

test_kptabl <- anti_join(dv_kptabl, train_kptabl)  

# sand hills  --------------------------------------------------------------- 
dv_sandhill <- gaged_ecoreg %>% 
  filter(ecoreg == "Sand Hills") 

set.seed(seed) 
index_sandhill <- dv_sandhill %>% 
     select(sta) %>% 
     unique() %>% 
     mutate(len = length(sta)) %>% 
     sample_n(0.8*.$len) %>% 
...
train_sandhill <- index_sandhill %>%  
   semi_join(dv_sandhill, ., 
             by = "sta") %>% 
  select(-c(sta, q1_depth, .fittedPC1, .fittedPC2, ecoreg))   

test_sandhill <- anti_join(dv_sandhill, train_sandhill)  

```

```{r build_lasso_models_ecoreg}

# pshale - lasso -- alpha = 1 -----------------------------------------------  
set.seed(seed)              # need to set a seed each time you call a rand num 
lasso_pshale <- train( 
  log_q1_depth ~.,            # x = 'medv', y = the rest of the columns, from   
  data = train_pshale,         #   the dataset 'train.data'   
  method = "glmnet",        #   using method "glmnet" 
  preProcess = c("center", "scale"),  
  trControl = reg.ctrl,  
  tuneGrid = expand.grid(alpha = 1, lambda = lambda) # df with tuning values 
...
# escarp - lasso -- alpha = 1 -----------------------------------------------  
set.seed(seed)              # need to set a seed each time you call a rand num 
lasso_escarp <- train( 
  log_q1_depth ~.,            # x = 'medv', y = the rest of the columns, from   
  data = train_escarp,         #   the dataset 'train.data'   
  method = "glmnet",        #   using method "glmnet" 
  preProcess = c("center", "scale"),  
  trControl = reg.ctrl,  
  tuneGrid = expand.grid(alpha = 1, lambda = lambda) # df with tuning values 
...
lasso_wrbad <- train( 
  log_q1_depth ~.,            # x = 'medv', y = the rest of the columns, from   
  data = train_wrbad,         #   the dataset 'train.data'   
  method = "glmnet",        #   using method "glmnet" 
  preProcess = c("center", "scale"),  
  trControl = reg.ctrl,  
  tuneGrid = expand.grid(alpha = 1, lambda = lambda) # df with tuning values 
...
# bhplat - lasso -- alpha = 1 -----------------------------------------------  
set.seed(seed)              # need to set a seed each time you call a rand num 
lasso_bhplat <- train( 
  log_q1_depth ~.,            # x = 'medv', y = the rest of the columns, from   
  data = train_bhplat,         #   the dataset 'train.data'   
  method = "glmnet",        #   using method "glmnet" 
  preProcess = c("center", "scale"),  
  trControl = reg.ctrl,  
  tuneGrid = expand.grid(alpha = 1, lambda = lambda) # df with tuning values 
...
# kptabl - lasso -- alpha = 1 -----------------------------------------------  
set.seed(seed)              # need to set a seed each time you call a rand num 
lasso_kptabl <- train( 
  log_q1_depth ~.,            # x = 'medv', y = the rest of the columns, from   
  data = train_kptabl,         #   the dataset 'train.data'   
  method = "glmnet",        #   using method "glmnet" 
  preProcess = c("center", "scale"),  
  trControl = reg.ctrl,  
  tuneGrid = expand.grid(alpha = 1, lambda = lambda) # df with tuning values 
...
# sandhill - lasso -- alpha = 1 -----------------------------------------------  
set.seed(seed)              # need to set a seed each time you call a rand num 
lasso_sandhill <- train( 
  log_q1_depth ~.,            # x = 'medv', y = the rest of the columns, from   
  data = train_sandhill,         #   the dataset 'train.data'   
  method = "glmnet",        #   using method "glmnet" 
  preProcess = c("center", "scale"),  
  trControl = reg.ctrl,  
  tuneGrid = expand.grid(alpha = 1, lambda = lambda) # df with tuning values 
...
```{r not-run_functional_programming_lasso_by_ecoreg}
# this is a future implementation ------------------------------------------ 
#df <- data.frame(x = 1:10000, y = sample(1:1000, 10000, replace = TRUE), 
#                 group = sample(c('A', 'B', 'C'), 10000, 
#                                replace = TRUE, prob = c(.1, .5, .4))) 
...
#  split(gaged$ecoreg) 
# 
# gaged_list <- lapply(gaged_list, function(x) select(x, -ecoreg)) 
# 
#train_gaged <- lapply(gaged_list, function(x) { 
#  return(createDataPartition(x$log_q1_depth, p = .8, list = FALSE)) 
#  }) 
# 
# 
# model_list <- purrr::map2(gaged_list, train_gaged, 
#                          function(gaged, train_index)  { 
#                            train(log_q1_depth ~., df[gaged_index,], 
#                                  method = 'glmnet', 
#                                  trControl = reg.ctrl,  
#                                  preProcess = c('nzv', 'center', 'scale')) 
#                            }) 
...
# get coeffs  ------------------------------------------------------------- 
coef_lasso_pshale <- coef( 
  lasso_pshale$finalModel, 
  lasso_pshale$bestTune$lambda) %>% 
  as.matrix() %>%    
  as_tibble(., rownames = "coeff") %>% 
  rename(pshale = 2) 

coef_lasso_escarp <- coef( 
  lasso_escarp$finalModel, 
  lasso_escarp$bestTune$lambda) %>% 
  as.matrix() %>%    
  as_tibble(., rownames = "coeff") %>% 
  rename(escarp = 2) 

coef_lasso_wrbad <- coef( 
  lasso_wrbad$finalModel, 
  lasso_wrbad$bestTune$lambda) %>% 
  as.matrix() %>%    
  as_tibble(., rownames = "coeff") %>% 
  rename(wrbad = 2) 

coef_lasso_bhplat <- coef( 
  lasso_bhplat$finalModel, 
  lasso_bhplat$bestTune$lambda) %>% 
  as.matrix() %>%    
  as_tibble(., rownames = "coeff") %>% 
  rename(bhplat = 2) 

coef_lasso_kptabl <- coef( 
  lasso_kptabl$finalModel, 
  lasso_kptabl$bestTune$lambda) %>% 
  as.matrix() %>%    
  as_tibble(., rownames = "coeff") %>% 
  rename(kptabl = 2) 

coef_lasso_sandhill <- coef( 
  lasso_sandhill$finalModel, 
  lasso_sandhill$bestTune$lambda) %>% 
  as.matrix() %>%    
  as_tibble(., rownames = "coeff") %>% 
  rename(sandhill = 2) 

# join model coeff ----------------------------------------------------------- 
coefs_ecoreg <- bind_cols( 
  coef_lasso_pshale, 
  coef_lasso_escarp, 
  coef_lasso_wrbad, 
  coef_lasso_bhplat, 
...
# remove dupicate columns 
  select(coeff, pshale, escarp, wrbad, bhplat, kptabl, sandhill) %>% 
