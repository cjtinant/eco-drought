...
pca_plot <- gage_mon %>% 
  ...
  mutate(q7_sta = median(q7_mon)) %>% 
  mutate(q1_q30_sta = median(q1_q30_rat)) %>% 
  ungroup() %>% 
  group_by(ecoreg) %>% 
  mutate(PC1_ecoreg = median(PC1_mon)) %>% 
  mutate(PC2_ecoreg = median(PC2_mon)) %>% 
  mutate(q7_ecoreg = median(q7_mon)) %>% 
  mutate(q1_q30_ecoreg = median(q1_q30_rat)) %>% 
  ungroup() %>%  
...
#      aes(color = as.factor(sta)), 
  ...
  geom_smooth(method = "lm", 
              color = "black", 
              size = 0.7) + 
  geom_jitter(aes(q1_q30_sta, PC2_sta),  # station averages 
        shape = 3,                   # cross 
        size = 1.5, 
    colour = "black") +  
  geom_point(aes(q1_q30_ecoreg, PC2_ecoreg),  # ecoregion averages 
             shape = 15,                   # filled square 
             size = 1.5, 
    colour = "black") +  
  facet_wrap( ~ ecoreg) + 
  ...
  scale_x_log10() + 
  theme(legend.position="none") + 
  xlab("Ratio between 1-day and 30-day mean daily flow depths") + 
  ylab("PC2 axis (4.5%)") 
...
mon_plot_pc1 <- gage_mon %>% 
    filter(axis == "PC1_mon") %>% 
  mutate(ecoreg = fct_reorder(
    as.factor(ecoreg), pca_plot$PC1_sta)  # sets up the ordering by pc-axis 
    ) %>% 
  mutate(sta = fct_reorder(
    as.factor(sta), pca_plot$PC1_sta)  # sets up the ordering by pc-axis 
    ) 
...
mon_sum_pc1_all <- mon_plot_pc1 %>% 
  summarize(median_pc1 = round(mean(observed), digits = 2), 
...
#  select(sta, ecoreg, observed, season, trend, remainder) %
  group_by(ecoreg) %>% 
  summarize(median_pc1 = round(mean(obs_eco), digits = 2), 
            obs_eco    = median(abs(obs_eco)), 
            seas_eco   = median(abs(seas_eco)), 
            trend_eco  = median(abs(trend_eco)), 
            remain_eco = median(abs(remain_eco)) 
