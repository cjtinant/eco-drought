...
#NEED TO SAVE OTHER RESULTS 

```

```{r clean_global_and_make_tables} 
# need to decide on how to clean up the global environmenta and what 
# tables to keep 
```
...
# 1.2. dv plots too busy -- scale daily flow values into monthly mean depths  
gage_mon <- gage_dv %>% 
  mutate(yr = year(Date)) %>% 
...
# 3.2 prepare for plotting by 
gage_mon <- gage_mon %>% 
  # Fixes error in rlang::is_quosure(index_quo) 
  as_tibble() %>%                    
...
pca_plot <- gage_mon %>% 
  ...
  spread(axis, PCval) %>% 
  group_by(sta) %>% 
  mutate(PC1_sta = median(PC1_mon)) %>% 
  mutate(PC2_sta = median(PC2_mon)) %>% 
  ungroup() %>% 
  group_by(ecoreg) %>% 
  mutate(PC1_ecoreg = median(PC1_mon)) %>% 
  mutate(PC2_ecoreg = median(PC2_mon)) %>% 
  ungroup() %>% 
  mutate(ecoreg = fct_reorder(
    as.factor(ecoreg), PC1_sta)  # sets up the ordering by pc-axis 
    ) %>% 
...
pc1_pc2_plot <- pca_plot %>% 
  ...
  geom_segment(data = pca_vars, 
    aes(x = 0, y = 0, xend = PC1, yend = PC2),  # pc eigenvectors 
  ...
  labs(title = "Principal components of mean daily streamflow for water years 1980-2017", 
       subtitle = "Monthly averages of N = 42 stations") + 
  xlab("PC1 axis (94.8%)") + 
  ylab("PC2 axis (4.5%)")
...
pc1_q7_plot <-pca_plot %>% 
  ...
  geom_jitter(aes(color = as.factor(sta)), 
             size = 0.7) + 
...
pc2_q_diff_plot <- pca_plot %>% 
  ggplot(aes(y =PC2_mon, x = q1_q30_rat))  + 
  geom_jitter(aes(color = as.factor(sta)), 
             size = 0.7) + 
  geom_smooth(method = "lm", 
              color = "black", 
  ...
  scale_x_log10() + 
  theme(legend.position="none") + 
  xlab("Ratio between 1-day and 30-day mean daily flow ratio") + 
  ylab("PC2 axis (4.5%)") 
...
pca_plot %>% 
  ggplot(aes(y =PC2_mon, x = q30_q1_mon + 0.1))  + 
  geom_jitter(aes(color = as.factor(sta)), 
             size = 0.7) + 
...
#  scale_y_log10() + 
  theme(legend.position="none") + 
  xlab("1-day mean daily flow") + 
  ylab("PC2 axis (4.5%)") 


# Prepare for PC1 vs Q 
#pca_plot2 <- pca_plot %>% 
#  select(sta:PC1_mon) %>% 
#  select(-q30_q1_mon) %>% 
#  gather(type, val, -c(sta, Date, waterYear, ecoreg, PC1_mon)) %>% 
#  mutate(type = as.factor(type)) %>% 
  # sets up the ordering by pc-axis 
#  mutate(type = fct_relevel(type, c("q1_mon", "q7_mon", "q30_mon"))  
#    ) 


#pca_plot2 %>% 
#  ggplot(aes(x =PC1_mon, y = val))  + 
#  geom_jitter(aes(color = as.factor(sta))) + 
#  geom_smooth(method = "lm", color = "black", size = 0.5) +
#  scale_y_log10() +  
#  facet_grid(type ~ ecoreg) + 
#  theme_bw() + 
#  xlab("Mean monthly PC1 value") + 
#  theme(legend.position="none")  
```
...
mon_sum_pc1 <- mon_plot_pc1 %>% 
#  select(sta, ecoreg, observed, season, trend, remainder) %
  group_by(ecoreg) %>% 
  summarize(median_pc1 = round(median(obs_eco), digits = 2), 
  ...
  ungroup() %>% 
  select(ecoreg, median_pc1, interann_var, multi_yr_var, remainder)
