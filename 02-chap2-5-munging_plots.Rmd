...
# 1. scale daily flow values into monthly mean flows 
gage_mon <- gage_dv %>% 
  mutate(yr = year(Date)) %>% 
...
# 2. make a tibble for decomposing monthly averages 
anom_mon <- gage_mon %>% 
  # drop q_vars to focus on PC vals 
  select(-starts_with("q")) %>%              
  gather(key = axis, value = PCval, 
         -c(sta, Date, waterYear, ecoreg) 
         )  

# 3. calculate a frequency parameter to adjust the "season" component that is removed from the "observed" values. 
...
# find the average frequency & enframe 
mon_freq <- mean(mon_freq$freq)   

mon_freq <- enframe(mon_freq) %>%
  select(freq = value)

# 4. find the characteristic time trend for pca data 
mon_trend <- anom_mon  %>% 
      select(-c(waterYear,ecoreg)) %>%  
  arrange(Date) %>% 
  unite(sta_axis, c(sta, axis), sep = ".") %>% 
...
# 6. mapping the time_decompose function requires data of the same length 
  
  
  

gage_anom_stl <- anom_mon %>% 
  filter(axis == "PC1_mon") %>% 
  arrange(Date) %>% 
#  filter(sta == "hat_edg") #%>% 
 # split(.$sta) %>% 
  map_dfc(~ time_decompose(
    data         = ., 
    method       = "stl", 
    target       = PCval, 
...
plot_anomaly_decomposition(gage_anom_stl) 


# Basic Usage 
stl <- tidyverse_cran_downloads %>%
