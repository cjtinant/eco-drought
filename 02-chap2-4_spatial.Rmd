...
# plot model predictions------------------------------------------------------  
model_pred_l <- obs_fitted %>% 
  gather(key = "model", value = "fitted", -c(observed, split)) %>%
  filter(model != "ridge") 

model_pred_l$split <- factor(model_pred_l$split, 
                             levels=c("train", "test")
                             ) 

ggplot(model_pred_l, aes(observed, fitted)) + 
...
write_csv(obs_preds, path = "figure/sup_obs_preds.csv") 
``` 
