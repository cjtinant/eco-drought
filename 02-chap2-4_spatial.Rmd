...
<!--
Purpose: 
develop a regression  equation for gaged stations by fitting PC1--the 
hydrolologic export coefficient of mean daily streamflow to environmental variables.  Then estimate PC1 for ungaged stations  
...
Regularization methods introduce bias into the regression solution but reduce 
...
library("fuzzyjoin")    # join tables together on inexact matching
library("ggrepel")      # repulsive textual annotations 
...
## ------------------------------------------ 
## cat_area_ln	-1.00			    reciprocal     1
## cat_out	     0.36			    square root    2
## cat_rel       0.46			    square root 	 3		
## drain_dens	   0.66			    square root 	 4		
## fc_mean	     1.36                        5
## kvert_mean	  -0.13			    natural log    6
## ksat_mean  	-0.11			    natural log    7 cut
## lw_ratio	    -0.18			    natural log    8
## perc_cov	     0.17			    natural log    9
## prcp_mean	   2.00			    square        10
## slop_med	    -0.13			    natural log   11
## t07_mean	     2.00			    square        12
## TWI_mean	     2.00				  square        13
## vpd_ann	     2.00				  square		    14
## vpd_07	       2.00					square	      15 cut
...
mae_call <- resamples(models) %>%  
  summary(metric = c("MAE")) 

mae_vals <- as_tibble( 
  as.data.frame(
    pluck(mae_call, 'statistics')
    ), rownames = "model"
  )
...
# plot model MAE
ggplot(mae_vals, aes(x = as.factor(model))) + 
...
# use the test data to calculate Rsquared and MAE 
model_fit_test <- fit_test %>% 
...
# get median Rsquared and MAE from training data 
r2_call <- resamples(models) %>%  
  summary(metric = c("Rsquared")) 

r2_vals <- as_tibble( 
  as.data.frame(
    pluck(r2_call, 'statistics')
    ), rownames = "model"
  ) %>% 
  select(model, Rsquared = Rsquared.Median)

mae_vals <- mae_vals %>% 
  select(model, MAE = med)

# arrange model fit data from training 
model_fit_train <- tibble(
  model = c("elastic", "lasso", "ridge")
  ) %>% 
  mutate(split = "train") 
  
model_fit_train <- full_join(model_fit_train, r2_vals, 
                             by = "model")   

model_fit_train <- full_join(model_fit_train, mae_vals, 
                             by = "model") 

# combine the training and test observation dataframes 
fit_train <- fit_train %>% 
  mutate(split = "train")
...
model_fit <- bind_rows(model_fit_train, model_fit_test)
obs_fitted <- bind_rows(fit_train, fit_test)

rm(observed_train, preds_train, observed_test, preds_test, 
   model_fit_train, model_fit_test, mae_vals, r2call, r2_vals, 
   fit_test, fit_train)
...
model_fit <- full_join(model_fit, num_obs, 
                  by = "split") 

rm(num_param, num_obs, test.data, train.data)

# calculate adj R2
model_fit <- model_fit %>% 
  mutate(
    R2adj = 
      Rsquared - ((1-Rsquared) * (num_param) / (n - num_param -1)) 
  ) %>% 
  mutate(R2adj = if_else(R2adj > 1, 0, R2adj)
  ) 
```
...
# prepare to add text 
num_obs_text <- model_fit %>%
  select(model, split, n)
  
adjR2_text <- model_fit %>% 
    select(model, split, R2adj) 

mae_text <- model_fit %>% 
    select(model, split, MAE)
...
write_csv(coefs, path = "output/super_coefs.csv") 
write_csv(model_fit, path = "output/super_fit.csv") 
write_csv(obs_fitted, path = "output/sup_obs_preds.csv") 

rm(obs_fitted_l, adjR2_text, model_fit, 
   adjR2_text, mae_text, num_obs_text) 

# explore correlations results
env_cor <- correlate(env_vars) %>%                 # create a correlation df
  shave() %>%                                      # shave off bottom half
  select(rowname, dden_sqr, fc_mean, crel_sqr) %>% # keep only important vars 
  replace(., is.na(.), 0) %>%                     # change na to zero    
  mutate(dden_abs = abs(dden_sqr)) %>%
  mutate(fc_abs = abs(fc_mean)) %>% 
  mutate(crel_abs = abs(crel_sqr)) %>%     
  mutate(abs_sum = dden_abs + fc_abs + crel_abs) %>%     # make abs vals & add
  arrange(abs_sum) %>% 
  filter(abs_sum > 0.9) %>%                  # remove low correlation cols 
  filter(rowname != "ksat_ln") %>% 
  select(rowname, dden_sqr, fc_mean, crel_sqr)
  
# plot highly correlated vars 
env_cor_fn <- env_vars %>% 
  select(env_cor$rowname, dden_sqr, fc_mean, crel_sqr) %>% 
  select(-vpd07_sq)

names(env_cor_fn)
names(env_cor_fn) <- c("cover", "twi", "T_jul", "vpd", "prcp",  "kvert", "DDEN", "FCAP", "CREL")

env_cor_fn <- env_cor_fn %>% 
  cor() %>% 
  round(., 2) 

  
ggcorrplot(env_cor_fn, hc.order = FALSE, type = "lower",
   lab = TRUE, insig = "blank", lab_size = 3, digits = 1,  
...
elastic.predict <- tibble(hydro_fit = 
  predict(elastic, newdata = pred.data)
  )   

# prepare to join gaged and ungaged 
ungaged <- bind_cols(elastic.predict, ungaged) %>% 
  select(sta_id, hydro_fit, everything()) %>% 
  arrange(hydro_fit) %>% 
  select(sta_id, hydro_fit) %>% 
#  select(-c(area_linv, cout_sqr, crel_sqr, lwrat_sqr, 
#           pcov_ln, slop_ln, t07_sq, TWI_sq)) %>% 
  mutate(dummy = 0)                             # needed to satisfy 'fuzzyjoin'

obs_fitted <- obs_fitted %>% 
  select(hydro_exp = observed, hydro_fit = elastic)

gaged <- full_join(gaged, obs_fitted, 
                   by = "hydro_fit")
...
sta_id.preds <- ungaged %>%
  difference_inner_join(., gage_join, 
                      by = c("hydro_fit", "dummy"), 
                      distance_col = "dist") %>% 
  select(-starts_with("dummy")) %>% 
  select(starts_with("sta_id"), everything()) %>% 
  rename(sta_id_ungaged = sta_id.x) %>% 
  rename(sta_id_gaged = sta_id.y) %>% 
  rename(hydro_exp_ungaged = hydro_fit.x) %>% 
  rename(hydro_exp_gaged = hydro_fit.y) %>% 
  arrange(hydro_fit.dist) %>%
  distinct(sta_id_ungaged, .keep_all = TRUE) 

write_csv(sta_id.preds, path = "output/sta_id_preds.csv") 
```

```{r plot_sta_id_results} 


ggplot(sta_id.preds, aes(hydro_exp_gaged, hydro_exp_ungaged)) + 
  geom_jitter() + 
  geom_abline(intercept = 0, slope = 1, size = 0.25) +
  xlim(-1.6, 1.6) + 
  ylim(-1.6, 1.6) + 
  ...
  geom_text_repel(aes(label = sta_id_ungaged),
                  size          = 2,  
                  segment.size  = 0.2,
                  segment.color = "grey50") +
  theme_classic() + 
  labs(title = 
         "Predicted hydrologic export coefficients for ungaged stations") + 
  xlab("Fitted hydrologic export--ungaged stations") + 
  ylab("Hydrologic export--nearest gaged station")
