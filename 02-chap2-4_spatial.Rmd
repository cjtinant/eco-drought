...
```{r prepare-for-adj-Rsq}
#rsquare_call <- resamples(models) %>%   
#  summary(metric = c("Rsquared")) 

#rsquare_vals <- as_tibble( 
#  as.data.frame(
#    pluck(rsquare_call, 'statistics')
#  ), rownames = "model"
#)
...
#  select(-c(num_param, n_test, n_train)) 

rm(num_param, n_test, n_train)
...
rm(obs_fitted_l, adjR2_text, obs_fitted, model_fit, train.data, test.data) 
...
gaged_fin <- gaged %>% 
  select(sta_id, hydro_exp, dden_sqr, )
...
ungaged <- wsd_summary %>% 
  ...
  mutate(prcp_sq    = prcp_mean^2) %>% 
  mutate(vpdan_sq   = vpd_ann^2) %>%   		
  select(sta_id, dden_sqr, fc_mean, kvert_ln, prcp_sq, vpdan_sq) 
