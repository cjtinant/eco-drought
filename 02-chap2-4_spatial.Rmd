...
# transform environmental vars for training dataset 
train <- wsd_summary %>% 
  filter(type == "gaged") %>% 
...
# PCA of streamflow summaries & sweep up PCs
...
# join explanatory and response vars
train <- full_join(train, resp_vars, 
                   by = "sta_id") %>% 
  arrange(hydro_exp) %>% 
...
rm(env_vars, gage_summary, lambda, pca, resp_vars)  
```
