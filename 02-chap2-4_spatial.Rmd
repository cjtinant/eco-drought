...
# create objects for storing the outcome variable &  predictor variables 
# using the function model.matrix()  automatically transform qualitative 
# variables (if any) into dummy variables.  Note glmnet() can only take 
# numerical, quantitative inputs. 
# The intercept component at index = 1 is removed after creating the 
# model matrix. 
...
# ridge
set.seed(123)                # need to set a seed each time you call a rand num
ridge <- train(
...
# WORKING ON CREATING AN OBSERVED VS PREDICTED PLOT - NOT DONE YET
#  Observed = sapply(models, predict, newdata = test.data)
#  )
# Validate on test set with ensemble
allPreds <- tibble(Observed = models$ridge
ensemblePred <- tibble(
  Predicted = rowSums(allPreds) / length(models)
  ) 
