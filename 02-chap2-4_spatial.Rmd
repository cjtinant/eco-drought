...
# check dimensionality of data & missing observations 
plot_str(train)
plot_missing(train) 
```

```{r EDA_train_02-histograms}
plot_histogram(train) 

## visual analysis of histogram
## the following look 'mostly' normally distributed: 
##   cat_area_ln, cat_out, drain_dens, fc_mean, lw_ratio, prcp_mean, slop_med, 
##   TWI_mean  
## 
## the following look 'mostly' right-skewed:
##   ksat_mean, kvert_mean, perc_cov, cat_rel 

## the following look 'mostly' left-skewed:
##   vpd_07, vpd_ann, t07_mean , 

``` 

```{r EDA_train_03-box-cox}
# use Box Cox to estimate transforms for the following vars:
#   - right-skewed vars: ksat_mean, kvert_mean, perc_cov, cat_rel 
#   - left-skewed vars: vpd_07, vpd_ann, t07_mean 

scratch <- train %>% 
  select(ksat_mean, kvert_mean, perc_cov, cat_rel, vpd_07, vpd_ann, t07_mean) 

lambda <- enframe(
  sapply(scratch, forecast::BoxCox.lambda)
  ) %>% 
  rename(sta = name) %>% 
...
```{r EDA_train_04-transforms} 

# transform variables in the following way: 
## variable    lambda_val     transform
## kvert_mean	-0.22254823		 natural log 
## cat_rel	  -0.17893201		 natural log 			
## ksat_mean	 0.04190197		 natural log 			
## perc_cov	   0.05849083		 natural log 		
## vpd_07	     1.99992425		   square	
## vpd_ann	   1.99992425			 square	   		
## t07_mean	   1.99992425		   square	
