...
# 5 linear models of severity vs L1====    
mod1_L1_sev <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(severity ~ L1 + threshold * sci_n, data = .) %>%  
  glance() %>%  
  mutate(model = "interaction") 

mod2_L1_sev <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(severity ~ L1 + threshold + sci_n, data = .) %>%  
  glance() %>%  
  mutate(model = "all")   

mod3_L1_sev <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(severity ~ L1 + threshold, data = .) %>%  
  glance() %>%  
  mutate(model = "no sci_n")  

mod4_L1_sev <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(severity ~ L1 + sci_n, data = .) %>%  
  glance() %>%  
  mutate(model = "no threshold")  

mod5_L1_sev <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(severity ~ L1, data = .) %>%  
  glance() %>%  
  mutate(model = "only L1") 

# 6 sum and test model fits====  
# * AIC & BIC slightly different -- chose all to fit with below  
mod_L1_sev_sum <- bind_rows(mod1_L1_sev,  
                            mod2_L1_sev,  
                            mod3_L1_sev,  
                            mod4_L1_sev,  
                            mod5_L1_sev) %>% 
...
# 7 linear models of duration vs L1====    
mod1_L1_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(duration ~ L1 + threshold * sci_n, data = .) %>%  
  glance() %>%  
  mutate(model = "interaction") 

mod2_L1_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(duration ~ L1 + threshold + sci_n, data = .) %>%  
  glance() %>%  
  mutate(model = "all")   

mod3_L1_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(duration ~ L1 + threshold, data = .) %>%  
  glance() %>%  
  mutate(model = "no sci_n")  

mod4_L1_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(duration ~ L1 + sci_n, data = .) %>%  
  glance() %>%  
  mutate(model = "no threshold")  

mod5_L1_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(duration ~ L1, data = .) %>%  
  glance() %>%  
  mutate(model = "only L1") 

# 8 sum and test model fits====  
# * AIC & BIC slightly different -- chose all to fit with above   
mod_L1_dur_sum <- bind_rows(mod1_L1_dur,  
                            mod2_L1_dur,  
                            mod3_L1_dur,  
                            mod4_L1_dur,  
                            mod5_L1_dur) %>% 
...
mod3_L1_sev_sum <- mod3_L1_sev %>%  
  glance() %>%  
  mutate(model = "no threshold") 

mod4_L1_sev_sum <- mod1_L1_sev %>%  
  glance() %>%  
  mutate(model = "no sci_n")  



mod1_L1_sev_tab <- mod1_L1_sev %>%  
  tidy() %>%  
  mutate(explan_var = "severity")  

mod1_L1_sev_pts <- mod1_L1_sev %>%  
  augment() #%>%  



sri_l1_sev_mod1_pts %>%  
ggplot(aes(x = L1, y = severity)) +  
  facet_grid(vars(sci_n),  
             vars(threshold)) +  
 # facet_g(~ iv, scales = "free_x") +
  geom_smooth(method = "lm", se = FALSE, color = "lightgrey") +
  geom_segment(aes(xend = L1, yend = .fitted), alpha = .2) +  # Note `.fitted`
  geom_point(aes(alpha = abs(.resid))) +  # Note `.resid`
  guides(alpha = FALSE) +
  geom_point(aes(y = .fitted), shape = 1) +  # Note `.fitted`
  theme_bw()

sri_L1_sev2 <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(severity ~ L1 + threshold , data = .) %>%  
  tidy() %>%  
  mutate(explan_var = "severity")  
...
sri_L1_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
  lm(duration ~ L1, data = .) %>%  
  tidy() %>%  
  mutate(explan_var = "duration")  

sri_L1_mod <-  bind_rows(sri_L1_sev, sri_L1_dur)

# 6 plot severity vs average streamflow L1====  
sri_drought_sev <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
...
# 6 plot duration vs average streamflow L1====  
sri_drought_dur <- sri_events %>%  
  filter(group != "Pierre Shale Low") %>%  
...
# patchwork the plots & save====  
sri_drought_sev / sri_drought_dur  
...
sri_sum_tab <- sri_summary %>%  
  select(group,  
         ...
         median.s,  
         max.s) %>%  
  group_by(group, threshold, sci_n) %>%  
...
# 2. prepare export table -- set the numeric columns====   
col_key_num <- sri_sum_tab %>%   
  names()  

# 3. convert tibble to a flextable====     
coef_table <- sri_sum_tab %>%   
  # create flextable object  
  flextable() %>%   
  ...
  set_header_labels(group = "",  
                    threshold = "",  
                    sci_n = "SPI accumulation period (months)",  
                    proportion   = "Average number of events per station",  
                    mean.d_all   = "Mean",  
                    median.d_all = "Median",  
                    max.d_all    = "Max.",     
                    mean.s_all   = "Mean",  
                    median.s_all = "Median",  
                    max.s_all    = "Max.") %>%  
  add_header(group = "Hydrologic group",  
             threshold = "Threshold", 
             sci_n        = "",  
             proportion        = "", 
             mean.d_all   = "",  
...
coef_table <- read_docx() %>%  
  body_add_flextable(value = coef_table)  
print(coef_table, target = "output/SRI_summary_table.docx")   
