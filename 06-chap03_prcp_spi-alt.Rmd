...
library("ggfortify")     # used for 'autoplot' to check regression assumptions  
library("tidyverse")  
...
gage_mon_ssi <- gage_mon %>%  
  filter(between(Date,  
                 as.Date("1988-10-15"),  
                 as.Date("2018-09-15")))  

# * create some data checks====  
gage_check <- gage_mon_ssi %>%  
  group_by(sta) %>%  
  summarise(min = min(Date),  
            max = max(Date) )  

gage_full <- gage_mon_ssi %>%  
  select(sta) %>%  
  distinct()  
...
gage_mon_ssi <- full_join(gage_mon_ssi, flow_regime,   
                          by = "sta")  
...
```{r compare_precip-locations_old, eval=FALSE}  
...
nw <- sta_pres %>%  
  ...
  mutate(prcp = prcp + 1) %>%  # prevent dups  
  spread(sta, prcp) %>%   
...
nw2 <- sta_pres %>%  
  ...
  mutate(prcp = prcp + 1) %>%  # prevent dups  
  spread(location, prcp) %>%   
  gather(other, prcp, -c(date, NC)) %>%  
  mutate(location = "NC") %>%  
  rename(control = NC)   
...
se2 <- sta_pres %>%  
  ...
  mutate(prcp = prcp + 1) %>%  
  spread(location, prcp) %>%   
  gather(other, prcp, -c(date, SC)) %>%  
  mutate(location = "SC")  %>%  
  rename(control = SC)  

# join splits & create factor    
prcp_compare <- bind_rows(nw2, se2) %>%  
  mutate(diff = control - prcp) %>%    
  mutate(other = fct_relevel(other,  
                             "NW",  
                             "SW",  
...
```{r 4a_export_precip-diff_results}
...
# 2 convert tibble to a flextable====    
...
# 3 export a docx of flextables & clean up global environment====     
...
ssi_input <- semi_join(gage_mon_ssi, mclust_aug,  
                       by = "sta") %>%  
  rename(date = Date) %>%   
  arrange(date) %>%  
  select(sta,  
...
```{r 17a_export sci data }

export(sci_sta, "data/sci_sta.csv")  

```

```{r 18a_calculate_SSI-PE3_part1}
...
year_count <- bind_rows(high_year, low_year) %>%  
  group_by(group, year) %>%  
  summarise(count = sum(count))  %>%  
  ungroup() %>%  
  mutate(count = as.numeric(count)) %>%  
  mutate(count = case_when(  
    group == "Pierre Shale Low" ~ count/5,  
    group == "Pierre Shale High" ~ count/3,  
    group == "Tertiary Clays" ~ count/3,  
    group == "Black Hills Plateau" ~ count/2,  
    group == "Keya Paha Tablelands" ~ count/5,  
    group == "Sandhills" ~ count/6,  
    group == "GW Control" ~ count/2,  
    TRUE ~ count))  
...
month_count <- bind_rows(high_month, low_month) %>%  
  group_by(group, month) %>%  
  summarise(count = sum(count))  %>%  
  ungroup() %>%  
  mutate(count = as.numeric(count)) %>%  
...
```{r 21_plot_exceedances}

# 1. prepare for plotting -- yearly exceedances====    
year_count %<>%  
...
# 2. prepare for plotting -- monthly exceedances====  
month_count %<>%  
...
# 4. plot exceedances by month==== 
exceed_mon_plot <- month_count %>%  
  drop_na() %>%      
  ggplot(aes(x = month, y = count)) +  #, fill = key
  # set theme, facets, labels  #  
  theme_bw() +  
  facet_wrap(vars(group)) +  
#  ggtitle("High and low streamflow months by ecoregion ") +  
...
# 5. plot using patchwork and save====  
...
# 2  identify frequency (seasonality) -- should be 12 months - precip data====  
freq_sta <- sci_sta  %>%  
...
# 3  identify frequency (seasonality) -- should be 12 months - gage data====    
freq_gage <- sci_gage  %>%  
...
# 4  identify trend of periodicity -- should be 60 months -- precip data====  
trend_sta <- sci_sta  %>%  
  pivot_wider(names_from = key, values_from = value) %>%  
  split(.$sta) %>%   
...
# 5  identify trend of periodicity -- should be 60 months -- gage data====  
trend_gage <- sci_gage  %>%   
  pivot_wider(names_from = type, values_from = value) %>%  
  split(.$sta) %>%   
  map_dfc(~ anomalize::time_trend(   
...
# 6  decompose SCIs into seasonal, trend, & remainder====     
...
# 16 clean up global environment====   
rm(freq_gage,  
   freq_sta,  
   trend_gage,  
   trend_sta,  
...
```{r 25_append_decomp_results} 

# 1 append decomp results -- spi ====     
...
# 2 append decomp results -- ssi ====     
ssi_decomp_long <- bind_rows(ssi_seas_1mo,  
                        ssi_seas_2mo,  
                        ssi_seas_3mo,  
                        ssi_seas_4mo,  
                        ssi_seas_6mo,  
...
export(ssi_decomp_long, "data/ssi_seas.csv")  

```

```{r 26_plot_spi-deconvolution} 

# 1 check for anomonies====        
spi_anom <- spi_decomp_long %>%   
  filter(anomaly == "Yes")  

# 2 add vars for plotting====    
sta_size   <- 0.2   
sta_color  <- "black"  

# 3 spi_1mo observations plot====  
...
```{r 27_prepare_ssi_plot}

# 1 check for anomolies====  
ssi_anom <- ssi_decomp_long %>%  
  filter(anomaly == "Yes")  

# 2 relevel groups for plotting====  
ssi_decomp_1mo <- ssi_decomp_long %>%  
  filter(name == "sci_1mo")  
...
ssi_1mo <- full_join(ssi_decomp_1mo, ssi_summary,  
                        by = "group")  
...
```{r 28_plot-sri-deconvolution}

# plot SSI obs, seasons, trend, and remainder  
# 1  streamflow observations plot====  
ssi_obs <- ssi_1mo %>%   
  ggplot(aes(date, value)) +  
...
# 2  streamflow seasons plot====    
ssi_seas_plot <- ssi_1mo %>%   
  mutate(month = month(date)) %>%  
  ggplot(aes(month, season)) +  
# set theme, labels, facets #  
  theme_bw() +  
...
# 3  streamflow trend plot====  
ssi_trend_plot <- ssi_1mo %>%   
  ggplot(aes(date, trend)) +  
# set theme, labels, facets #  
  theme_bw() +  
  xlab("") +  
...
ssi_obs / ssi_seas_plot / ssi_trend_plot / ssi_remain  

ggsave("figure/ssi_deconv.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
# SPI event summary statistics 
```{r 29_prepare-SPI1-summary-statistics-moderate}
...
#    note: this is the set of all drought months after a drought is initiated  
...
# 7 create a unique name for sci-n and threshold====  
spi01_mod <- sci_obs %>%  
  mutate(sci_n = "1") %>%  
  mutate(threshold = "moderate")  
...
```{r 30_prepare-SPI1-summary-statistics-severe}
...
# 3. create a df of start ids====  
sci_start <- sci_drought %>%  
  # keep only drought start points of a threshold  
  filter(between(sci_value, SCI_hi, SCI_lo)) %>%   
  group_by(sta) %>%  
  mutate(id = seq_len(n())) %>%  
  unite("id_long", sta, id) %>%  
...
```{r 31_prepare-SPI1-summary-statistics-extreme}
...
```{r 32_prepare-SPI6-summary-statistics-moderate}
...
```{r 33_prepare-SPI6-summary-statistics-severe}
...
```{r 34_prepare-SPI6-summary-statistics-extreme}
...
```{r 35_prepare-SPI18-summary-statistics-moderate}
...
```{r 36_prepare-SPI18-summary-statistics-severe}
...
```{r 37_prepare-SPI18-summary-statistics-extreme}
...
```{r 38_join-SPI-summaries}

# 1.  join SPI  observations -- drop duplicates====    
...
# 2.  get drought counts====  
spi01_count <- spi01_obs %>%  
  group_by(sta, threshold) %>%  
...
# 3.  get duration of individual drought events====  
spi01_dur <- spi01_obs %>%  
  group_by(sta, threshold, drought_id, sci_n) %>%  
...
# 4.  get duration summaries====  
spi01_duration <- spi01_obs %>%  
  group_by(sta, threshold, drought_id) %>%  
  summarise(duration = n()) %>%  
  group_by(sta, threshold) %>%  
  summarise(min = min(duration),  
            max = max(duration),  
            perc_25 = quantile(duration,  
                               probs=0.25,  
                               na.rm=TRUE),  
            perc_75 = quantile(duration,  
                               probs=0.75,  
                               na.rm=TRUE),  
            mean = mean(duration),  
...
# 5.  get severity of individual drought events====  
spi01_sevy <- spi01_obs %>%  
  group_by(sta, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

spi06_sevy <- spi06_obs %>%  
  group_by(sta, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

spi18_sevy <- spi18_obs %>%  
  group_by(sta, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  


# 6.  get severity summaries====  
spi01_severity <- spi01_obs %>%  
  group_by(sta, threshold, drought_id) %>%  
  summarise(severity = sum(sci_value)) %>%  
  group_by(sta, threshold) %>%  
  summarise(max = min(severity),  
            min = max(severity),  
            perc_25 = quantile(severity, probs=0.25, na.rm=TRUE),  
            perc_75 = quantile(severity, probs=0.75, na.rm=TRUE),  
            mean = mean(severity),  
...
spi06_severity <- spi06_obs %>%  
  ...
  group_by(sta, threshold) %>%  
  summarise(max = min(severity),  
            perc_25 = quantile(severity,  
                               probs=0.25,  
                               na.rm=TRUE),  
            perc_75 = quantile(severity,  
                               probs=0.75,  
                               na.rm=TRUE),  
            min = max(severity),  
            mean = mean(severity),  
...
# 7.  join individual event durations====  
spi_dur <- bind_rows(spi01_dur,  
                     spi06_dur,  
                     spi18_dur) %>%  
  # prepare for plotting  
  mutate(scratch = "SPI") %>%  
  unite("sci_n", scratch, sci_n, sep = "-") %>%   
  mutate(threshold = factor(threshold)) %>%  
  mutate(threshold = fct_relevel(threshold,   
                            "moderate",   
                            "severe",   
                            "extreme")) %>%  
  mutate(sci_n = factor(sci_n)) %>%  
  mutate(sci_n = fct_relevel(sci_n,   
                            "SPI-1",   
                            "SPI-6",   
                            "SPI-18")) %>%  
  mutate(sta = factor(sta)) %>%  
  mutate(sta = fct_relevel(sta,   
                           "RAP",  
                           "OEL",  
                           "COT",   
                           "GOR",   
...
# 8.  join individual event severities====  
spi_sevy <- bind_rows(spi01_sevy,  
                     spi06_sevy,  
                     spi18_sevy) %>%  
  # prepare for plotting  
  mutate(scratch = "SPI") %>%  
...
# 9.  join summaries--by threshold ====  
spi01_summary <- full_join(spi01_count,  
                           spi01_duration,  
                           by = c("sta", "threshold")) %>%  
...
spi06_summary <- full_join(spi06_count,  
                           spi06_duration,  
                           by = c("sta", "threshold")) %>%  
...
spi18_summary <- full_join(spi18_count,  
                           spi18_duration,  
                           by = c("sta", "threshold")) %>%  
...
# 10. join summaries====  
spi_summary <- bind_rows(spi01_summary,  
                         spi06_summary,  
                         spi18_summary) %>%  
  mutate(scratch = "SPI") %>%  
  unite("sci_n", scratch, sci_n, sep = "-") %>%   
...
# 11. subset for plotting====  
spi_summary_dur <- spi_summary %>%   
  pivot_longer(min.d:median.d) %>%   
  select(-c(max.s:median.s)) %>%  
  group_by(name, threshold, sci_n) %>%  
  summarize(value = mean(value)) %>%   
  pivot_wider() %>%   
  ungroup() 

spi_summary_sev <- spi_summary %>%   
  pivot_longer(max.s:median.s) %>%   
  select(-c(min.d:median.d)) %>%  
  group_by(name, threshold, sci_n) %>%  
  summarize(value = mean(value)) %>%   
  pivot_wider() %>%   
  ungroup() 

# 10. clean up global environment====  
rm(spi01_ext,  
   spi01_sev,  
   spi01_mod,  
   spi06_ext,  
...
```{r 39_plot_SPI-summaries}

# 1 plot the count of droughts====     
spi_count <- spi_summary %>%  
  ggplot() +   
  theme_bw() +  
  xlab("") +  
  ylim(0, 50) +       # put axis limits same as SSI count  
  ylab("Number of events") +  
  # legend values -- # color is the station  -- legend is @ patchwork bottom 
  scale_color_grey() +  
  theme(legend.position="none") +  
  facet_wrap(vars(sci_n),   
             nrow = 1) +  
  geom_boxplot(aes(y = count, x = threshold),  
               outlier.shape = NA) +  
  geom_quasirandom(aes(y = count, x = threshold,  
                       color = sta),  
                   dodge.width = 0.5)   

spi_count  

# 2 plot the drought duration====  
spi_duration <- spi_summary_dur %>%  
  ggplot(aes(threshold)) +   
  theme_bw() +  
  xlab("") +      # supress label to show at the bottom of the patchwork  
  ylim(0, 50) +       # put axis limits same as SSI count  
  ylab("Drought duration (months)") +  
  facet_wrap(vars(sci_n),   
             nrow = 1)  + 
  scale_color_grey() +  
  # legend values -- # color is the station  -- legend is @ patchwork bottom  
...
# 3 plot the drought severity====  
spi_severity <- spi_summary_sev %>%  
  ggplot(aes(threshold)) +   
  theme_bw() +  
  xlab("") +  
    facet_wrap(vars(sci_n),   
             nrow = 1)  +  
  scale_y_reverse()  +                  # negative severity values  
  ylab("Drought severity") +  
  facet_wrap(vars(sci_n),   
             nrow = 1)  +  
  # legend values -- color is the station  
  theme(legend.position="bottom") +  
  guides(color=guide_legend(title="Station")) +  
  scale_color_grey() +  
geom_boxplot(aes(ymin = min.s,  
                 lower = perc_25.s,  
                 middle = median.s,  
                 upper = perc_75.s,  
                 ymax = max.s),  
             stat = "identity") +   
  geom_quasirandom(data = spi_sevy,  
                   aes(x = threshold,  
                       y = severity,  
                       group = sci_n,  
                       color = sta),  
...
# 4 plot above as a grid & save====  
spi_count / spi_duration / spi_severity  

ggsave("figure/spi_droughts.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
```{r 40_export_SPI-summary_table}

# 1.  arrange SPI-summary data for median station drought====   
spi_sum_tab <- spi_summary %>%  
  select(threshold,  
         sci_n,  
         count,  
         mean.d,  
         ...
         median.s,  
         max.s) %>%  
  group_by(threshold, sci_n) %>%  
  summarise(count = sum(count),  
            mean.d_all = median(mean.d),  
            median.d_all = median(median.d),  
            max.d_all = median(max.d),     
            mean.s_all = median(mean.s),  
            median.s_all = median(median.s),  
            max.s_all = median(max.s)) %>%  
  ungroup() %>%  
  mutate(threshold = case_when(  
    threshold == "moderate" ~ -1.0,  
    threshold == "severe"   ~ -1.5,  
    threshold == "extreme"  ~ -2.0)) %>%  
  mutate(sci_n = as.character(sci_n)) %>%  
  mutate(threshold = as.character(threshold)) %>%  
  mutate(sci_n = str_remove_all(sci_n, "SPI-")) %>%  
  mutate_if(is.numeric, round, 2)  

# 2.  prepare export table -- set the numeric columns====   
col_key_num <- spi_sum_tab %>%   
  names()  

# 3.  convert tibble to a flextable====     
coef_table <- spi_sum_tab %>%   
  # create flextable object  
  flextable() %>%   
  colformat_num(col_keys = col_key_num,   
                big.mark=",",   
  ...
  set_header_labels(threshold = "",  
                    sci_n = "SPI accumulation period (months)",  
                    ...
                    median.s_all = "Median",  
                    max.s_all    = "Max.") %>%  
  add_header(threshold = "Threshold", 
                    sci_n        = "",  
                    count        = "", 
                    mean.d_all   = "",  
                    median.d_all = "Duration (months)",  
                    max.d_all    = "",     
                    mean.s_all   = "",  
                    median.s_all = "Severity (-)",  
                    max.s_all    = "") %>%  
  autofit() %>%  
  theme_booktabs() %>%  
  align(align = "center")    # not working!!!  
...
coef_table <- read_docx() %>%  
  body_add_flextable(value = coef_table)  
print(coef_table, target = "output/SPI_summary_table.docx")   
...
# SSI summary statistics 
```{r 41_prepare_SSI-1_summary_statistics-moderate}
...
# 2. create the set of SSI drought====  
sci_drought <- ssi_decomp_long %>%  
 select(sci_value = value,  
         sta,  
         date,  
         group,  
...
# 4. join start ids and drought obs====   
sci_obs <- left_join(sci_drought, sci_start,  
                     by = c("date", "sci_value", "sci_length", "group")) %>%  
  # drop duplicates with different stations  
  separate(id_long, c("sta_ck1", "sta_ck2", "id"), remove = FALSE) %>%  
  unite("sta_ck", c(sta_ck1, sta_ck2)) %>%  
  filter(is.na(sta_ck) |  
           sta == sta_ck)  %>%  
  select(-sta_ck) %>%  
  mutate(id = as.numeric(id))  
...
```{r 42_prepare_SSI-1_summary_statistics-severe} 
...
```{r 43_prepare_SSI-1_summary_statistics-extreme}
...
```{r 44_prepare_SSI-6_summary_statistics-moderate}
...
```{r 45_prepare_SSI-6_summary_statistics-severe}
...
```{r 46_prepare_SSI-6_summary_statistics-extreme}
...
```{r 47_prepare_SSI-18_summary_statistics-moderate}
...
```{r 48_prepare_SSI-18_summary_statistics-severe}
...
```{r 49_prepare_SSI-18_summary_statistics-extreme}
...
```{r 50_join-SSI-summaries}

# 1.  join SSI  observations -- drop duplicates====     
ssi01_obs <- bind_rows(ssi01_ext,  
                  ssi01_sev,  
                  ssi01_mod) %>%  
  group_by(sta) %>%  
  distinct(date, .keep_all = TRUE) %>%  
...
# 2.  get drought counts====  
ssi01_count <- ssi01_obs %>%  
  group_by(group, threshold) %>%  
  summarise(count = n_distinct(drought_id)) %>%  
  ungroup()  

ssi06_count <- ssi06_obs %>%  
  group_by(group, threshold) %>%  
  summarise(count = n_distinct(drought_id)) %>%  
  ungroup()  

ssi18_count <- ssi18_obs %>%  
  group_by(group, threshold) %>%  
  summarise(count = n_distinct(drought_id)) %>%  
  ungroup()  
...
# 3.  get duration of individual drought events====  
ssi01_dur <- ssi01_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(duration = n()) %>%  
  ungroup()  
...
# 4.  get duration summaries====  
ssi01_duration <- ssi01_obs %>%  
  group_by(group, threshold, drought_id) %>%  
  summarise(duration = n()) %>%  
  group_by(group, threshold) %>%  
  summarise(min = min(duration),  
            max = max(duration),  
...
# 5.  get severity of individual drought events====  
ssi01_sevy <- ssi01_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

ssi06_sevy <- ssi06_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

ssi18_sevy <- ssi18_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

# 6.  get severity summaries====  
ssi01_severity <- ssi01_obs %>%  
  group_by(group, threshold, drought_id) %>%  
  summarise(severity = sum(sci_value)) %>%  
  group_by(group, threshold) %>%  
  summarise(max = min(severity),  
...
# 9.  join summaries--by threshold ====  
ssi01_summary <- full_join(ssi01_count,  
                           ssi01_duration,  
                           by = c("group", "threshold")) %>%  
  full_join(., ssi01_severity,  
            by = c("group", "threshold"),  
            suffix = c(".d", ".s")) %>%  
  mutate_if(is.numeric, round, 2) %>%  
  mutate(sci_n = "1")  

ssi06_summary <- full_join(ssi06_count,  
                           ssi06_duration,  
                           by = c("group", "threshold")) %>%  
  full_join(., ssi06_severity,  
            by = c("group", "threshold"),  
            suffix = c(".d", ".s")) %>%   
  mutate_if(is.numeric, round, 2) %>%  
  mutate(sci_n = "6")    

ssi18_summary <- full_join(ssi18_count,  
                           ssi18_duration,  
                           by = c("group", "threshold")) %>%  
  full_join(., ssi18_severity,  
            by = c("group", "threshold"),  
            suffix = c(".d", ".s")) %>%  
  mutate_if(is.numeric, round, 2) %>%  
  mutate(sci_n = "18")    
...
# 10. join summaries====  
ssi_summary <- bind_rows(ssi01_summary,  
                         ssi06_summary,  
                         ssi18_summary) %>%  
  mutate(scratch = "SSI") %>%  
  unite("sci_n", scratch, sci_n, sep = "-")  

# 11. join station count to summaries ====  
gage_count <- mclust_aug %>%  
  select(c(sta, hydro_group)) %>%  
  rename(group = hydro_group) %>%  
  group_by(group) %>%  
  summarise(gage_count = n()) %>%  
  ungroup() %>%   
  mutate(group = as.character(group)) %>%  
  mutate(group = case_when(  
    group == "GW control" ~ "GW Control",  
...
ssi_summary <- left_join(ssi_summary, gage_count,  
                         by = "group") %>%  
  mutate(proportion = round(count/gage_count, 0)) %>%  
  select(group:count,  
         gage_count,  
         proportion,  
         everything()) %>%  
...
# 12. subset for plotting====   
ssi_summary_dur <- ssi_summary %>%   
  pivot_longer(min.d:median.d) %>%   
  select(-c(max.s:median.s)) %>%  
  group_by(name, threshold, sci_n) %>%  
  summarize(value = mean(value)) %>%   
...
# 13. clean up global environment====    
rm(ssi01_ext,  
   ssi01_sevy,  
   ssi01_sev,  
   ssi01_mod,  
...
```{r 51_plot_SSI-summaries}

# 1 plot the count of droughts====    
ssi_count <- ssi_summary %>%  
  ggplot() +   
  theme_bw() +  
  xlab("") +  
  ylim(0, 50) +     # put axis limits same as SPI count  
  ylab("Events per station") +  
  facet_wrap(vars(sci_n),   
             nrow = 1) +  
  # prepare legend  
  scale_color_grey() +  
  theme(legend.position="none") +  
  # outlier.shape = NA ~ drops outlier that is plotted by geom_quasirandom  
  geom_boxplot(aes(y = proportion, x = threshold),  
               outlier.shape = NA) +  
  geom_quasirandom(aes(y = proportion, x = threshold,  
                       color = group),  
                   dodge.width = 0.5)  
...
ssi_duration <- ssi_summary_dur %>%  
  ...
  facet_wrap(vars(sci_n),   
             nrow = 1)  +  
  # legend values -- color is gage - legend is @ patchwork bottom  
  scale_color_grey() +  
  theme(legend.position="none") +  
geom_boxplot(aes(ymin = min.d,  
                 lower = perc_25.d,  
                 middle = median.d,  
                 upper = perc_75.d,  
                 ymax = max.d),  
             stat = "identity") +   
  geom_quasirandom(data = ssi_dur,  
                   aes(x = threshold,  
                       group = sci_n,  
                       y = duration,  
                       color = group),  
...
# 3 plot the drought severity====  
ssi_severity <- ssi_summary_sev %>%  
  ggplot(aes(threshold)) +   
  theme_bw() +  
  xlab("") +  
  scale_y_reverse(limits = c(0, -50)) +  # put axis limits same as SPI  
  ylab("Drought severity") +  
  facet_wrap(vars(sci_n),   
             nrow = 1)  +  
  # legend values -- color is gage  
  theme(legend.position = "bottom") +  
  scale_color_grey() +  
  guides(color=guide_legend(title="Hydrologic group")) +  
  geom_boxplot(aes(ymin = min.s,  
                   lower = perc_25.s,  
                   middle = median.s,  
                   upper = perc_75.s,  
                   ymax = max.s),  
               stat = "identity") +   
  geom_quasirandom(data = ssi_sevy,  
                   aes(x = threshold,  
                       group = sci_n,  
                       y = severity,  
                       color = group),  
...
# 4 plot above as a grid & save====   
ssi_count / ssi_duration / ssi_severity  

ggsave("figure/ssi_droughts.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
```{r 52_plot_SSI-duration_vs_severity} 

# 1 join ssi_events====   
ssi_events <- full_join(ssi_sevy, ssi_dur,  
                  by = c("group",  
                         "threshold",  
                         "drought_id",  
                         "sci_n"))  

# 2 this needs some work.. prepare to plot L1-magnitude vs duration====  
hydro_group <- hydro_group %>%  
  distinct() %>%  
  as.matrix() %>% as_tibble() %>%      # gets rid of hidden grouping var  
  mutate(hydro_group = case_when(  
...
ssi_events <- left_join(ssi_events, hydro_group,  
                          by = "group") %>%  
  mutate_at(vars(starts_with("L")), as.numeric) %>%  
    ...
    mutate(group = fct_relevel(group,   
                             ...
                             "GW Control",  
                             "Sand Hills")) %>%  
  mutate(severity = -1 * severity)  

# 3 plot severity vs duration====  
ssi_events %>%  
  ggplot(aes(duration, severity)) +  
  theme_bw() +  
  xlab("Drought event duration (months)") +  
#  scale_y_reverse() +  
  ylab("Drought event severity") +  
  scale_color_grey() +  
  guides(color = guide_legend(title = "Hydrologic group")) +  
  guides(size = guides(size = FALSE)) +  
  theme(legend.position = "bottom") +  
  facet_grid(vars(sci_n), vars(threshold)) +  
  geom_smooth(method = "lm",  
              color = "gray50",  
              linetype = 2)  +  
...
# 4 save severity vs duration plot====  
ggsave("figure/ssi_drought_comp.png",  
                 units = "in",  
                 width = 7,  
                 height = 4.5)  
...
# L1-regression  
```{r 53_prepare_SSI-L1-OLS_regression}

# 1.  prepare linear model inputs====  
ols_input <- ssi_events %>%  
  mutate(sci_num = as.character(sci_n)) %>%  
  mutate(sci_num = str_remove_all(sci_num, "SSI-")) %>%  
  mutate(sci_num = as.numeric(sci_num)) %>%  
  mutate(severity = severity + 0.001) %>%   # used to fix lm.fit error  
  mutate(duration = duration + 0.001)       # used to fix lm.fit error  

ols_psl_input <- ols_input %>%  
...
```{r 54_select_OLS_model-severity}

# Use forward selection to fit an ols model of severity vs stuff.

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Reminders on OLS model fitting assumptions  
...
ols2_mod <- ols_input %>%  
  lm(log10(severity) ~ L1, data = .) 

ols3_mod <- ols_input %>%  
  lm(log10(severity) ~ L1 + threshold, data = .) 

ols4_mod <- ols_input %>%  
  lm(log10(severity) ~ L1 + threshold + sci_num, data = .) 

ols5_mod <- ols_input %>%  
  lm(log10(severity) ~ L1 + threshold * sci_num, data = .) 

# 2. check residuals 
autoplot(ols1_mod)  # fails normality of residuals assumption 
autoplot(ols2_mod)  # better but long negative tail on qqplot 
autoplot(ols3_mod)  # better but somewhat long negative tail on qqplot 
autoplot(ols4_mod)  # not bad  
autoplot(ols5_mod)  # not bad  

# 3. check terms for not bad residuals 
ols4_tidy <- ols4_mod %>% 
  tidy()  
...
# 4. check model fits for not bad residuals 
ols4_sev <- ols4_mod %>%  
  glance() %>%  
  mutate(model = "all-terms")  

ols5_sev <- ols5_mod %>%  
  glance() %>%  
  mutate(model = "interaction")  

ols_summary_sev <- bind_rows(ols4_sev,  
                             ols5_sev) %>%  
  mutate(resp_var = "severity")  

# Results -- fits are very close -- choosing the less confusing model  
...
```{r 55_select_OLS_model-duration}

# Use forward selection to fit an ols model of duration vs stuff. 
...
# 4. check terms for not bad residuals 
ols4_tidy <- ols4_mod %>% 
  tidy()  
...
# 5. check model fits for not bad residuals 
ols4_dur <- ols4_mod %>%  
  glance() %>%  
  mutate(model = "all-terms")  
...
ols_summary_dur <- bind_rows(ols4_dur,  
                             ols5_dur) %>%  
  mutate(resp_var = "severity")  

# fits are very close -- choosing the less confusing model -- no interactions 
...
```{r 56_fit_OLS_models}
# 6.  prepare ols models -- sev & dur====  
ols_sev <- ols_input %>%   
  lm(log10(severity) ~ L1 + threshold + sci_num, data = .)  

ols_dur <- ols_input %>%  
  lm(log10(duration) ~ L1 + threshold + sci_num, data = .)  

ols_psl_sev <- ols_psl_input  %>%   
  lm(log10(severity) ~ L1 + threshold + sci_num, data = .)  

ols_psl_dur <- ols_psl_input  %>%  
  lm(log10(duration) ~ L1 + threshold + sci_num, data = .)  

ols_sev_fit <- glance(ols_sev)
```

```{r 57_get_OLS_table_values}

# get ols model table values====  
ols_results_tab_dur <- ols_dur %>%  
  tidy() %>%  
  mutate(explain_var = "Duration") %>%  
  mutate(flow_type = "Perennial")  

ols_results_tab_sev <- ols_sev %>%  
  tidy() %>%  
  mutate(explain_var = "Severity") %>%  
  mutate(flow_type = "Perennial")    

ols_tab_psl_dur <- ols_psl_dur %>%  
  tidy() %>%  
  mutate(explain_var = "Duration") %>%  
  mutate(flow_type = "Intermittant")  

ols_tab_psl_sev <- ols_psl_sev %>%  
  tidy() %>%  
  mutate(explain_var = "Severity") %>%  
  mutate(flow_type = "Intermittant")  

# 8.  prepare ols table====  
ols_results_tab <- bind_rows(ols_results_tab_dur,  
                       ols_results_tab_sev,  
                       ols_tab_psl_dur,  
                       ols_tab_psl_sev) %>% 
  mutate_if(is.numeric, round, 2) %>%  
  select(explain_var, everything()) %>%  
  arrange(flow_type, explain_var, term) %>%  
  mutate(term = case_when(  
    term == "(Intercept)" ~ "Intercept",  
    term == "sci_numSRI-18" ~ "SRI-18",  
    term == "sci_numSRI-6" ~ "SRI-6",  
    term == "thresholdextreme" ~ "Extreme drought",  
    term == "thresholdsevere" ~ "Severe drought",  
    TRUE ~ term)) %>%  
...
```{r 58_get_points_for_plotting_OLS}

# get model points for plotting====  
ols_pts_sev <- ols_sev %>%  
  augment(data = ols_input) %>%  
  mutate(explain_var = "Severity") %>%   
  mutate(fitted_tr = 10^.fitted) %>%  
  mutate(se.fit_tr = 10^.se.fit)  
...
ols_psl_pts_sev <- ols_psl_sev %>%  
  augment(data = ols_psl_input) %>%  
  mutate(explain_var = "Severity") %>%   
  mutate(fitted_tr = 10^.fitted) %>%  
  mutate(se.fit_tr = 10^.se.fit)    

ols_psl_pts_dur <- ols_psl_dur %>%  
  augment(data = ols_psl_input) %>%  
  mutate(explain_var = "Duration") %>%   
  mutate(fitted_tr = 10^.fitted) %>%  
  mutate(se.fit_tr = 10^.se.fit)    

ols_all_pts_sev <- bind_rows(ols_pts_sev, ols_psl_pts_sev)  
ols_all_pts_dur <- bind_rows(ols_pts_dur, ols_psl_pts_dur)  
...
```{r 59_plot_SRI-L1-OLS_regression}

# 1.  plot ols -- duration====  
ols_plot_dur <- ols_all_pts_dur %>%  
  # set up the plotting environment  
  ggplot(aes(L1, duration)) +  
  theme_bw() +  
  xlab("Average streamflow magnitude (L1)") +  
  ylab("Event duration (months)") +  
  facet_grid(vars(sci_n), 
             vars(threshold)) +  
  scale_color_grey() +  
  # prepare legend   
#  guides(color = guide_legend(title = "Hydrologic group")) +  
#  guides(size = guides(size = FALSE)) +  
 ...
 # plot fitted line and error-bars for OLS -- most  
  geom_line(data = ols_pts_dur, 
            aes(x = L1, y = fitted_tr),  
            color = "gray40",  
              linetype = 1) +  
  geom_errorbar(data = ols_pts_dur,  
                aes(ymin = fitted_tr - se.fit_tr,  
                    ymax = fitted_tr + se.fit_tr),  
                width = .2,  
                color = "gray40") +    
# plot PSL fitted line and error-bars for OLS -- psl  
  geom_line(data = ols_psl_pts_dur,  
            aes(x = L1, y = fitted_tr),  
            color = "gray40",  
              linetype = 2) +  
  geom_errorbar(data = ols_psl_pts_dur,  
                aes(ymin = fitted_tr - se.fit_tr,  
                    ymax = fitted_tr + se.fit_tr),  
                width = .3,  
                color = "gray40") #+  
...
# 1.  plot ols -- severity====   
ols_plot_sev <- ols_all_pts_sev %>%  
  ggplot(aes(L1, severity)) +  
  theme_bw() +  
  xlab("Average streamflow magnitude (L1)") +  
  ylab("Event severity (-)") +  
  facet_grid(vars(sci_n), 
  ...
  geom_jitter(aes(color = group)) +  
 # plot fitted line and error-bars for OLS -- most  
  geom_line(data = ols_pts_sev, 
            aes(x = L1, y = fitted_tr),  
            color = "gray40",  
              linetype = 1) +  
  geom_errorbar(data = ols_pts_sev,  
                aes(ymin = fitted_tr - se.fit_tr,  
                    ymax = fitted_tr + se.fit_tr),  
                width = .2,  
                color = "gray40") +  
# plot PSL fitted line and error-bars for OLS -- psl  
  geom_point(data = ols_psl_pts_sev,  
              aes(color = group)) +  
#  geom_line(data = ols_psl_pts_sev,  
#            aes(x = L1, y = fitted_tr),  
#            color = "gray40",  
#              linetype = 2) +  
  geom_errorbar(data = ols_psl_pts_sev,  
                aes(ymin = fitted_tr - se.fit_tr,  
                    ymax = fitted_tr + se.fit_tr),  
                width = .3,  
                color = "gray40")  #+ 
...
# 3.  patchwork the plots & save====  
ols_plot_dur / ols_plot_sev  

ggsave("figure/ssi_drought_L1_comp-alt.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
ols3_sev <- ols_input %>%  
  mutate(severity = severity + 0.001) %>%  
  lm(log(log10(severity)) ~ L1 + threshold + sci_num, data = .) %>%  
  glance() %>%  
  mutate(model = "ln-all")   
...
ols4_sev <- ols_input %>%   
  lm(log10(severity) ~ L1 + threshold, data = .) %>%  
  glance() %>%  
  mutate(model = "no sci_n")  

ols5_sev <- ols_input %>% 
  lm(log10(severity) ~ L1 + sci_num, data = .) %>%  
  glance() %>%  
  mutate(model = "no threshold")  

ols6_sev <- ols_input %>% 
  lm(log10(severity) ~ L1, data = .) %>%  
  glance() %>%  
  mutate(model = "only L1") 
...
# 3.  prepare to select ols model -- dur====    
ols1_dur <- ols_input %>%   
  lm(log10(duration) ~ L1 + threshold * sci_num, data = .) %>%  
  glance() %>%  
  mutate(model = "interaction") 
...
ols3_dur <- ols_input %>%   
  lm(log(log10(duration)) ~ L1 + threshold + sci_num, data = .) %>%  
  glance() %>%  
  mutate(model = "ln-all")   

ols4_dur <- ols_input %>%   
  lm(log10(duration) ~ L1 + threshold, data = .) %>%  
  glance() %>%  
  mutate(model = "no sci_num")  

ols5_dur <- ols_input %>%   
  lm(log10(duration) ~ L1 + sci_num, data = .) %>%  
  glance() %>%  
  mutate(model = "no threshold")  

ols6_dur <- ols_input %>%   
  lm(log10(duration) ~ L1, data = .) %>%  
  glance() %>%  
  mutate(model = "only L1") 

# 4.  examine ols models -- sev & dur====  
# * AIC != BIC -- chose all vars -- interaction had non-significant terms  
ols_summary_sev <- bind_rows(ols1_sev,  
                             ols2_sev,  
                             ols3_sev,  
                             ols4_sev,  
                             ols5_sev,  
...
ols_summary_dur <- bind_rows(ols1_dur,  
                             ols2_dur,  
                             ols3_dur,  
                             ols4_dur,  
                             ols5_dur,  
                             ols6_dur) %>%  
  mutate(resp_var = "duration")  

ols_model_summary <- bind_rows(ols_summary_sev,  
                               ols_summary_dur) %>%  
  mutate_if(is.numeric, round, 2)  

# 5.  filter the selected ols models & clean up global environment====  
ols_model_summary %<>%  filter(model == "all")  
...
# 1. prepare ols points table====  
ols_pts_table <- bind_rows(ols_pts_sev, ols_pts_dur) %>%  
  mutate(explain_var = as.factor(explain_var)) %>%  
  mutate(explain_var = fct_relevel(explain_var,  
                                   "Severity",  
                                   "Duration")) %>%  
  select(-c(severity, duration, drought_id,  
         L1:.std.resid)) %>%  
  mutate_if(is.numeric, round, 2) %>%  
  distinct() %>%    
    rename_at(vars(ends_with("_tr")),  
            list(~str_replace(., "_tr", ""))) %>%  
  pivot_longer(cols = c(fitted, se.fit), 
               names_to = "scratch", values_to = "value") %>%  
  pivot_wider(names_from = c(group, scratch),  
              values_from = value)  %>%    
  select(explain_var,  
         sci_n,  
         threshold,  
         `Pierre Shale Low_fitted`,  
         `Pierre Shale Low_se.fit`,  
         `Tertiary Clays_fitted`,  
         `Tertiary Clays_se.fit`,  
         `Pierre Shale High_fitted`,  
         `Pierre Shale High_se.fit`,  
         `Black Hills Plateau_fitted`,  
         `Black Hills Plateau_se.fit`,  
         `Keya Paha Tablelands_fitted`,  
         `Keya Paha Tablelands_se.fit`,  
         `GW Control_fitted`,  
         `GW Control_se.fit`,  
         `Sand Hills_fitted`,  
         `Sand Hills_se.fit`)  
...
# 2. prepare export table -- set the numeric columns====   
col_key_num <- ols_pts_table %>%   
  names()  

# 3.  convert tibble to a flextable====     
coef_table <- ols_pts_table %>%   
  arrange(explain_var, sci_n, threshold) %>%  
  ...
  colformat_num(col_keys = col_key_num,   
                ...
                digits = 1, na_str = "") %>%  
  # rename header labels  
  set_header_labels(explain_var = "",  
                    sci_n = "SPI accumulation period (months)",   
                    threshold = "Threshold",   
                    `Pierre Shale Low_fitted` = "Fitted",  
                    `Pierre Shale Low_se.fit`= "SE +/-",  
                    `Tertiary Clays_fitted` =  "Fitted",   
                    `Tertiary Clays_se.fit`= "SE +/-",  
                    `Pierre Shale High_fitted` =  "Fitted",   
                    `Pierre Shale High_se.fit` = "SE +/-",  
                    `Black Hills Plateau_fitted` = "Fitted",    
                    `Black Hills Plateau_se.fit` = "SE +/-",  
                    `Keya Paha Tablelands_fitted` = "Fitted",    
                    `Keya Paha Tablelands_se.fit` = "SE +/-",  
                    `GW Control_fitted` =  "Fitted",   
                    `GW Control_se.fit` = "SE +/-",  
                    `Sand Hills_fitted` = "Fitted",    
                    `Sand Hills_se.fit` = "SE +/-") %>%  
  add_header(explain_var = "",  
                    sci_n = "",   
                    threshold = "",   
                    `Pierre Shale Low_fitted` = "Pierre Shale Low",  
                    `Pierre Shale Low_se.fit`= "",  
                    `Tertiary Clays_fitted` =  "Tertiary Clays",   
                    `Tertiary Clays_se.fit`= "",  
                    `Pierre Shale High_fitted` =  "Pierre Shale High",   
                    `Pierre Shale High_se.fit` = "",  
                    `Black Hills Plateau_fitted` = "Black Hills Plateau",    
                    `Black Hills Plateau_se.fit` = "",  
                    `Keya Paha Tablelands_fitted` = "Keya Paha Tablelands",    
                    `Keya Paha Tablelands_se.fit` = "",  
                    `GW Control_fitted` =  "GW Control",   
                    `GW Control_se.fit` = "",  
                    `Sand Hills_fitted` = "Sand Hills",    
                    `Sand Hills_se.fit` = "") %>%  
  autofit()  %>%  
  theme_booktabs() 
#%>%  
#  merge_v(target = explain_var, part = "body") %>%  
#  align(align = "center", part = "all") %>%   
#  fontsize(ft, size = 6, part = "all")  

# 4.  export a docx of flextables====     
coef_table <- read_docx() %>%  
  body_add_flextable(value = coef_table)  
print(coef_table, target = "output/ols_pts_table.docx")   
...
# 3.  add location to sri dataframe====  
mclust_aug <- mclust_aug %>%     
    rename(gage = sta)  
...
# * add location to mclust and SRI dataframes   
mclust_aug <- full_join(mclust_aug, gage_location,  
                         by = "gage") #%>%  
...
# 6.  prepare for SPI & SRI correlations====   
...
# 10.  calculate correlation coefficents for SPI-SRI by gage====  
...
```{r plot-SCI-correlation-all}  

# 1.  prepare for plotting====  
sci_corr_sta <- sci_corr_sta %>%   
  mutate(scratch = "SPI") %>%  
  unite(spi_fac, "scratch", "spi",  
        remove = FALSE,  
        sep = "-") %>%  
  select(-scratch) %>%  
  mutate(spi_fac = as.factor(spi_fac)) %>%  
  mutate(spi_fac = fct_relevel(spi_fac,   
                             "SPI-1",  
                             "SPI-2",   
                             "SPI-3",   
                             "SPI-4",   
...
# 2.  plot r vs lags -- facet by spi====      
r_vs_lag_plot <- sci_corr_sta %>%   
  ggplot(aes(x = lag, y = r, group = group)) +  
  xlab("Months of lag") +  
  ylab("Correlation") +  
  # set theme, labels, facets #   
  theme_bw() +  
  facet_wrap(vars(spi_fac),   
             nrow = 3) +   
  #  guides(color = guide_legend(title = "Hydrologic group")) +  
  guides(color = guides(color = FALSE)) +  
  guides(linetype = guides(linetype = FALSE)) +  
  # plot data  
...
# 3.  plot spi  vs r -- facet by lags====      
r_vs_spi_plot <- sci_corr_sta %>%   
  ggplot(aes(x = spi, y = r, group = group)) +  
  xlab("SPI accumulation length (months)") +  
  ylab("Correlation") +  
  # set theme, labels, facets #   
  facet_wrap(vars(lag),   
             nrow = 3) +   
  ...
  theme_bw() +  
  scale_colour_grey()  +  
  #  scale_x_log10(breaks = c(1, 2, 3, 4, 6, 9, 12, 18, 24)) +  
  # plot data  
  geom_smooth(se = FALSE,  
              aes(#linetype = group,  
                color = group)) +  
...
# 4.  patchwork the plots & save====  
r_vs_lag_plot / r_vs_spi_plot  

ggsave("figure/lag_corr_plot.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
```{r plot-SCI-correlation_by-group}

# 1.  plot r vs lags -- facet by group====    
r_vs_lag_plot_01 <- sci_corr_sta %>%  
  filter(spi == 1) %>%  
  ggplot(aes(x = lag, y = r)) +  
  # set theme, labels, facets #   
  theme_bw() + 
  xlab("") +  
  ylab("SPI-1 correlation") +  
  ...
  scale_colour_grey() +   
  geom_point(aes(color = group)) + 
  geom_smooth(fill = "gray80", 
              aes(#linetype = group,  
                  color = group))  

r_vs_lag_plot_01

r_vs_lag_plot_06 <- sci_corr_sta %>% 
  filter(spi == 6) %>%  
  ggplot(aes(x = lag, y = r)) +  
  # set theme, labels, facets #   
  theme_bw() + 
  xlab("") +  
  ylab("SPI-6 correlation") +  
  scale_y_continuous(limits = c(-0.1, 0.7)) +  
  #  guides(color = guide_legend(title = "Hydrologic group")) +  
  guides(color = guides(color = FALSE)) +  
  guides(linetype = guides(linetype = FALSE)) +  
...
r_vs_lag_plot_18 <- sci_corr_sta %>%  
  filter(spi == 18) %>%  
  ggplot(aes(x = lag, y = r)) +  
  # set theme, labels, facets #   
  theme_bw() + 
  xlab("Months of lag") +  
  ylab("SPI-18 correlation") +  
  scale_y_continuous(limits = c(-0.1, 0.7)) +  
  guides(color = guide_legend(title = "Hydrologic group")) +  
#  guides(color = guides(color = FALSE)) +  
#  guides(linetype = guides(linetype = FALSE)) +  
...
# 4.  patchwork the plots & save====  
r_vs_lag_plot_01 / r_vs_lag_plot_06 / r_vs_lag_plot_18

ggsave("figure/lag_corr_gp_plot.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
# sci_corr -- long obs without grouping variable all correlations - not useful
# sci_corr_table -- spi vs lag  
# sci_corr_sta -- long obs with grouping variable  
# spi_corr_table -- correlations within spi  
# sri_corr_table -- lag vs lag table  

sci_corr_group <- sci_corr_sta %>%  
  group_by(group) %>%  
  summarise(r = round(mean(r), digits = 2)) %>%  
  ungroup() 

sci_corr_spi_lag <- sci_corr_sta %>%  
  group_by(spi, lag) %>%  
  summarise(r = round(mean(r), digits = 2)) %>%  
  ungroup()  

sci_corr_spi_group <- sci_corr_sta %>%  
  group_by(spi, group) %>%  
  summarise(r = round(mean(r), digits = 2)) %>%  
  ungroup()  

sci_corr_spi_group_01 <- sci_corr_spi_group %>%  
  filter(spi == 1)  

sci_corr_spi_group_06 <- sci_corr_spi_group %>%  
  filter(spi == 6)  

sci_corr_spi_group_12 <- sci_corr_spi_group %>%  
  filter(spi == 12)  

sci_corr_spi_group_18 <- sci_corr_spi_group %>%  
  filter(spi == 18)  
```
...
<!--  
```{r ccor_long}
# 7.  make sci longer for plotting as points====   
...
```{r dleteee-plot}
# plot PSL fitted line and error-bars
  geom_line(data = psl_L1_sev_pts, 
            aes(x = L1, y = .fitted),  
            color = "gray40",  
              linetype = 2) +  
  geom_errorbar(data = psl_L1_sev_pts, 
                aes(ymin = .fitted - .se.fit, 
                    ymax = .fitted + .se.fit), 
                width = .3, 
                color = "gray40")   
  # plot fitted line and error-bars
  geom_line(data = mod_L1_dur_pts, 
            aes(x = L1, y = .fitted),  
            color = "gray40",  
              linetype = 2) +  
  geom_errorbar(data = mod_L1_dur_pts, 
                aes(ymin = .fitted - .se.fit, 
                    ymax = .fitted + .se.fit), 
                width = .1, 
                color = "gray40") +
