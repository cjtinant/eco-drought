...
# 8.  prepare ols table====  
ols_table <- bind_rows(ols_table_dur,  
                       ols_table_sev,  
                       ols_tab_psl_dur,  
                       ols_tab_psl_sev) %>% 
  mutate_if(is.numeric, round, 2) %>%  
...
ols_pts_all_sev <- bind_rows(ols_pts_sev, ols_psl_pts_sev)  
ols_pts_all_dur <- bind_rows(ols_pts_dur, ols_psl_pts_dur)  
...
# 13. prepare ols results tables====  
ols_pts_table_sev <- ols_pts_all_sev %>%  
  select(group, threshold, sci_n, fitted_tr, se.fit_tr) %>%  
  mutate(fit_hi = fitted_tr + se.fit_tr) %>%  
  mutate(fit_lo = fitted_tr - se.fit_tr) %>%  
  select(-c(fitted_tr, se.fit_tr)) %>%  
  mutate_if(is.numeric, round, 2) %>%  
  distinct() %>%  
  pivot_wider(names_from = c(group),  
              values_from = c(fit_hi, fit_lo),  
              names_repair = "unique") %>%  
  mutate(hi_diff = `fit_hi_Tertiary Clays` - `fit_hi_Sand Hills`) %>%  
  mutate(lo_diff = `fit_lo_Tertiary Clays` - `fit_lo_Sand Hills`) %>%  
  select(threshold, sci_n, hi_diff, lo_diff, everything())  

ols_pts_table_dur <- ols_pts_all_dur %>%  
  select(group, threshold, sci_n, fitted_tr, se.fit_tr) %>% 
  mutate(fit_hi = fitted_tr + se.fit_tr) %>%  
  mutate(fit_lo = fitted_tr - se.fit_tr) %>%  
  select(-c(fitted_tr, se.fit_tr)) %>%  
...
# 14. clean up global environment====   
rm(ols_plot_sev,  
   ols_plot_dur,  
   ols_psl_pts_sev,  
   ols_psl_pts_dur, 
...
```  
# 11 clean up global environment====  
rm(mod_L1_dur_tab,  
   mod_L1_sev_tab,  
   mod_L1_dur,  
   mod_L1_sev,  
