...
# plot SRI obs, seasons, trend, and remainder  
...
# SRI summary statistics 
```{r prepare-SRI1-summary-statistics-moderate}
...
# 2. create the set of SRI drought====  
sci_drought <- sri_decomp_long %>%  
 select(sci_value = value,  
         sta,  
         date,  
         group,  
...
```{r prepare-SRI1-summary-statistics-severe} 
...
```{r prepare-SRI1-summary-statistics-extreme}
...
```{r prepare-SRI6-summary-statistics-moderate}
...
```{r prepare-SRI6-summary-statistics-severe}
...
```{r prepare-SRI6-summary-statistics-extreme}
...
```{r prepare-SRI18-summary-statistics-moderate}
...
```{r prepare-SRI18-summary-statistics-severe}
...
```{r prepare-SRI18-summary-statistics-extreme}
...
```{r join-SRI-summaries}

# 1.  join SRI  observations -- drop duplicates====     
sri01_obs <- bind_rows(sri01_ext,  
                  sri01_sev,  
                  sri01_mod) %>%  
  group_by(sta) %>%  
  distinct(date, .keep_all = TRUE) %>%  
...
# 2.  get drought counts====  
sri01_count <- sri01_obs %>%  
  group_by(group, threshold) %>%  
  summarise(count = n_distinct(drought_id)) %>%  
  ungroup()  

sri06_count <- sri06_obs %>%  
  group_by(group, threshold) %>%  
  summarise(count = n_distinct(drought_id)) %>%  
  ungroup()  

sri18_count <- sri18_obs %>%  
  group_by(group, threshold) %>%  
  summarise(count = n_distinct(drought_id)) %>%  
  ungroup()  
...
# 5.  get severity of individual drought events====  
sri01_sevy <- sri01_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

sri06_sevy <- sri06_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

sri18_sevy <- sri18_obs %>%  
  group_by(group, threshold, drought_id, sci_n) %>%  
  summarise(severity = sum(sci_value)) %>%  
  ungroup()  

# 6.  get severity summaries====  
sri01_severity <- sri01_obs %>%  
  group_by(group, threshold, drought_id) %>%  
  summarise(severity = sum(sci_value)) %>%  
  group_by(group, threshold) %>%  
  summarise(max = min(severity),  
...
# 9.  join summaries--by threshold ====  
sri01_summary <- full_join(sri01_count,  
                           sri01_duration,  
                           by = c("group", "threshold")) %>%  
  full_join(., sri01_severity,  
            by = c("group", "threshold"),  
            suffix = c(".d", ".s")) %>%  
  mutate_if(is.numeric, round, 2) %>%  
  mutate(sci_n = "1")  

sri06_summary <- full_join(sri06_count,  
                           sri06_duration,  
                           by = c("group", "threshold")) %>%  
  full_join(., sri06_severity,  
            by = c("group", "threshold"),  
            suffix = c(".d", ".s")) %>%   
  mutate_if(is.numeric, round, 2) %>%  
  mutate(sci_n = "6")    

sri18_summary <- full_join(sri18_count,  
                           sri18_duration,  
                           by = c("group", "threshold")) %>%  
  full_join(., sri18_severity,  
            by = c("group", "threshold"),  
            suffix = c(".d", ".s")) %>%  
  mutate_if(is.numeric, round, 2) %>%  
  mutate(sci_n = "18")    
...
# 10. join summaries====  
sri_summary <- bind_rows(sri01_summary,  
                         sri06_summary,  
                         sri18_summary) %>%  
  mutate(scratch = "SRI") %>%  
  unite("sci_n", scratch, sci_n, sep = "-")  
...
sri_summary <- left_join(sri_summary, gage_count,  
                         by = "group") %>%  
...
```{r plot_SRI-summaries}

# 1 plot the count of droughts====    
sri_count <- sri_summary %>%  
  ggplot() +   
  theme_bw() +  
  xlab("") +  
...
sri_severity <- sri_summary_sev %>%  
  ...
  geom_boxplot(aes(ymin = min.s,  
                   ...
                   ymax = max.s),  
               stat = "identity") +   
  geom_quasirandom(data = sri_sevy,  
                   aes(x = threshold,  
                       group = sci_n,  
                       y = severity,  
                       color = group),  
...
# 4 plot above as a grid & save====   
sri_count / sri_duration / sri_severity  

ggsave("figure/sri_droughts.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
```{r plot_SRI-duration_vs_severity} 

# 1 join sri_events====   
sri_events <- full_join(sri_sevy, sri_dur,  
                  by = c("group",  
                         "threshold",  
                         "drought_id",  
                         "sci_n"))  
...
hydro_group <- hydro_group %>%  
  distinct() %>%  
  as.matrix() %>% as.tibble() %>%      # gets rid of hidden grouping var  
  mutate(hydro_group = case_when(  
...
sri_events <- left_join(sri_events, hydro_group,  
                          by = "group") %>%  
...
ggsave("figure/sri_drought_comp.png",  
                 units = "in",  
                 width = 7,  
                 height = 4.5)  
...
```{r SRI_L1_OLS-regression}

# 1.  prepare linear model inputs====  
ols_input <- sri_events %>%  
  mutate(sci_num = as.character(sci_n)) %>%  
  mutate(sci_num = str_remove_all(sci_num, "SRI-")) %>%  
...
# 2.  prepare to select ols model -- sev====    
ols1_sev <- ols_input %>%  
...
# 9.  get model points for plotting====  
ols_pts_sev <- ols_sev %>%  
...
ols_pts_sev <- bind_rows(ols_pts_sev, ols_psl_pts_sev)  
ols_pts_dur <- bind_rows(ols_pts_dur, ols_psl_pts_dur)  
...
# 10. plot ols -- severity====  
ols_plot_sev <- ols_pts_sev %>%  
...
# 11. plot ols -- duration====  
ols_plot_dur <- ols_pts_dur %>%  
...
# 12. patchwork the plots & save====  
ols_plot_sev / ols_plot_dur  

ggsave("figure/sri_drought_L1_comp.png",  
                 units = "in",  
                 width = 7,  
                 height = 9)  
...
mclust_aug <- mclust_aug %>%     # import("data/lmom_mclust_fit.csv") %>%  
    rename(gage = sta)  
