...
<!--
Purpose:
This R markdown file prepares streamflow data for combined analysis in
07-chap04_eco_combined
-->  
...
sci_gage <- bind_rows(sci_kpt,
    ...
    gather(key = type, val = value,
         -c(sta, date, waterYear, group, q1_depth, log_q1))


# 3.1 check results====
# check for possible missing values or other errors
sci_gage_ck <- sci_gage %>%
  filter(date == "2008-03-15") %>%
  filter(type == "sci_6mo")

# from gages_in & sci_gage_cl--
#1     blc_wan     blc_wan
#2     blp_bel     blp_bel
#3     bat_bhr     bat_bhr
#4     che_was     che_was
...
# 5.0 prepare wshd data
```{r save_data_scores}

export(data_scores, "data/wsd_data_scores.csv")
export(gage_meta, "data/gage_scores.csv")

```

```{r 5a_fix_lat-lon_errors}

# fix lat lons of ungaged watersheds
# from Google Maps and TetraTech
# BEA3 is upstream of Hisle
#   N 43.403726, W -101.743046
# LOD1 is NE of Potato Creek
#   N 43.601012, W -101.96805

wsd_summary <- import("data/wsd_summary.csv") %>%
  mutate(dec_lat = case_when(
    sta == "BEA3" ~ 43.40373,
    sta == "LOD1" ~ 43.601012,
    TRUE ~ dec_lat)) %>%
  mutate(dec_lon = case_when(
    sta == "BEA3" ~ -101.74305,
    sta == "LOD1" ~ -101.96805,
    TRUE ~ dec_lon))
...
wsd_summary %>%
  filter(type ==  "ungaged") %>%
  ggplot(aes(dec_lon, dec_lat)) +
  geom_point() +
  geom_text_repel(aes(label = sta))

# update names
wsd_summary %<>%
  mutate(gage_nm = na_if(gage_nm, "")) %>%
  mutate(sta = case_when(
    sta == "WHR5" ~ "WHI5",
    sta == "WHR4" ~ "WHI4",
    sta == "WHR3" ~ "WHI3",
...
```{r 5a_prepare_watshd-data_for_invert_analysis}
...
# export results
export(drought_in, "data/sci_ungaged_june.csv")
export(sci_gage, "data/sci_gaged.csv")
