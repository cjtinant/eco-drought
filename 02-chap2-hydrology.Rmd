...
gage_dv <- read_csv("data/gage_dv.csv") %>% 
  clean_names() %>% 
  select(-log_q) %>%    # going to censor below so will be incorrect 
  distinct()            # somehow there are 4 dups for 'che_pla' 
...
# Stream flow is analyzed for 1980 - 2017 

# This code chunk identifies missing gage-year combinations    
# An iterative approach is used to fill missing years with the 
#streamflow depths of the nearest neighbor in the group.   

gage_run_01 <- read_csv("data/gage_mod_input.csv") %>% 
  select(-Qualifier) %>% 
  clean_names()
attr(gage_dv_run_01, 'spec') <- NULL     # removes attribute data 
