...
gage_wet_meta <- import("data/gage_meta_9297.csv") 
gage_wet_raw <- import("data/gage_9297.csv") 

gage_wet_sum <- gage_wet_raw %>% 
  group_by(sta) %>% 
...
# clean up Global Environment----
gage_sum <- full_join(gage_wet_meta, gage_sum)

rm(lambda_q1, lambda_q7, lambda_q30, gage_depth, gage_wet_meta, 
   pca_eigen, pca_meta, pca_vars)
...
gage_dry_meta <- import("data/gage_meta_03post.csv") 
gage_dry_raw <- import("data/gage_03post.csv") 

# Eliminate the effects of different sizes of watersheds & 
# remove missing values created by rolling averages

gage_dry_depth <- gage_dry_raw %>% 
  as.tibble() %>% 
  clean_names() %>% 
  mutate(contrib_drain_area_km = contrib_drain_area_va * 2.59) %>% 
  mutate(q1_depth = q * (60*60*24) / contrib_drain_area_km) %>% 
...
# find lambda values for BoxCox transformation  
lambda_q1 <- BoxCox.lambda(gage_dry_depth$q1_depth)   #  q1 ==> 0.014  
lambda_q7 <- BoxCox.lambda(gage_dry_depth$q7_depth)   #  q7 ==> 0.109  
lambda_q30 <- BoxCox.lambda(gage_dry_depth$q30_depth) # q30 ==> 0.202 
...
# Apply transformation and scale data
gage_dry_depth <- gage_dry_depth %>% 
  mutate(q1_tr = BoxCox(gage_dry_depth$q1_depth,lambda_q1)) %>%
  mutate(q7_tr = BoxCox(gage_dry_depth$q7_depth,lambda_q7)) %>% 
  mutate(q30_tr = BoxCox(gage_dry_depth$q30_depth,lambda_q30)) %>%
  mutate(q1_mean = mean(q1_tr, na.rm = TRUE)) %>% 
  mutate(q1_sd = sd(q1_tr, na.rm = TRUE)) %>% 
  mutate(q1_depth = (q1_tr - q1_mean)/q1_sd) %>% 
  mutate(q7_mean = mean(q7_tr, na.rm = TRUE)) %>% 
...
pca_dry_input <- gage_dry_depth %>%  
  dplyr::select(q1_depth, q7_depth, q30_depth) 

pca_dry_meta <- gage_dry_depth %>% 
  select(sta, date, q1_depth, q7_depth, q30_depth) 

# Calculate PCA matrix & summary info
pca_dry_matrix <- prcomp(pca_dry_input, scale = TRUE) 
...
# Eigenvectors--results about PC axes
pca_dry_eigen <-  tidy(pca_dry_matrix, matrix = "pcs") 

# PCA variables--the loadings on the PCA axes.  
pca_dry_vars <-  tidy(pca_dry_matrix, matrix = "variables") 

# Drop the 3rd PC-axis because it's not useful 
pca_dry_vars <- pca_dry_vars %>% 
  filter(PC != 3) %>% 
  rename(var = column) %>% 
  mutate(var = as.factor(var)) 

# Bind sample vals to PCA matrix 
gage_dry_aug <- augment(pca_dry_matrix, data = pca_dry_input) 
gage_dry_aug <- bind_cols(pca_dry_meta, gage_dry_aug) 

gage_dry_aug <- gage_dry_aug %>% 
  select(-.fittedPC3, -.rownames, 
         -q1_depth1, -q7_depth1, -q30_depth1) %>% 
  mutate(q1_q30_diff = q1_depth - q30_depth) 

# summarize pca of gages
gage_dry_sum <- gage_dry_aug %>% 
  group_by(sta) %>% 
  summarize(PC1_mean = mean(.fittedPC1),
            PC2_mean = mean(.fittedPC2), 
            q1_mean = mean(q1_depth), 
...
# clean up Global Environment----
gage_dry_sum <- full_join(gage_dry_meta, gage_dry_sum)
...
# plot PCA----
# PC1 explanatory plot
ggplot(gage_dry_aug, aes(q7_depth, .fittedPC1, color = factor(sta))) + 
  geom_jitter() + 
  theme_classic() +
  xlab("Q7 depth") +
  ylab("Fitted PC1") + 
...
# PC2 explanatory plot
ggplot(gage_dry_aug, aes(q1_q30_diff, .fittedPC2, color = factor(sta))) +
  geom_jitter() + 
  theme_classic() +
  xlab("Q1 depth minus Q30 depth") +
  ylab("Fitted PC2") +
...
# mean eigenvector plot
ggplot(gage_dry_sum, aes(PC1_mean, PC2_mean)) +
  geom_jitter() + 
  geom_text(aes(label = sta), check_overlap = TRUE, 
            nudge_y = 0.02) + 
  theme_classic() + 
...
# transformed variable plot
ggplot(gage_dry_sum, aes(q7_mean, q1_q30_mean)) +
  geom_jitter() + 
  geom_text(aes(label = sta), check_overlap = TRUE, 
              nudge_y = 0.02) + 
  scale_x_continuous(limits = c(-2, 1)) + 
...
# The plot suggests potentially 5 clusters in the mixture.  The highest 
# "parts of the mountain" are in the very high hydrologic export & 
# average  flashiness & high hydrologic export & lower than average 
# flashiness.  A third cluster is lower than average hydrologic export 
# coefficient and lower than average flashiness.  A fourth and fifth 
# cluster, and sixth cluster show decreasing hydrologic export and 
...
#       153.7647 28 41 170.9091 170.7395
...
gage_clust <- full_join(gage_sum, gage_clust, by = "sta")
gage_clust <- gage_clust %>% 
...
ggplot(gage_clust, aes(q7_mean, q1_q30_mean)) + 
...
# highest Q1-Q30, moderately low hydrologic export = flashy
...
# high Q1-Q30, average hydrologic export = flashy
...
# averarage Q1-Q30, highes hydrologic export = GW
...
# similiar export to V1, but much lower (e.g higher? flashy)
