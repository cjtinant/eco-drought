...
# import estimates from prior run 
...
gage <- bind_rows(gage_cont, gage_disc, 
                  gage_other, gage_est_9092)  %>% # orig  n = 286,954
  group_by(sta) %>%                               # run02 n = 290,254
  mutate(min_yr = min(water_year)) %>% 
  mutate(max_yr = max(water_year)) %>% 
           ungroup() %>% 
  distinct() %>% 
  filter(!is.na(q30))
rm(gage_cont, gage_disc, gage_other, gage_est_9092)      
...
#gage_test_over <- semi_join(gage, gage_test_sum) 
rm(gage_test)

# find actual count of years - post1990  
gage_summary <- gage %>%                               # N = 44 sta
  group_by(sta) %>%                              # orig  n = 40 sta 
  summarise(min_yr = min(water_year),            # run02 n = 33 sta 
...
gage_incomp <- semi_join(gage, gage_incomp, 
                         by = c("sta", "water_year")) %>% 
  filter(!is.na(q30))                         # orig n  = 2742 obs 
                                              # run02 n = 2582 obs

gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>% # orig  n = 254,945 
  filter(days_record >= 365)                      # run01  n = 256,771    
                                                  # run02  n = 260,423
...
rm(gage_meta, gage_run01, gage_run02, gage_run03)
...
#  03   1996   2015 158,883    0.0191     0.0100     0.2624
...
#   03  1    0.578      0.588     0.567    
#   03  2    0.570      0.207    -0.795    
