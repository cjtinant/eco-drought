...
gage_summary <- gage %>%                              # N = 44 sta
  ...
  filter(count_years_act > 5)                           # n = 39 sta
 
#  summarise(count_yrs_act = n()) %>% 

# remove stations with < 5 yr records - clean up 
gage <- semi_join(gage, gage_summary, by = "sta")     # n = 275,281 

# drop stations with issues  
gage <- gage %>%                                           
  filter(sta != "bad_mid") %>%          # incomplete days
  filter(sta != "che_ang") %>%          # incomplete days
#  filter(sta != "nio_abb") %>% 
#  filter(sta != "sna_dou") %>%          # n = 258,855  
#  filter(sta != "cas_hot") %>%          # run01 - dropped for >> gw
#  filter(sta != "sna_bur") %>%          # run01 - dropped for >> gw
  filter(sta != "che_was")              # run01 - dropped for >> area

# update gage summary & separate out incomplete years of record
gage_summary <- gage %>%                              # n = 716 obs
  group_by(sta, water_year) %>%            
  summarise(days_record = n()) %>%          
  ungroup()                         

incomp_years_sum <- gage %>%                          # n = 15 obs 
  group_by(sta, water_year) %>% 
  summarise(days_record = n()) %>% 
  ungroup() %>% 
  filter(days_record < 365)                           ## n =  18 obs

# remove stations-years with incomplete days of record 
incomp_years <- semi_join(gage, incomp_years_sum)

gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>% 
  filter(days_record >= 365)                          # n = 256,040 

# filter metadata to only active stations & add area to gage data 
gage_meta <- semi_join(gage_meta, gage_summary, by = "sta")

gage <- full_join(gage, gage_meta, by = c("sta", "site_no"))
gage <- gage %>% 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# check on: rm(gage_sum_run_02) 
# runs:
#pca_input <- gage_run01
#pca_input <- gage_run02  
...
# overview 
# want to create missing years as the following:
# date_vals <- from(gage): "date", "julian", "month", "day", 
#          "dec_year", "month_seq", "water_year", "qualifier", "i" 

# from(gage_flow) = "q", "q7", "q30", "log_q", water_year, date, sta
# from()
# from(metadata): "site_no"    
# joined by: "sta"     

# prepare to estimate missing years data----
# 1. create a dataframe with missing years for run02 stations
gage_summary_run02 <- gage_summary %>% 
  spread(water_year, days_record) %>% 
...
# 2. update gage_run02 with clustering data & split off date values
gage_run02 <- inner_join(gage, gage_clust, by = "sta")
...
# 3. create a dataframe of streamflow by group
gage_flow_run02 <- gage_run02 %>% 
  select(group, date, q, q7, q30, log_q, water_year) %>% 
  group_by(group, date) %>% 
...
# Update 1990 missing stations---- 

# 1. identify stations with missing data for 1990
gage_miss_1990 <- as.data.frame(gage_summary_run02$'1990') %>% 
  rename(sta = 1) %>% 
  filter(!is.na(.)) 
...
# 2. subset streamflows for run02 stations for water-year 1990 
gage_flow_1990 <- gage_flow_run02 %>% 
...
# Update 1991 missing stations---- 

# 1. identify stations with missing data for 1991
gage_miss_1991 <- as.data.frame(gage_summary_run02$'1991') %>% 
  rename(sta = 1) %>% 
...
gage_flow_1991 <- gage_flow_run02 %>% 
...
gage_flow_9091 <- bind_rows(gage_flow_1990, gage_flow_1991) 

gage_flow_9091 <- inner_join(date_vals, gage_flow_9091) 


# 4.   find any incomplete days 
gage_miss_names <- gage_flow_9091 %>% 
  distinct(sta) 

partial_days <- semi_join(incomp_days, gage_miss_names)



# clean up 
rm(date_vals, gage_flow_1990, gage_flow_1991, gage_flow_run02, 
   gage_run01, gage_run02, gage_summary, gage_summary_run02)

export(gage_flow_9091, file = "data/gage_miss9091.csv")
