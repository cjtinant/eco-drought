...
# import and append daily flow values > 1990 
gage_orig <- import(file = "data/gage_inputs.csv")   # n = 249,247 obs
...
gage <- bind_rows(gage_orig, gage_est_9002, gage_est_0317, 
  ...
  mutate(min_yr = min(water_year)) %>%         # run01 n = 264,415 
  mutate(max_yr = max(water_year)) %>%         # run02 n = 268,452      
           ungroup()                           # run04 n = 286,631
...
gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>%  
  filter(days_record >= 365)                   # run00 n = 245,814 obs    
                                               # run01 n = 262,979 obs   
                                               # run02 n = 267,362 obs  
                                               # run03 n = 285,989 obs  
...
gage_run02_sum <- gage_summary %>% 
  ...
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                 # n =     728 obs

gage_run02 <- semi_join(gage, gage_run02_sum, 
                        by = c("sta", "water_year")) # n = 132,592 obs
...
#  select(c(sta:'2005')) %>%                    # subsets the matrix
  filter(!is.na(`2005`)) %>%                            # 25 sta 
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                 # n =     738 obs

gage_run03 <- semi_join(gage, gage_run03_sum, 
                        by = c("sta", "water_year")) # n = 233,396 obs
...
# [1990, 2002] filtered based on 2004 complete records 

gage_run04_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(c(sta:'2002'))  %>%             # subsets the matrix
#  filter(!is.na(`2004`)) %>%                            # 32 sta 
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                 # n =     416 obs

gage_run04 <- semi_join(gage, gage_run04_sum, 
                        by = c("sta", "water_year")) # n = 148,284 obs
...
#  02   2003   2017 132,592   -0.014      0.071      0.250
#  03   1990   2017 233,396    0.015      0.021     -0.004
#  04   1990   2002 148,284    0.049      0.030      0.042

pca_eigen 
# num   PC1perc PC2perc PC1cum PC2cum 
#  01    0.944   0.049  0.944   0.993     
#  02    0.944   0.050  0.944   0.994   
#  03    0.963   0.031  0.963   0.994   
#  04    0.968   0.027  0.968   0.995   

pca_vars 
# run  PC  q1_depth  q7_depth  q30_depth       
#   01  1    0.577      0.589     0.566 
#   01  2    0.588      0.182    -0.788  
#   02  1    0.578      0.589     0.565       
#   02  2    0.573      0.201    -0.795      
#   03  1    0.578      0.584     0.570           
#   03  2    0.543      0.247    -0.803       
#   04  1    0.578      0.583     0.571           
#   04  2    0.549      0.241    -0.801     
...
#  02   EEV     91.22     25  23    108.4    108.4   estimate NA 
#  03   VVV    112.38     25  19    163.6    163.5   estimate NA   
#  04   EVV    190.67     32  27    287.0    285.1   estimate NA  
...
#  02 cluster val:     1        2        3
#  02 num members:    11       11        3       
#  02 cluster type: mixed      gw   ephemeral  
#  03 cluster val:   1        2          
#  03 num members:   
#  03 cluster type: mixed     gw 
#  04 cluster val:   1        2          
#  04 num members:   
...
gage_part_miss <- gage_run02_sum %>% 
  filter(days_record < 365) %>% 
    spread(water_year, days_record)                 # n = 3 x 8  obs 
...
gage_pairs <- gage_clust %>% 
  ...
  rename(group_sta = group) %>% 
  rename(sta_orig = sta) %>% 
  select(group_sta, sta_orig, sta_lead, sta_lag) # n = 25 = gage_clust
...
rm(gage_est_miss, gage_run02_incomp, gage_run02_map, gage_run02_sum)
...
gage_part_miss <- gage_run03_sum %>% 
  filter(days_record < 365) %>% 
    spread(water_year, days_record)                 # n = 5 x 14  obs 

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 61 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  rename(sta_orig = sta)
...
gage_pairs <- full_join(gage_pairs, group_lead_id,  
                         by = "sta_lead") %>%              
  select(group_sta, group_lead, sta_orig, sta_lead, sta_lag) # 256 obs
...
# 7. join the missing water-year pairs 
gage_part_miss <- inner_join(gage_part_miss, gage_pairs,
                        by = c("sta_orig", "group"))  # n = 122 pairs 
