...
```{r import-gages}
gage_8097_meta <- import("data/gage_meta_8097.csv") 
gage_8097_raw <- import("data/gage_8097.csv") 

gage_8097_sum <- gage_8097_raw %>% 
  group_by(sta) %>% 
  summarise(start_yr = min(waterYear), 
            end_yr = max(waterYear)) %>% 
  arrange(desc(start_yr)) %>% 
  arrange(end_yr)

gage_8097_sum 
# A tibble: 28 x 3 (n = 306,052)
#   sta     start_yr end_yr
#   <chr>      <dbl>  <dbl>
# 1 bat_bhr     1989   2017
# 2 bat_her     1980   2017
...
# 8 brsf_co     1989   2017
# 9 che_was     1980   2017
#10 fal_hot     1980   2017
#11 frn_fai     1982   2017
#12 hat_edg     1980   2017
#13 hor_oel     1983   2017
#14 lcr_bel     1980   2017
#15 lcr_vet     1980   2017
#16 lwr_abv     1982   2004
#17 lwr_mar     1980   2017
#18 lwr_ros     1980   2017
#19 lwr_whi     1980   2017
20 rap_far     1980   2017
21 ros_ros     1980   2004
22 spr_her     1980   2004
23 wcc_ogl     1980   1999
24 whi_kad     1980   2017
25 whi_ogl     1980   2017
26 whi_slm     1991   1997
27 whi_sta     1988   2017
28 wkc_wok     1992   1997
```
...
# This dataset is inclusive of data from 1980 - 2017 for 28 stations 

# filter to 1992-1997 data only
gage_9297 <- gage_9297_raw %>% 
...
# find lambda values for BoxCox transformation  
lambda_q1 <- BoxCox.lambda(gage_depth$q1_depth)   #  q1 ==> 0.005 
lambda_q7 <- BoxCox.lambda(gage_depth$q7_depth)   #  q7 ==> 0.102  
lambda_q30 <- BoxCox.lambda(gage_depth$q30_depth) # q30 ==> 0.195 
...
#    PC std.dev percent cumulative
#  <dbl>   <dbl>   <dbl>      <dbl>
#1     1   1.68  0.938        0.938
#2     2   0.394 0.0517       0.990
#3     3   0.173 0.00993      1    
...
rm(pca_eigen, pca_matrix, pca, vars, pca_input) 
...
# clean up Global Environment----
gage_9297_sum <- full_join(gage_9297_meta, gage_sum)

gage_9297 <- gage_9297 %>% 
  mutate(date = Date)
gage_9297_aug <- left_join(gage_aug, gage_9297, by = c("sta", "date")) 

rm(lambda_q1, lambda_q7, lambda_q30, gage_depth, gage_9297_meta, 
   pca_eigen, pca_meta, pca_vars, gage_aug, gage_9297, gage_sum)

# plot PCA----
# PC1 explanatory plot
ggplot(gage_9297_aug, aes(q7_depth, .fittedPC1, color = factor(sta))) + 
  geom_jitter() + 
  theme_classic() +
  xlab("Q7 depth") +
  ylab("Fitted PC1") + 
...
ggplot(gage_9297_aug, aes(q1_q30_diff, .fittedPC2, color = factor(sta))) +
  geom_jitter() + 
  theme_classic() +
...
ggplot(gage_9297_sum, aes(PC1_mean, PC2_mean)) +
  geom_jitter() + 
...
ggplot(gage_9297_sum, aes(q7_mean, q1_q30_mean)) +
  geom_jitter() + 
  geom_text(aes(label = sta), check_overlap = TRUE, 
              nudge_y = 0.02) + 
...
```{r EDA-PCA-0317-gages}
# Examines data from 2003-2017

gage_0317_meta <- import("data/gage_meta_03post.csv") 
gage_0317_raw <- import("data/gage_03post.csv") 

gage_0317_sum <- gage_0317_raw %>% 
  group_by(sta) %>%  
  summarise(start_yr = min(waterYear), 
            end_yr = max(waterYear)) 
...
# 8 che_red     2003   2017
# 9 che_was     2003   2017
#10 fal_hot     2003   2017

# This dataset is inclusive of data from 2003 - 2017 for 24 stations 

# ~~~~~~~~~~~~~~~
# Eliminate the effects of different sizes of watersheds & 
# remove missing values created by rolling averages

gage_0317_depth <- gage_0317_raw %>% 
  as.tibble() %>% 
  clean_names() %>% 
  mutate(contrib_drain_area_km = contrib_drain_area_va * 2.59) %>% 
  mutate(q1_depth = q * (60*60*24) / contrib_drain_area_km) %>% 
...
# find lambda values for BoxCox transformation  
lambda_q1 <- BoxCox.lambda(gage_0317_depth$q1_depth)   # q1 ==> -0.013  
lambda_q7 <- BoxCox.lambda(gage_0317_depth$q7_depth)   # q7 ==>  0.070  
lambda_q30 <- BoxCox.lambda(gage_0317_depth$q30_depth) # q30 ==> 0.214 
...
# Apply transformation and scale data
gage_0317_depth <- gage_0317_depth %>% 
  mutate(q1_tr = BoxCox(gage_0317_depth$q1_depth,lambda_q1)) %>%
  mutate(q7_tr = BoxCox(gage_0317_depth$q7_depth,lambda_q7)) %>% 
  mutate(q30_tr = BoxCox(gage_0317_depth$q30_depth,lambda_q30)) %>%
  mutate(q1_mean = mean(q1_tr, na.rm = TRUE)) %>% 
  mutate(q1_sd = sd(q1_tr, na.rm = TRUE)) %>% 
  mutate(q1_depth = (q1_tr - q1_mean)/q1_sd) %>% 
  mutate(q7_mean = mean(q7_tr, na.rm = TRUE)) %>% 
...
pca_input <- gage_0317_depth %>%  
  dplyr::select(q1_depth, q7_depth, q30_depth) 

pca_meta <- gage_0317_depth %>% 
  select(sta, date, q1_depth, q7_depth, q30_depth) 
...
# Bind sample vals to PCA matrix 
gage_0317_aug <- augment(pca_matrix, data = pca_input) 
gage_0317_aug <- bind_cols(pca_meta, gage_0317_aug) 

gage_0317_aug <- gage_0317_aug %>% 
  select(-.fittedPC3, -.rownames, 
         -q1_depth1, -q7_depth1, -q30_depth1) %>% 
  mutate(q1_q30_diff = q1_depth - q30_depth) 

# summarize pca of gages
gage_0317_sum <- gage_0317_aug %>% 
  group_by(sta) %>% 
  summarize(PC1_mean = mean(.fittedPC1),
            PC2_mean = mean(.fittedPC2), 
            q1_mean = mean(q1_depth), 
...
#     PC std.dev percent cumulative
#  <dbl>   <dbl>   <dbl>      <dbl>
#1     1   1.68  0.946        0.946
#2     2   0.377 0.0473       0.993
#3     3   0.142 0.00676      1  
...
# clean up Global Environment----
gage_0317_sum <- full_join(gage_0317_meta, gage_0317_sum)

rm(lambda_q1, lambda_q7, lambda_q30, gage_0317_depth, gage_0317_meta, 
   pca_eigen, pca_meta, pca_vars, pca_input, pca_matrix)


# plot PCA----
# PC1 explanatory plot
ggplot(gage_0317_aug, aes(q7_depth, .fittedPC1, color = factor(sta))) + 
  geom_jitter() + 
  theme_classic() +
  xlab("Q7 depth") +
  ylab("Fitted PC1") + 
...
# PC2 explanatory plot
ggplot(gage_0317_aug, aes(q1_q30_diff, .fittedPC2, color = factor(sta))) +
  geom_jitter() + 
  theme_classic() +
  xlab("Q1 depth minus Q30 depth") +
  ylab("Fitted PC2") +
...
# mean eigenvector plot
ggplot(gage_0317_sum, aes(PC1_mean, PC2_mean)) +
  geom_jitter() + 
  geom_text(aes(label = sta), check_overlap = TRUE, 
            nudge_y = 0.02) + 
  theme_classic() + 
...
gage_0317_sum <- gage_0317_sum %>% 
  mutate(period = "2003-17") 

gage_9297_sum <- gage_9297_sum %>% 
  mutate(period = "1992-97") 

gage_sum <- bind_rows(gage_9297_sum, gage_0317_sum)

# transformed variable plot
#ggplot(gage_0317_sum, aes(q7_mean, q1_q30_mean)) +
#  geom_jitter() + 
#  geom_density2d() + 
#  geom_text(aes(label = sta), check_overlap = TRUE, 
#              nudge_y = 0.02) + 
...
mc_9297_input <- gage_9297_sum %>% 
  select(q1_mean:q30_mean)

mc_9297_input_meta <- gage_9297_sum %>% 
  select(sta)

gage_clust_l <- Mclust(mc_9297_input) # Model-based-clustering 
...
#       155.2134 28 41 173.8064 173.7889
...
# and orientation, and equal shape. The summary contains also the clustering 
...
gage_clust <- bind_cols(mc_9297_input_meta, gage_clust) 

gage_clust <- gage_clust %>% 
  gather(key = group, value = prob, -sta) %>% 
  filter(prob > 0.8) 

gage_9297_sum <- full_join(gage_9297_sum, gage_clust, 
                             by = "sta")
gage_9297_clust <- gage_9297_clust %>% 
  dplyr::select(sta, group, everything())

# Clean up Global Environment
rm(gage_clust, gage_clust_l, mc_9297_input, mc_9297_input_meta)
...
test <- left_join(gage_9297_sum, gage_9297_clust)




ggplot(gage_9297_clust, aes(q7_mean, q1_q30_mean)) + 
...
# Preparing to cluster by the central tendancy
mc_input <- gage_0317_sum %>% 
  select(q1_mean:q30_mean)

mc_input_meta <- gage_0317_sum %>% 
  select(sta)
...
# log.likelihood  n df      BIC      ICL
#        93.2016 24 18 129.1982 129.1863

#Clustering table:
# 1  2 
# 4 20 

gage_clust_l$modelName # Optimal selected model ==> "EVV" 
gage_clust_l$G # Optimal number of cluster => 2 
...
gage_clust <- full_join(gage_0317_sum, gage_clust, by = "sta")
gage_clust <- gage_clust %>% 
...
ggplot(gage_clust, aes(q7_mean, q1_q30_mean, color = group)) +
  geom_jitter() + 
  geom_density2d() + 
  geom_text(aes(label = sta), check_overlap = TRUE, 
              nudge_y = 0.02) + 
