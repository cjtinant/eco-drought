...
7. Calculated SPI for 'cot', 'oel', 'rap', 'int', and 'ora' datasets


## Issues: there is a duplicate variable - int for 'intermediate' and for 'Interior' 
## Next Steps: 
1. Run SPI using Pearson rather than gamma.
1. Think about combining data frames for the various datasets.
...
spi_1rap_coeff <- spi_1rap_coeff %>% 
  rename(rap1_alpha = alpha) %>%
   rename(rap1_beta = beta) 

spi_2rap_coeff <- spi_2rap_coeff %>% 
  rename(rap2_alpha = alpha) %>%
   rename(rap2_beta = beta) 

spi_3rap_coeff <- spi_3rap_coeff %>% 
  rename(rap3_alpha = alpha) %>%
   rename(rap3_beta = beta) 

spi_4rap_coeff <- spi_4rap_coeff %>% 
  rename(rap3_alpha = alpha) %>%
   rename(rap4_beta = beta) 

spi_5rap_coeff <- spi_5rap_coeff %>% 
  rename(rap5_alpha = alpha) %>%
   rename(rap5_beta = beta) 

spi_6rap_coeff <- spi_6rap_coeff %>% 
  rename(rap6_alpha = alpha) %>%
   rename(rap6_beta = beta) 

spi_9rap_coeff <- spi_9rap_coeff %>% 
  rename(rap9_alpha = alpha) %>%
   rename(rap9_beta = beta) 

spi_12rap_coeff <- spi_12rap_coeff %>% 
  rename(rap12_alpha = alpha) %>%
   rename(rap12_beta = beta) 

spi_18rap_coeff <- spi_18rap_coeff %>% 
  rename(rap18_alpha = alpha) %>%
   rename(rap18_beta = beta) 

spi_24rap_coeff <- spi_24rap_coeff %>% 
  rename(rap24_alpha = alpha) %>%
   rename(rap24_beta = beta) 
...
spi_1ora_coeff <- spi_1ora_coeff %>% 
  rename(ora1_alpha = alpha) %>%
   rename(ora1_beta = beta) 

spi_2ora_coeff <- spi_2ora_coeff %>% 
  rename(ora2_alpha = alpha) %>%
   rename(ora2_beta = beta) 

spi_3ora_coeff <- spi_3ora_coeff %>% 
  rename(ora3_alpha = alpha) %>%
   rename(ora3_beta = beta) 

spi_4ora_coeff <- spi_4ora_coeff %>% 
  rename(ora3_alpha = alpha) %>%
   rename(ora4_beta = beta) 

spi_5ora_coeff <- spi_5ora_coeff %>% 
  rename(ora5_alpha = alpha) %>%
   rename(ora5_beta = beta) 

spi_6ora_coeff <- spi_6ora_coeff %>% 
  rename(ora6_alpha = alpha) %>%
   rename(ora6_beta = beta) 

spi_9ora_coeff <- spi_9ora_coeff %>% 
  rename(ora9_alpha = alpha) %>%
   rename(ora9_beta = beta) 

spi_12ora_coeff <- spi_12ora_coeff %>% 
  rename(ora12_alpha = alpha) %>%
   rename(ora12_beta = beta) 

spi_18ora_coeff <- spi_18ora_coeff %>% 
  rename(ora18_alpha = alpha) %>%
   rename(ora18_beta = beta) 

spi_24ora_coeff <- spi_24ora_coeff %>% 
  rename(ora24_alpha = alpha) %>%
   rename(ora24_beta = beta) 
