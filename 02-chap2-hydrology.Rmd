...
# [2003, 2017] filtered based on 2003 complete records 

gage_run02_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% 
#  select(c(sta,'2003':'2017')) %>%
#  filter(!is.na(`2003`)) %>%                              # 33 sta 
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
...
gage_run02 <- semi_join(gage, gage_run02_sum, 
                        by = c("sta", "water_year")) # n = 273,206 obs
...
gage_input <- gage_run02                       # run02 = 273,206  obs

rm(gage_meta, gage_incomp, gage_run02, 
   gage_run02_na, gage_test) 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
```
...
#  02   1990   2017 273,206   -0.0749    -0.0115    -0.1643 

# run01    "bat_bhr" "bat_her" "bat_key" "bev_abf" "bev_buf" "bev_pri" 
...
# run02    "bat_bhr" "bat_her" "bat_key" "bev_abf" "bev_buf" "bev_pri" 
#          "blp_bel" "brsf_co" "che_buf" "che_red" "che_sce" "fal_hot" 
#          "frn_fai" "hat_edg" "hor_oel" "lcr_abv" "lcr_bel" "lcr_vet" 
#          "lwr_abv" "lwr_mar" "lwr_ros" "lwr_whi" "rap_far" "ros_ros" 
#          "spr_her" "wcc_ogl" "whi_int" "whi_kad" "whi_ogl" "whi_slm" 
#          "whi_sta" "whi_whi" "wkc_wok"

pca_eigen 
# num   PC1perc PC2perc PC1cum PC2cum 
#  01    0.945   0.047  0.945   0.993     
#  02    0.958   0.036  0.958   0.994   
...
#   02  1    0.578      0.585     0.569      
#   02  2    0.549      0.236    -0.802    
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Mclust EEV (ellipsoidal, equal volume and shape) or 
#       VVV (ellipsoidal, equal shape) model 
#       with 3 or 2 components (wet - dry):

# num  model  log.likeli  n   df     BIC      ICL
#  01   EEV    147.8495   22  51  138.0558 138.0555
#  02   VEV    157.695    33  19  248.9564 248.4741
...
# Run 1 clustering table:
#  cluster val:       1        2      3 
# cluster type:  mixed        gw  ephemeral
# num members:     9          16      3 

# Run 2 clustering table:
#  cluster val:       1        2       
# cluster type:  ephem-mixed  gw  
# num members:       15       18     
...
gage_dups <- semi_join(gage_est_9002, gage_test) %>% 
  filter(qualifier != "est") # 1,997 obs

gage_est_9002a <- anti_join(gage_est_9002, gage_dups, 
                            by = c("date", "sta")) 

gage_est_9002a <- gage_est_9002a %>% 
  select(-contrib_drain_area_va) 

export(gage_est_9002, file = "data/gage_est_9002a.csv")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
gage_part_miss <- gage_run02_sum %>% 
  filter(days_record < 365) %>% 
    spread(water_year, days_record)                 # n = 13 x 29 obs

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 176 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  rename(sta_orig = sta)
...
gage_pairs <- inner_join(group_lead_id, gage_pairs, 
                         by = "sta_lead") %>%              
  filter(group_sta == group_lead) %>% # drops obs of different groups
  select(group_sta, group_lead, sta_orig, sta_lead, sta_lag) # 32 obs

# 5. join the group__lag id to stations 
gage_pairs <- inner_join(gage_pairs, group_lag_id, 
                         by = "sta_lag") %>% 
  filter(group_sta == group_lag) %>% # drops obs of different groups 
  rename(group = group_sta) %>%                              # 31 obs
  select(-c(group_lead, group_lag)) 
...
# 7. join the missing water-year pairs 
gage_part_miss <- inner_join(gage_part_miss, gage_pairs,
                        by = c("sta_orig", "group"))  # n = 322 pairs 
...
gage_est_miss <- inner_join(gage, gage_part_miss, 
  ...
  select(date, sta_orig, q:qualifier) %>% 
  mutate(qualifier = "est") %>% 
  group_by(date, sta_orig) %>%                # n = 82,910 obs
  summarise(                                   
    q          = mean(q), 
    log_q      = mean(log_q), 
    q7         = mean(q7), 
    q30        = mean(q30), 
    water_year = mean(water_year),
    qualifier  = first(qualifier)              # n = 58,074 obs 
  ) %>% 
  ungroup() %>% 
  rename(sta = sta_orig)
...
gage_test1 <- bind_rows(gage, gage_part_miss)

gage_test2 <- gage_test1 %>%  
  group_by(sta, water_year) %>% 
  summarize(count = n()) %>% 
  ungroup() %>% 
  filter(
    ...
    sta != "sna_dou" & 
    sta != "che_ang") %>% 
  filter(count > 366)  # 71 cases

gage_dups <- semi_join(gage_test1, gage_test2) %>% 
  filter(is.na(date)) 

gage_est_miss <- gage_est_miss %>% 
  filter(!is.na(date))     # n = 58,074 obs
...
#export(gage_est_miss, file = "data/gage_est_9017.csv") 
...
# fill_missing_data_wet_years_run04,
rm(gage_clust_l) 
