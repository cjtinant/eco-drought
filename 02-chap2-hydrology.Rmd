...
# import estimates from s prior run 
#gage_est_9091 <- import(file = "data/gage_flow_9091.csv") %>%  #run02
#  select(-c(group, contrib_drain_area_va)) %>% 
#  mutate(qualifier = "est")

# bind the gage dfs together, make min & max yr vars - clean up
gage <- bind_rows(gage_cont, gage_disc, 
                  gage_other)  %>%  
  distinct() %>%                              # orig  n = 286,954
  group_by(sta) %>%                           # run02 n = 289,144
  mutate(min_yr = min(water_year)) %>% 
  mutate(max_yr = max(water_year)) %>% 
           ungroup()
rm(gage_cont, gage_disc, gage_other, gage_est_9091)                    

# find actual count of years - post1990  
gage_summary <- gage %>%                               # N = 44 sta
  group_by(sta) %>%                              # orig  n = 39 sta 
  summarise(min_yr = min(water_year),            # run02 n = 36 sta 
...
gage <- semi_join(gage, gage_summary, by = "sta")  # orig n = 275,281 
                                                  # run02 n = 277,471
# drop other stations with issues  
gage <- gage %>%                                           
  filter(sta != "bad_mid") %>%       # incomplete days
  filter(sta != "che_ang") %>%       # incomplete days
  filter(sta != "che_was") %>%       # run01 - dropped for >> area 
  filter(sta != "whi_cra")           # run01 - dropped for short rec.

# update gage summary & separate out incomplete years of record
gage_summary <- gage %>%                      # orig  n = 713 obs
  group_by(sta, water_year) %>%               # run02 n = 719 obs 
  summarise(days_record = n()) %>%          
  ungroup()                         

gage_incomp_years <- gage %>%                 # orig  n = 15 obs 
  group_by(sta, water_year) %>%               # run02 n = 15 obs
  summarise(days_record = n()) %>%            # 
  ungroup() %>% 
  filter(days_record < 365)                           

# split stations-years with incomplete days of record 
gage_incomp_years <- semi_join(gage, gage_incomp_years, 
                               by = c("sta", "water_year"))
  
gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>% # orig  n = 254,945 
  filter(days_record >= 365)                      # run02  n = 257,135    

# filter metadata to active stations 
# & add area to gage & gage_incomp_year 
gage_meta <- semi_join(gage_meta, gage_summary, by = "sta")

gage <- full_join(gage, gage_meta, 
                  by = "sta") %>% 
  select(date, sta, q, log_q:q30, water_year, contrib_drain_area_va, 
         qualifier)

gage_incomp_years <- full_join(gage_incomp_years, gage_meta, 
                  by = "sta") %>% 
...
# set up a road-map
gage_summary_sp <- gage_summary %>% 
  spread(water_year, days_record) 

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# fix missing station vals - run 01
...
# run01 outcome - dropped cas_hot, sna_bur, & che_was
rm(gage_run01)
...
gage_run02 <- gage_summary %>% 
  filter(water_year < 2002) %>% 
  spread(water_year, days_record) %>% 
  filter(!is.na(`1992`)) %>%  # 28 sta w/ complete recs [1992, 1997]
  select(sta)         # run03 - 28 sta w/ complete recs [1992, 1997]

gage_run02 <- inner_join(gage, gage_run02, by = "sta") 
gage_run02 <- gage_run02 %>%  
  filter(water_year <= 1997) %>%              ##       n = 47,476 
  filter(water_year >= 1993)                  #        n = 47,476
                                   # blp_bel & wkn_wok have na in 92

# update road-map to show only stations with data in 1993
gage_summary_sp <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(sta:'1993') %>% 
  rename(wy_1990 = '1990') %>% 
...
# set up to identify lcr_abv during drought years: [1997, 2002]
# also need to add data for partial missing years:
#     bev_pri-1991, blp_bel-1992,lcr_abv-1996, lwr_abv-2000, 

# prepare for run03 by creating a set of stations w/o NA values
gage_run03 <- gage_summary %>% 
  filter(water_year <= 2002) %>% 
  ...
  filter(!is.na(`1998`)) %>%
  filter(!is.na(`2002`)) %>%   
    select(sta)      # run03 - 22 sta w/ complete recs [1997, 2002]

gage_run03 <- inner_join(gage, gage_run03, by = "sta") 
gage_run03 <- gage_run03 %>%  
  filter(water_year <= 2002) %>%            
  filter(water_year >= 1997)                  #        n = 48,202 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# gage_input changes with each run...  sorry!
gage_input <- gage_run02 
rm(gage_run02)

# name check
# gage - the complete set of flow & metadata for 1990-2018 stations 
# gage_meta - complete set of metadata for 27 stations
# gage_summary - stations, water_year, days record 
# gage_clust - station groups & PCA_means 

# gage_miss_names - ? 
# gage_mod <- stations, groups, PCA & probabilities 
# incomp_days - missing days for stations by year
# partial days - partial year data for runs 
...
lambda_q1 <- BoxCox.lambda(pca_input$q1_depth)   # q1  ==> ~0.01  
lambda_q7 <- BoxCox.lambda(pca_input$q7_depth)   # q7  ==> ~0.10  
lambda_q30 <- BoxCox.lambda(pca_input$q30_depth) # q30 ==> ~0.20    
...
# run01:dropped cascade springs & sna_bur - outliers w/
# substantial GW inputs & not in study area

# num  min_yr max_yr  n_obs  lambda_q1  lambda_q7  lambda_q30
#  01   1990   1995  48,202    0.0129     0.0110     0.2250
#  02   1993   1997  49,302    0.0073     0.0101     0.1960
...
gage_clust_l <- Mclust(gage_clust) # Model-based-clustering 
summary(gage_clust_l) # Print a summary 

# Extract results - add metadata - drop low probs - join & rearrange
...
# join cluster to PCA data & clean up environment 
...
rm(gage_clust_meta, gage_pca_sum, gage_pca) 

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Mclust EEV (ellipsoidal, equal volume and shape) model with 5
#components:   

# num  model  log.likeli  n   df     BIC      ICL
#  01   EEV    147.8495   22  51  138.0558 138.0555
#  02   EEV    166.7266   26  44  190.097  190.0732
...
#Run 02 Clustering table:
# 1  2  3  4  5 
# 7  5  2 11  2 

#1 2 3 4 5 6 - new after changing some boundary conditions
#5 5 2 4 8 2
...
# 1. make roadmap of sta with missing & partial years
gage_part_miss_yr <- gage_summary_sp %>% 
  gather(key = year, val = days_record, -sta) %>% 
  filter(is.na(days_record)| 
           days_record <365) %>% 
  spread(year, days_record)

# attach station groups to the station - missing year pairs
gage_part_miss_yr <- full_join(gage_clust, gage_part_miss_yr, 
                               by = "sta") %>% 
  select(-c(PC1_mean:q1_q30_mean)) %>% 
  select(-c(eigen_dist, prob))

#  spread(water_year, days_record) %>% 
# gather(key = water_year, val = days_record, -sta) %>% 
# select(-days_record) %>% 
#  mutate(grouped_id = row_number()) %>%
 # spread(water_year, sta) %>% 
#  select('1990':'2002') 

# 2. make df of streamflow by group & filter for missing water years
gage_grouped <- inner_join(gage, gage_clust, by = "sta") %>% 
  select(date:group) %>% 
  group_by(date, group) %>% 
...
#gage_grouped <- bind_rows(gage, gage_incomp_years)

# Find 1990 missing stations---- 
# 1. identify stations with missing data in 1990
#gage_miss_1990 <- as.data.frame(gage_sum_miss_yr$'1990') %>% 
#  rename(sta = 1) %>% 
#  filter(!is.na(.)) 

gage_miss_1990 <- semi_join(gage_clust, 
                            gage_miss_1990, by = "sta") %>% 
  select(group, sta)

# 2. subset streamflows for water-year 1990 for  
gage_flow_1990 <- gage_grouped %>% 
  filter(water_year == 1990) 

# 3. join missing stations to 1990 streamflow data
gage_flow_1990 <- full_join(gage_miss_1990, gage_flow_1990) %>% 
  filter(!is.na(sta)) 

rm(gage_miss_1990)

# Find 1991 missing stations---- 

# 1. identify stations with missing data for 1991
gage_miss_1991 <- as.data.frame(gage_sum_miss_yr$'1991') %>% 
  rename(sta = 1) %>% 
  filter(!is.na(.))     # gets the 1991 missing stations

gage_miss_1991 <- semi_join(gage_clust, 
                            gage_miss_1991, by = "sta") %>% 
  select(group, sta)

# 2. subset streamflows for run02 stations for water-year 1991 
gage_flow_1991 <- gage_grouped %>% 
  filter(water_year == 1991) 

# 3. join missing stations to 1991 streamflow data
gage_flow_1991 <- full_join(gage_miss_1991, gage_flow_1991) %>% 
  filter(!is.na(sta))

rm(gage_miss_1991)

# append and join to date values
gage_flow_est <- bind_rows(gage_flow_1990, gage_flow_1991) 

# clean up 

export(gage_flow_est, file = "data/gage_flow_9091.csv") 

rm(gage_flow_1990, gage_flow_1991)
rm(lambda_q1, lambda_q30, lambda_q7)
```

```{r fill_missing_data_wet_years_run03}
rm(gage_clust_l) 

# 1. make roadmap of sta with missing years
gage_sum_miss_yr <- gage_summary %>% 
  spread(water_year, days_record) %>% 
...
# 2. make df of streamflow by group after rejoining incomp data 
gage_grouped <- bind_rows(gage, gage_incomp_years)

gage_grouped <- inner_join(gage_grouped, gage_clust, by = "sta") %>% 
  select(date:group) %>% 
  group_by(date, group) %>% 
  summarise(q          = mean(q, na.rm = TRUE), 
            log_q      = mean(log_q, na.rm = TRUE),
...
```{r old-code-delete-soon}
# prepare by-group streamflow depths for groups
#gage_pred_run02 <- gage_mod %>% 
#  select(group:q30_depth) %>% 
#  select(-sta) %>% 
#  gather(key = dur, value = depth, -group, -date) %>% 
#  group_by(group, date, dur) %>% 
#    summarise(depth_ave = mean(depth)) %>% 
#  spread(dur, depth_ave) %>% 
#  ungroup() 

#date_vals  <- gage %>% 
#  select(date, julian, month, day, dec_year, 
#         month_seq, water_year, qualifier) %>% 
...
# bev_prix      y        n      n    bev_buf
# blp_belx      y        n      n    brsf_co
# lwr_abv      y        n      n    lwr_ros
# ros_ros      y        n      y    lwr_abv
# whi_slmx      y        n      y    whi_ogl
# wkc_wokx      y        n      y    bev_buf
# bev_abfx      n        y      y    frn_fai
# rap_farx      n        y      n    fal_hot 



# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# goal of run02 is to find neighbors for: 
# bev_abf -> frn_fai
# bev_pri -> bev_buf 
# blp_bel -> whi_kad
# rap_far -> ros_ros
# whi_slm -> (whi_ogl + whi_sta)/2
# whi_sta -> whi_slm
# wkc_wok -> bat_bhr
