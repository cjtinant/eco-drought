...
# Change to time series for input
# This creates -Inf values in data for 'rap', 'int', 'ora' 
sta_ts <- sta_mon %>% 
  select(-c(year, month, date)) %>%
  ts(end = c(2018, 05), frequency = 12)

spi_list  <- spi(sta_ts, 1, 
                 distribution = 'PearsonIII', na.rm = TRUE) 

spi1_coeff <- as.tibble(spi_list$coefficients) %>% 
  t() %>% as.tibble() 

spi1_ts <- as.tibble(spi_list$fitted) 
spi1_ts <- bind_cols(Date, spi1_ts) 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Use only stations with long record
# This creates same -Inf values in data for 'rap', 'int', 'ora'  
sta_ts_long <- sta_mon %>% 
  select(cot, oel) 

spi_list  <- spi(sta_ts_long, 1, 
                 distribution = 'PearsonIII', na.rm = TRUE) 

spi1_coeff <- as.tibble(spi_list$coefficients) %>% 
  t() %>% as.tibble() 

spi1 <- as.tibble(spi_list$fitted) 
spi_1 <- bind_cols(Date, spi_1) 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
