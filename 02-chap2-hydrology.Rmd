...
# import and append daily flow values > 1990 
gage_orig <- import(file = "data/gage_inputs.csv")   # n = 245,614 obs 
...
# This code chunk prepares to estimate missing values.  The years 
...
# 01    wet     1992     26      351     111,396   1,748  
# 02    dry     2006     23      345     125,286       0 
# 03    wet     1996     27      338     125,811     714 
# 04    dry     2017     24      360     129,862     192 
# 05    all     2017     24      672     228,615     141 


# 06    all     none     31      868     295,440       0
...
gage_run01 <- semi_join(gage, gage_run01_sum, # 109,205
                        by = c("sta", "water_year")) # n = 111,396 obs
...
gage_run04_sum <- gage_summary %>% 
  spread(water_year, days_record) %>%           
  select(c(sta,'2003':'2017')) %>%    # subsets the matrix n = 24 sta
  filter(!is.na(`2017`)) %>%                                
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
...
gage_run04 <- semi_join(gage, gage_run04_sum, 
                        by = c("sta", "water_year")) # n = 129,862 obs
...
gage_run05_sum <- gage_summary %>% 
  spread(water_year, days_record) %>%               # n =      24 sta    
    filter(!is.na(`2017`)) %>%      
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  mutate(days_record = case_when(
...
gage_run05 <- semi_join(gage, gage_run05_sum, 
                        by = c("sta", "water_year")) # n = 228,615 obs
...
gage_run06_sum <- gage_summary %>% 
  spread(water_year, days_record)  %>%               # n =      31 sta    
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                 # n =     868 obs

gage_run06 <- semi_join(gage, gage_run06_sum, 
                        by = c("sta", "water_year")) # n = 295,294 obs
...
# update incomplete observations
gage_run06_incomp <- semi_join(gage_incomp, gage_run06_sum, 
                  by = c("sta", "water_year")) %>%  
  filter(!is.na(date))                               # n =       0 obs
# ~~~~~~~~~~~~~~~~~~~~~~~ 

# show a roadmap 
gage_run06_map <- semi_join(gage_summary, gage_run06_sum,
                            by = "sta") %>% 
  spread(water_year, days_record)
...
gage_run07 <- semi_join(gage, gage_run07_sum, 
                       by = c("sta", "water_year")) # n = 317,2910 obs
...
# show a roadmap 
gage_run07_map <- semi_join(gage_summary, gage_run07_sum,
                            by = "sta") %>% 
  spread(water_year, days_record)
...
#  01    0.028      0.111      0.278     0.941   0.049   0.944   0.993    
#  02   -0.007      0.071      0.244     0.947   0.047   0.947   0.994    
#  03   -0.066     -0.101     -0.145     0.954   0.049   0.963   0.994  
#  04   -0.001      0.067      0.202     0.952   0.042   0.952   0.994   
#  05   -0.002      0.071      0.067     0.961   0.032   0.961   0.993 

#  06   -0.002      0.071      0.067     0.961   0.032   0.961   0.993 

pca_vars 
# run  PC  q1_depth  q7_depth  q30_depth       
#   01  1    0.577      0.589     0.565   
#   01  2    0.580      0.191    -0.792    
#   02  1    0.578      0.588     0.566       
#   02  2    0.566      0.211    -0.797      
#   03  1    0.578      0.585     0.569           
#   03  2    0.547      0.240    -0.802       
#   04  1    0.577      0.587     0.568           
#   04  2    0.575      0.201    -0.793      
#   05  1    0.578      0.584     0.571           
#   05  2    0.568      0.214    -0.794       


#   06  1    0.578      0.583     0.571           
#   06  2    0.545      0.245    -0.802     
#   07  1    0.578      0.583     0.571           
#   07  2    0.541      0.250    -0.803      
...
#  01  VVV     103.97    26  19  146.0   145.5   estimate NA 
#  02  VEV      79.98    23  17  106.7   106.2   estimate NA 
#  03  VVV     157.59    31  19  249.9   249.9   estimate NA   
#  04  EEV     128.57    24  44  117.3   117.2   estimate NA    
#  05  EEV     152.26    24  51  142.4   142.4   estimate NA    

# Clustering table:
# run#  Cl1#  Cl2#  Cl3#  Cl4#  Cl5#  Cl6#  Cl7#
#  01    7      2    14     4    na    na    na  
#  02   15      8    na    na    na    na    na  
#  03   15     16    na    na    na    na    na  
#  04    5      4     3     5     6     1    na 
#  05    2      8     4     1     4     3     2  
...
gage_part_miss <- gage_run01_sum %>% 
  filter(days_record < 365) %>% 
    spread(water_year, days_record)                   # n = 9 x 9 obs

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                          # n = 39 obs
...
gage_part_miss <- inner_join(gage_part_miss, gage_pairs,
                        by = c("sta_orig", "group"))  # n = 78 pairs 
...
gage <- import(file = "data/gage_inputs.csv")       # n = 245,614  
...
gage_dups <- semi_join(gage_est_9002, gage_summary) %>% 
  filter(qualifier != "est")                           # 0 obs

gage_est_9002a <- anti_join(gage_est_9002, gage_dups, 
                            by = c("date", "sta"))    # n = 12,667
...
# import and append daily flow values > 1990 
gage_orig <- import(file = "data/gage_inputs.csv")       # n = 245,614 
...
# 7. join the missing water-year pairs 
gage_part_miss <- inner_join(gage_part_miss, gage_pairs,
                        by = c("sta_orig", "group"))  # n = 14 pairs 
...
gage <- bind_rows(gage_orig, gage_est_9002a, 
                  gage_est_0317, gage_est_9002b)  
                                                     # n = 261,569 obs
gage_summary <- gage %>%                          
  group_by(sta, water_year) %>%                   
  summarise(days_record = n()) %>%                # n run03 = 770 obs   
  ungroup()     

# make a df of prior observations, test for dups & update export df
gage_test <- bind_rows(gage_orig, gage_est_9002a) 

gage_dups <- semi_join(gage_est_9002b, gage_test, 
                       by = c("date", "sta")) # 172 obs
...
gage_summary <- gage %>%                          
  group_by(sta, water_year) %>%                   
  summarise(days_record = n()) %>%                 # n run03 = 724 obs   
  ungroup()     
...
```{r fill_missing_data_wet_years_run06, eval=FALSE} 
 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# 1. create a df of missing & partial year-station pairs 
gage_part_miss <- gage_run06_sum %>% 
  filter(days_record < 365) %>% 
    spread(water_year, days_record)                  # n = 4 x 16 obs 

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 60 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  rename(sta_orig = sta)
...
gage_pairs <- gage_clust %>% 
  ...
  rename(group_sta = group) %>% 
  rename(sta_orig = sta) %>% 
  select(group_sta, sta_orig, sta_lead, sta_lag) # n = 32 = gage_clust
...
gage_est_0317  <- import(file = "data/gage_est_0317.csv")
                                                     # n =   2,365 obs
...
gage_summary <- gage %>%                          
  group_by(sta, water_year) %>%                   
  summarise(days_record = n()) %>%                # n = 868 obs   
  ungroup()     
