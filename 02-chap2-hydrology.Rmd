...
# The end of the 'wet-cycle' is 2002. 
...
gage_disc <- import("data/gage_disc.csv") %>% 
  ...
  filter(water_year >= 1990)                        # n =  27,313
...
# make a summary of mostly complete years
gage_yr_summary <- gage %>% 
  group_by(sta, water_year) %>% 
  summarise(days = n()) %>% 
  filter(days > 360) %>%              # filters out yrs w/ < 360 days
  group_by(sta) %>% 
  summarise(count_yrs_act = n()) %>% 
  ungroup()

# join count of years, clean up, look at na vals, find expected yrs 
gage <- full_join(gage, gage_yr_summary, by = "sta") 

gage_sta_na <- gage %>% 
  filter(is.na()) %>%    # bad river midlands removed
  distinct(sta) 
...
# summarize stations & remove very short records
