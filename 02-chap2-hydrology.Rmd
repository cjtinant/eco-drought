...
2.1.1 Precip data: finished
2.1.2 Stream data: finished - July 16
2.2   Identify distribution: _05_L-moment_diagram
2.3   Calculate SPI: this; but having difficulties!
2.3.1 Learn purrr::map(): ok.finished http://r4ds.had.co.nz/iteration.html#the-map-functions
2.4   Cluster time series - in progress July 16
2.5   Calculate SDI 
...
# For clustering:  rows are observations and columns are variables 
# (in the first case stations, in the final case stations & averaging 
#  periods).  
<<<<<<< HEAD
# Dropped Cheyenne River at Angustora with  ~50% NA values
...
>>>>>>> 908e8dab17af20879f182cb195eb9ef7fdfffe99
# The original data is saved in a long format: 
# sta                   <chr> "bat_bhr", "bat_bhr", "bat_bhr", "bat...
# contrib_drain_area_va <dbl> 284, 284, (in mi)
# Q                     <dbl> 0.12518933, 0.12518933, 0.14217944, 0...
# Q7                    <dbl> 0.14420207, 0.14015681, 0.13894323, 0...
# Q30                   <dbl> 0.1342507, 0.1346283, 0.1356666, 0.13...

# 2. To eliminate the effects of different sizes of watersheds  
#    calculate daily flow depths by dividing flow (cms) by watershed 
#    area (sq-km) and multiplying the resultant by the number of 
#    seconds in a day

# 3. Missing values in the data must be removed or estimated. 
#    The NA vals created by calculating Q7 & Q30 need to be
#    removed or estimated.  Removal seems the more conservative 
#    approach - especially as the NA values are very small given 
#    the number of observations of 365 obs/yr * 7 years. 

gage_prep <- gage_raw %>% 
  as.tibble() %>% 
  clean_names() %>% 
<<<<<<< HEAD
  select(sta, contrib_drain_area_va, q, q7, q30, date) %>% 
=======
  select(sta, contrib_drain_area_va, q, q7, q30) %>% 
>>>>>>> 908e8dab17af20879f182cb195eb9ef7fdfffe99
  mutate(contrib_drain_area_km = contrib_drain_area_va * 2.59) %>% 
  mutate(q_depth = q * (60*60*24) / contrib_drain_area_km) %>% 
  mutate(q7_depth = q7 * (60*60*24) / contrib_drain_area_km) %>% 
  mutate(q30_depth = q30 * (60*60*24) / contrib_drain_area_km)  %>% 
<<<<<<< HEAD
  select(sta, q_depth, q7_depth, q30_depth, date) 
  
=======
  select(sta, q_depth, q7_depth, q30_depth) %>% 
  drop_na() 

>>>>>>> 908e8dab17af20879f182cb195eb9ef7fdfffe99
# glimpse(gage_prep)
# sta       <chr> "bat_bhr", "bat_bhr", "bat_bhr", "bat_bhr", "b...
# q_depth   <dbl> 14.704930, 14.704930, 16.700614, 18.696298, 17...
# q7_depth  <dbl> 16.93820, 16.46303, 16.32048, 16.46303, 16.463...
# q30_depth <dbl> 15.76929, 15.81364, 15.93560, 16.12408, 16.268...

# 4. The data must be standardized (i.e., scaled) such that they have 
# mean zero and standard deviation one (z-score). 
...
# drop the NA values (num obs drops from 2192 to 1945)
