...
gage_orig <- gage_orig %>% 
  ...
  gather(key = q_type, val = q_val,       # prepares to censor to 0.01 cfs
         -c(sta, water_year, date, qualifier)) 
...
# START HERE
```{r cluster-testing_fix-PC-direction} 
# NEXT STEPS: change gage_clust_all2 -> gage_clust_all 
# imports clustering analysis group results 
gage_clust <- import(file="gage_clusters.csv") %>% 
  select(-eigen_dist) 

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# splits gage_clusters for pre-processing 
...
```{r cluster-testing_rename-vars} 
# fixes the different clusters so they are in the same PC direction 
# renames variables for wet & dry parts to prepare for cbind
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
gage_clust_all <- gage_clust_all %>% 
  mutate(PC1_mean             = PC1_mean) %>% 
  mutate(PC2_mean             = PC2_mean) %>% 
  mutate(group_hydro          = case_when( 
         group                == "V4" ~ "eph", 
         group                == "V5" ~ "G2-G3", 
         group                == "V1" ~ "G2-G3", 
         group                == "V2" ~ "G4", 
    ...
    rename(group_all          = group) 

gage_clust_wet <- gage_clust_wet %>% 
  mutate(PC1_mean           = PC1_mean) %>% 
  mutate(PC2_mean           = PC2_mean) %>% 
...
# Find the convex hulls of the group_alls
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
...
hull_G1 <- gage_clust_all %>% 
  filter(group_hydro == "eph")  %>% 
  slice(chull(PC1_mean, PC2_mean)) 

hull_G2_G3 <- gage_clust_all %>% 
  filter(group_hydro == "G2-G3") %>% 
  slice(chull(PC1_mean, PC2_mean)) 

hull_G4 <- gage_clust_all %>% 
  filter(group_hydro == "G4") %>% 
  slice(chull(PC1_mean, PC2_mean))

hull_G5 <- gage_clust_all %>% 
  filter(group_hydro == "G5") %>% 
  slice(chull(PC1_mean, PC2_mean)) 

gage_clust_hull <- bind_rows(hull_G1, hull_G2_G3, hull_G4, hull_G5) 

rm(hull_G1, hull_G2_G3, hull_G4, hull_G5)
```


```{r cluster-testing_cbind_clusters} 
# binds cluster data as wide data
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
...
gage_clust <- gage_clust %>% 
  select(sta, group_hydro, group_all, group_wet, group_dry, 
...
# clean-up 
rm(gage_clust_all, gage_clust_wet, gage_clust_dry)
```

```{r Clustering_visualization} 
p <- ggplot(gage_clust, aes(PC1_mean, PC2_mean)) + 
    geom_jitter(aes(shape = as.factor(group_all),
                    color = as.factor(group_all))) +   
#  scale_x_continuous(limits = c(-0.8, 2)) + # -4.1
#  scale_y_continuous(limits = c(-0.2, 0.15)) +  # -0.4, 0.2
  theme_classic() 

# Overlay the convex hull
p + geom_polygon(data = gage_clust_hull, 
                 alpha = 0.5, 
                aes(group = group_hydro,
                    fill = group_hydro))
#p <- ggplot(mtcars, aes(mpg, wt)) + geom_point(shape = 21)
```
# find the relative temporal differences (wet vs dry) 



```{r cluster-testing_fix_random-groups} 
# creates new groups based on hydrologic export chars 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
# all <-> mutate(group_hydro = case_when( 
#        group == "V4" ~ "eph", 
#        group == "V5" ~ "G2-G3", 
#        group == "V1" ~ "G2-G3", 
#        group == "V2" ~ "G4", 
...
# wet <-> mutate(group_hydro = case_when( 
#        group == "V6" ~ "eph", 
#        group == "V3" ~ "eph",         
#        group == "V4" ~ "G2-G3", 
#        group == "V2" ~ "G2-G3", 
...
# dry <-> mutate(group_all = case_when( 
#        group == "V8" ~ "eph", 
#        group == "V7" ~ "eph",         
#        group == "V5" ~ "eph", 
#        group == "V6" ~ "G2-G3", 
...
ggplot(gage_clusters, aes(q1_mean, q30_mean)) + 
  geom_point()

gage_test <- gage_clusters %>% 
  filter(group2 == "G5" | 
         group2 == "G4") # | 
         #  group2 == "eph")
...
ggplot(gage_test, aes(PC1_mean, PC2_mean)) + 
  geom_density2d(na.rm = TRUE, contour = TRUE, linejoin = "round",
                 aes(alpha = 0.5, color = as.factor(group2)))  +
  geom_jitter(aes(shape = as.factor(type),
                  color = as.factor(sta))) +   
  scale_x_continuous(limits = c(-0.8, 2)) + # -4.1
  scale_y_continuous(limits = c(-0.2, 0.15)) +  # -0.4, 0.2
  theme_classic() 
...
# ephemeral streams PC2 increases with drier
# hor_oel - least efficient & most flashy 
# hat_edg - close to hor_oel
# brsf_co - most stable of the ephemeral streams
...
# A tibble: 31 x 4 Dry 
#   group sta     PC1_mean PC2_mean
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ White River
# 1 V1    whi_int   0.351   0.139  
# 2 V1    whi_kad   0.256   0.118  
# 3 V1    whi_slm  -0.0451  0.0933 
# 4 V1    whi_whi   0.346   0.102  
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 5 V2    bat_her  -0.400  -0.0183 
# 6 V2    bev_buf  -0.531  -0.0202 
# 7 V2    fal_hot  -1.02   -0.0491 
# 8 V2    frn_fai  -0.270  -0.00917
# 9 V2    lcr_bel  -0.761  -0.00208
#10 V2    lcr_vet  -0.730  -0.0314 
#11 V2    lwr_abv  -0.771  -0.0342 
#12 V2    lwr_mar  -0.504  -0.0199 
#13 V2    lwr_ros  -0.839  -0.0374 
#14 V2    lwr_whi  -0.690  -0.0278 
#15 V2    rap_far  -0.654  -0.0180 
#16 V2    wcc_ogl  -0.731  -0.0122 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#17 V3    bev_abf  -1.46   -0.0798 
#18 V3    lcr_abv  -1.25   -0.0634 
#19 V3    ros_ros  -1.31   -0.0694 
#20 V3    spr_her  -0.778  -0.0512 
#21 V3    wkc_wok  -1.60   -0.0960 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Black Pipe
#22 V4    blp_bel   0.562   0.210  
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Ephemeral - flashy
#23 V5    brsf_co   2.39    0.284  
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
#24 V6    bat_bhr   0.234  -0.00528
#25 V6    bev_pri   0.247  -0.0121 
#26 V6    che_buf   0.679  -0.0157 
#27 V6    che_red   0.570  -0.0123 
#28 V6    whi_ogl   1.03   -0.00813
#29 V6    whi_sta   0.707   0.0143 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  Ephemeral stable
#30 V7    hat_edg   2.90   -0.0708 
#31 V8    hor_oel   4.07   -0.197  

# add area to gage & gage_incomp_year 
gage_meta <- import("data/gage_meta_full.csv") 
gage_meta <- semi_join(gage_meta, gage_summary, by = "sta")

gage_clusters <- full_join(gage_clusters, gage_meta, by = "sta") %>% 
  select(type, group, sta, contrib_drain_area_va, everything()) 
...
# A tibble: 31 x 4 - All Years
#   group sta     PC1_mean  PC2_mean
#   <chr> <chr>      <dbl>     <dbl>
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  precip regime 
# 1 V1    bat_bhr  -0.0923  0.0476  
# 2 V1    bev_pri  -0.250   0.0756  
# 3 V1    blp_bel  -0.488  -0.177   
# 4 V1    spr_her   0.193   0.00909 
# 5 V1    whi_int  -0.509  -0.0697  
# 6 V1    whi_kad  -0.339  -0.0697  
# 7 V1    whi_slm  -0.292  -0.0145  
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  precip regime 
# 8 V2    bat_her   0.533   0.0353  
# 9 V2    bev_buf   0.515   0.0386  
#10 V2    frn_fai   0.377   0.0338  
#11 V2    lcr_bel   0.987  -0.0302  
#12 V2    lcr_vet   0.850   0.0237  
#13 V2    lwr_abv   0.899   0.0220  
#14 V2    lwr_mar   0.538   0.0354  
#15 V2    lwr_ros   0.978   0.0174  
#16 V2    lwr_whi   0.776   0.0246  
#17 V2    rap_far   0.834   0.0127  
#18 V2    wcc_ogl   0.526   0.000529
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ # groundwater control
#19 V3    bev_abf   1.45   -0.0225  
#20 V3    fal_hot   1.21    0.00635 
#21 V3    lcr_abv   1.48   -0.0215  
#22 V3    ros_ros   1.58   -0.0315  
#23 V3    wkc_wok   1.64   -0.0425  
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Small river - small export
#24 V4    brsf_co  -2.12   -0.317   
#25 V4    hat_edg  -2.63    0.0286  
#26 V4    hor_oel  -3.52    0.0609  
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Large river small export
#27 V5    che_buf  -1.34    0.109   
#28 V5    che_red  -1.25    0.109   
#29 V5    whi_ogl  -0.899   0.0482  
#30 V5    whi_sta  -0.652   0.0464  
#31 V5    whi_whi  -0.971   0.0103  
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#   <chr> <chr>      <dbl>    <dbl>
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  precip regime 
# 1 V1    blp_bel  -0.291  -0.184  
# 2 V1    whi_int  -0.497  -0.0780 
# 3 V1    whi_kad  -0.233  -0.0931 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  mixed regime 
# 4 V2    bat_bhr   0.256   0.0379 
# 5 V2    bev_pri   0.0143  0.0444  
# 6 V2    whi_ogl  -0.571   0.00258
# 7 V2    whi_slm  -0.565   0.0167  
# 8 V2    whi_sta  -0.350   0.00542 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  ephemeral  
# 9 V3    brsf_co  -1.81   -0.379  
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Large river? small export
#10 V4    che_buf  -1.78    0.0684 
#11 V4    che_red  -1.73    0.0766 
#12 V4    spr_her  -0.916   0.102  
#13 V4    whi_whi  -1.57    0.0644 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ # groundwater control 
#14 V5    bat_her   0.835   0.0200 
#15 V5    bev_abf   0.937   0.0233 
#16 V5    bev_buf   0.601   0.0184 
#17 V5    fal_hot   1.30    0.0211 
#18 V5    frn_fai   0.680   0.0105 
#19 V5    lcr_abv   1.46    0.00969
#20 V5    lcr_bel   1.18   -0.0165 
#21 V5    lcr_vet   1.02    0.0191 
#22 V5    lwr_abv   1.06    0.0188 
#23 V5    lwr_mar   0.693   0.0135 
#24 V5    lwr_ros   1.12    0.0189 
#25 V5    lwr_whi   0.920   0.0147 
#26 V5    rap_far   1.10    0.0122 
#27 V5    ros_ros   1.57    0.00384
#28 V5    wcc_ogl   0.189   0.00896
#29 V5    wkc_wok   1.03    0.00731
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Small river - small export
#30 V6    hat_edg  -2.46    0.0340 
#31 V6    hor_oel  -3.19    0.0777 




# |   sta   | wet-group | dry-group | all-group | area | 
# |:-------:|:---------:|:---------:|:---------:|:----:| 

# | ros_ros |    V3     |   V3      |    V5    |  49.0 | 
# | lcr_abv |    V3     |   V3      |    V5    |  57.0 | 
# | wkc_wok |    V3     |   V3      |    V5    |  73.5 | 
# | frn_fai |    V2     |   V2      |    V5    |  105  | 
# | bev_abf |    V3     |   V3      |    V5    |  111  |  
# | lcr_bel |    V2     |   V2      |    V5    |  121  | 
# | bev_buf |    V2     |   V2      |    V5    |  127  |  
# | fal_hot |    V2     |   V2      |    V5    |  136  |  
# | bat_her |    V2     |   V2      |    V5    |  169  | 
# | bat_bhr |    V6     |   V6      |    V5    |  232  | 
# | brsf_co |    V5     |   V5      |    V3    |  257  | 

# | bev_pri | V6-whi-up |   V6      | V2-whi-up  |  45.6 | 
# | blp_bel |    V4     |   V4      |    V1-badl   |  273  | 

# | whi_slm | V1-whi-riv | V1-whi-riv | V2-whi-up  | 1367  | 
# | whi_sta | V6-whi-up  | V6-whi-up  | V2-whi-up  | 1426  | 
# | whi_ogl | V6-whi-up  | V6-whi-up  | V2-whi-up  | 2156  | 

# | whi_int | V1-whi-riv | V1-whi-riv | V1-badl | 4094  | 
# | whi_kad | V1-whi-riv | V1-whi-riv | V1-badl | 4953  | 

# | whi_whi | V1-whi-riv | V1-whi-riv | V4-whi-low | 6202  | 





#      sta group type contrib_drain_area_va

#4  bat_bhr    V2  all                 284.0
#5  bev_pri    V2  all                  45.6

#9  brsf_co    V3  all                 257.0
#10 che_buf    V4  all                9788.0
#11 che_red    V4  all               10254.0
#12 spr_her    V4  all                 205.0
#13 whi_whi    V4  all                6202.0
#14 bat_her    V5  all                 169.0
#15 bev_abf    V5  all                 111.0
#16 bev_buf    V5  all                 127.0
#17 fal_hot    V5  all                 136.0
#18 frn_fai    V5  all                 105.0
#19 lcr_abv    V5  all                  57.0
#20 lcr_bel    V5  all                 121.0
#21 lcr_vet    V5  all                 571.0
#22 lwr_abv    V5  all                 855.0
#23 lwr_mar    V5  all                 299.0
#24 lwr_ros    V5  all                 979.0
#25 lwr_whi    V5  all                1495.0
#26 rap_far    V5  all                 603.0
#27 ros_ros    V5  all                  49.0
#28 wcc_ogl    V5  all                 375.0
#29 wkc_wok    V5  all                  73.5
#30 hat_edg    V6  all                 956.0
#31 hor_oel    V6  all                 188.0

#36 bat_her    V2  wet                 169.0
#37 bev_buf    V2  wet                 127.0
#38 fal_hot    V2  wet                 136.0
#39 frn_fai    V2  wet                 105.0
#40 lcr_bel    V2  wet                 121.0
#41 lcr_vet    V2  wet                 571.0
#42 lwr_abv    V2  wet                 855.0
#43 lwr_mar    V2  wet                 299.0
#44 lwr_ros    V2  wet                 979.0
#45 lwr_whi    V2  wet                1495.0
#46 rap_far    V2  wet                 603.0
#47 wcc_ogl    V2  wet                 375.0
#48 bev_abf    V3  wet                 111.0
#49 lcr_abv    V3  wet                  57.0
#50 ros_ros    V3  wet                  49.0
#51 spr_her    V3  wet                 205.0
#52 wkc_wok    V3  wet                  73.5
#53 blp_bel    V4  wet                 273.0
#54 brsf_co    V5  wet                 257.0
#55 bat_bhr    V6  wet                 284.0
#56 bev_pri    V6  wet                  45.6
#57 che_buf    V6  wet                9788.0
#58 che_red    V6  wet               10254.0

#61 hat_edg    V7  wet                 956.0
#62 hor_oel    V8  wet                 188.0

#67 bat_her    V2  dry                 169.0
#68 bev_buf    V2  dry                 127.0
#69 fal_hot    V2  dry                 136.0
#70 frn_fai    V2  dry                 105.0
#71 lcr_bel    V2  dry                 121.0
#72 lcr_vet    V2  dry                 571.0
#73 lwr_abv    V2  dry                 855.0
#74 lwr_mar    V2  dry                 299.0
#75 lwr_ros    V2  dry                 979.0
#76 lwr_whi    V2  dry                1495.0
#77 rap_far    V2  dry                 603.0
#78 wcc_ogl    V2  dry                 375.0
#79 bev_abf    V3  dry                 111.0
#80 lcr_abv    V3  dry                  57.0
#81 ros_ros    V3  dry                  49.0
#82 spr_her    V3  dry                 205.0
#83 wkc_wok    V3  dry                  73.5
#84 blp_bel    V4  dry                 273.0
#85 brsf_co    V5  dry                 257.0
#86 bat_bhr    V6  dry                 284.0
#87 bev_pri    V6  dry                  45.6
#88 che_buf    V6  dry                9788.0
#89 che_red    V6  dry               10254.0
#90 whi_ogl    V6  dry                2156.0
#91 whi_sta    V6  dry                1367.0
#92 hat_edg    V7  dry                 956.0
#93 hor_oel    V8  dry                 188.0
...
gage_out <- gage_clusters %>% 
  select(sta, group, type, contrib_drain_area_va) %>% 
  as.data.frame()
...
"bat_bhr" |
           sta == "bat_her") # fails in the 2d-clustering

gage_clusters2 <- gage_clusters %>% 
  filter(sta == "bat_her") # fails in the 2d-clustering
