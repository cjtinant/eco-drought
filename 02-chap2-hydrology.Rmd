...
rm(lambda_q1, lambda_q7, lambda_q30, pca_input_run_01, pca_matrix, pca_check)
```
...
rm(dups, dups_not, gage_fill_run_02, gage_pairs_02, sta_fill_02_lag, 
   sta_fill_02_lead, miss_contrib_area, sta_fill_02, fill_ck) 
```
...
pca_input_run_02 <- semi_join(gage_dv_run_02,  sta_run_02, by = "sta") %>% 
  filter(water_year == 1993|
           water_year == 1994|
           water_year == 1995|
           water_year == 1996|
...
rm(lambda_q1, lambda_q7, lambda_q30, sta_run_01, pca_input_run_02, 
   pca_matrix, lambda_vals_01, lambda_vals_02, pca_check, pca_eigen_run_01, 
   pca_eigen_run_02, pca_run_01, pca_run_02, pca_sum_run_01, pca_vars_run_01, 
   pca_vars_run_02) 
```

```{r prepare-pca_run_02} 

# This prepares missing data for the third run
# this run is 1993-1997 selected to find data for short-year station 'wkn_wok' 
...
# 2. identify nearest station pairs based on pc1 
sta_fill_03 <- anti_join(sta_run_02, sta_run_03) 

# 2.1 find lead and lag stations 
gage_pairs_03 <- pca_sum_run_02 %>% 
  mutate(sta_lead = lead(sta, 
                     order_by = PC1_mean)) %>%   # creates a lead col
  mutate(sta_lag = lag(sta, 
                     order_by = PC1_mean)) %>%   # creates a  lag col 
