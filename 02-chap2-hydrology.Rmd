...
#gage_est_9017 <- import(file = "data/gage_est_9017.csv")  # run02 
                                                    # n =  58,074 obs

# bind the gage dfs together, make min & max yr vars - clean up
gage <- bind_rows(gage, gage_est_9002) %>% 
  group_by(sta) %>%                        # orig  n = 249,247       
  mutate(min_yr = min(water_year)) %>%     # run02 n = 264,415 
...
gage_summary <- gage %>%                          #  orig n = 689 obs
  group_by(sta, water_year) %>%                   # n run01 = 728 obs
  summarise(days_record = n()) %>%                  
  ungroup()             
...
gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>%  
  filter(days_record >= 365)                      # run01 n = 245,814     
                                                  # run02 n = 262,979   
                                                  # run03 n = 
                                                  # run04 n =   
...
gage_run02_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% #        32 sta 
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                 # n =     896 obs

gage_run02 <- semi_join(gage, gage_run02_sum, 
                        by = c("sta", "water_year")) # n = 262,979 obs
...
gage_input <- gage_run02                       # run02 = 262,979  obs
...
lambda_q1  <- BoxCox.lambda(pca_input$q1_depth)  # q1  => [0.01,-0.08]  
lambda_q7  <- BoxCox.lambda(pca_input$q7_depth)  # q7  => [0.11,-0.13]  
lambda_q30 <- BoxCox.lambda(pca_input$q30_depth) # q30 => [0.21,-0.16]    
...
#  01   1990   2002 111,031    0.0135     0.112      0.264 
#  02   1990   2017 262,979   -0.0842    -0.125     -0.159

# run01    "bat_bhr" "bat_her" "bev_abf" "bev_buf" "bev_pri" "blp_bel"
#          "brsf_co" "fal_hot" "frn_fai" "hat_edg" "hor_oel" "lcr_abv"
#          "lcr_bel" "lcr_vet" "lwr_abv" "lwr_mar" "lwr_ros" "lwr_whi"
#          "rap_far" "ros_ros" "spr_her" "wcc_ogl" "whi_kad" "whi_ogl"
#          "whi_slm" "whi_sta" "wkc_wok"

# run02    "bat_bhr" "bat_her" "bev_abf" "bev_buf" "bev_pri" "blp_bel"
#          "brsf_co" "fal_hot" "frn_fai" "hat_edg" "hor_oel" "lcr_abv" 
#          "lcr_bel" "lcr_vet" "lwr_abv" "lwr_mar" "lwr_ros" "lwr_whi" 
#          "rap_far" "ros_ros" "spr_her" "wcc_ogl" "whi_kad" "whi_ogl" 
#          "whi_slm" "whi_sta" "wkc_wok"
#          "che_buf" "che_red" "che_sce" "whi_int"  "whi_whi"  

# 
pca_eigen 
# num   PC1perc PC2perc PC1cum PC2cum 
#  01    0.944   0.049  0.944   0.993     
#  02    0.960   0.035  0.960   0.995   
...
#  01   EEV    132.66     27  30    166.44   165.52
#  02   VEV    150.28     32  19    234.71   234.68 

# Run 1 clustering table:
#  cluster val:   1        2        3        4
# num members:    6        5       13        3
# cluster type: mixed   precip   gw-dom   ephemeral 
...
```{r fill_missing_data_wet_years_run01old, eval=FALSE} 
...
gage_part_miss <- gage_run02_sum %>% 
  filter(days_record < 365) %>% 
    spread(water_year, days_record)                 # n = 14 x 29 obs

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 188 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  rename(sta_orig = sta)
...
gage_pairs <- gage_clust %>% 
  ...
  rename(group_sta = group) %>% 
  rename(sta_orig = sta) %>% 
  select(group_sta, sta_orig, sta_lead, sta_lag)     # n = 32 obs
...
gage_pairs <- inner_join(group_lead_id, gage_pairs, 
                         by = "sta_lead") %>%              
  filter(group_sta == group_lead) %>% # drops obs of different groups
  select(group_sta, group_lead, sta_orig, sta_lead, sta_lag) # 31 obs
...
# 7. join the missing water-year pairs 
gage_part_miss <- inner_join(gage_part_miss, gage_pairs,
                        by = c("sta_orig", "group"))  # n = 376 pairs 
...
gage_est_miss <- inner_join(gage, gage_part_miss, 
  ...
  summarise(                                   
    ...
    q30        = mean(q30), 
    water_year = mean(water_year),
    qualifier  = first(qualifier)              # n = 12,052 obs 
  ) %>% 
  ungroup() %>% 
  rename(sta = sta_orig)
...
gage_dups <- semi_join(gage_est_9002, gage_test) %>% 
  filter(qualifier != "est")                           # 1,850 obs

gage_est_9002a <- anti_join(gage_est_9002, gage_dups, 
                            by = c("date", "sta"))    # n = 11,014
...
```{r fill_missing_data_all_years_run02old, eval=FALSE} 
...
gage_test <- gage_est_miss %>%  
  group_by(sta, water_year) %>% 
  summarize(count = n()) %>% 
  ungroup() %>% 
  filter(count > 366)  # 0 cases
...
gage_test3 <- bind_rows(gage, gage_est_miss)

gage_test3 <- gage_test3 %>%  
  group_by(sta, water_year) %>% 
  summarize(count = n()) %>% 
  ungroup() %>% 
  filter(count > 366)  
...
rm(gage_est_miss, gage_pairs, gage_part_miss, gage_run02_incomp, 
   gage_run02_map, gage_run02_sum, group_lag_id, group_lead_id, 
   gage_test, gage_test1, gage_test2, gage_test3)
