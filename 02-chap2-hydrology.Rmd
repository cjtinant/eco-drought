...
# Drop Cheyenne River at Angustora with ~50% NA values --------------
...
# Estimate or remove missing values in the data ---------------------
...
# Standardize the data (i.e. scale or z-score) so columns have mean 
...
gage_scale <- gage_raw %>% 
  ...
  select(sta, q1_depth, q7_depth, q30_depth, date) %>% 
  mutate(q1_mean = mean(q1_depth, na.rm = TRUE)) %>% 
  mutate(q1_sd = sd(q1_depth, na.rm = TRUE)) %>% 
  mutate(q1_scaled = (q1_depth - q1_mean)/q1_sd) %>% 
  mutate(q7_mean = mean(q7_depth, na.rm = TRUE)) %>% 
  mutate(q7_sd = sd(q7_depth, na.rm = TRUE)) %>% 
  mutate(q7_scaled = (q7_depth - q7_mean)/q7_sd) %>% 
  mutate(q30_mean = mean(q30_depth, na.rm = TRUE)) %>% 
  mutate(q30_sd = sd(q30_depth, na.rm = TRUE)) %>% 
...
# date       <chr> "1991-10-01", "1991-10-02", "1991-10-03", "199... 
# q1_depth   <dbl> 14.704930, 14.704930, 16.700614, 18.696298, 17... 
# q7_depth   <dbl> 16.93820, 16.46303, 16.32048, 16.46303, 16.463... 
# q30_depth  <dbl> 15.76929, 15.81364, 15.93560, 16.12408, 16.268... 
# q1_scaled  <dbl> -0.3585591, -0.3585591, -0.3508945, -0.3432298... 
# q7_scaled  <dbl> -0.4316930, -0.4339446, -0.4346201, -0.4339446... 
# q30_scaled <dbl> -0.5543399, -0.5540731, -0.5533394, -0.5522055... 
```
