...
gage <- gage %>%                                  # n = 269,932 
  ...
  filter(sta != "nio_abb") %>% 
  filter(sta != "sna_dou") %>% 
  filter(sta != "cas_hot") %>%          # dropped because so unusual!
  filter(sta != "sna_bur")              # dropped because so unusual!
...
gage_run02 <- gage_run02 %>%  
  filter(water_year <= 1997) %>%                    # n = 48,202
  filter(water_year >= 1993)        # blp_bel & wkn_wok have na in 92
...
# run01 prior:dropped cascade springs & sna_bur - outliers w/
...
ggplot(gage_pca, aes(PC1_mean, PC2_mean)) + 
#  scale_y_reverse() +
  geom_jitter() + 
...
gage_pca <- full_join(gage_pca, gage_clust, 
                             by = "sta")
gage_clust <- gage_clust %>% 
...
rm(gage_clust_input, gage_clust_input_meta, gage_clust)
```
...
ggplot(gage_pca, 
       aes(q7_mean, q1_q30_mean, color = as.factor(group))) + 
  geom_density2d(na.rm = FALSE, contour = TRUE, aes(color = as.factor(group)))  + 
  geom_jitter() + 
  geom_text(aes(label = sta, color = "blk"), check_overlap = TRUE, 
              nudge_y = 0.02) + 
