...
gage_run03 <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(-c('1990':'2007', '2016', '2017')) %>%
  filter(!is.na(`2008`)) %>%  # 26 sta w/ complete recs [2008, 2015]
  select(sta)               

gage_run03 <- inner_join(gage, gage_run03, by = "sta") %>%  
  filter(water_year <= 1997) %>%              ##        
  filter(water_year >= 1993)                  #        n = 36,885

# update summary to show only stations with data in 2008 
gage_run03_sum <- gage_summary %>% 
  ...
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                  # n = 208 obs

gage_run03_incomp <- semi_join(gage_incomp, gage_run03_sum, 
                  by = c("sta", "water_year")) %>%  # n = 336 obs
  filter(!is.na(date)) 
...
#  03   2008   2015  36,885    0.0102     0.0895     0.1684
...
#             "whi_kad", "whi_ogl", "whi_sta"
#  new:       "che_buf", "che_red", "che_sce","whi_int", "whi_whi"
pca_eigen
# num   PC1perc PC2perc PC1cum PC2cum 
#  01    0.952   0.043  0.950   0.993     
#  02    0.939   0.052  0.939   0.990     
#  03    0.941   0.492  0.941   0.991
...
#   03  1    0.578      0.588     0.566    
#   03  2    0.572      0.203    -0.795   

# eigenvecter:
#   45% of covarience is explained by PCA1 & 5% of varience by PCA2
# variables:
...
#  03   EEV    141.1276   21  44  141.1276 141.1248
