...
pca_eigen_vals <- bind_rows(pca_eigen_vals, pca_eigen)
pca_vars_vals <- bind_rows(pca_vars_vals, pca_vars) 
pca_run_vals <- bind_rows(pca_run_vals, pca_input) 
...
# 1.1 add lead PCA information to summary table 
gage_pairs.01 <- left_join(gage_pairs, gage_pairs, 
                           by = c("sta" = "sta_lead")) %>% 
  select(-c(sta.y, sta_lag.y)) %>% 
  rename(PC1_mean.sta = PC1_mean.x) %>% 
  rename(PC2_mean.sta = PC2_mean.x) %>% 
...
# 1.2 add lag PCA information to summary table & clean up 
gage_pairs <- left_join(gage_pairs.01, gage_pairs, 
                           by = c("sta" = "sta_lag")) %>% 
  rename(sta_lead = sta_lead.x) %>% 
  select(-c(sta.y, sta_lead.y)) %>% 
  rename(PC1_mean.lag = PC1_mean) %>% 
