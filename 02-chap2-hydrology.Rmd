...
# remove r = 1.00 from dataframe, remove "spi_" & create "to-from" var
spi_corr <- spi_corr %>% 
  filter(pears_r < .99) %>% 
...
spi_corr_int1 <- spi_corr %>% 
  filter(sta == "oel" |
         sta == "rap") 

spi_corr_int2 <- spi_corr %>% 
  filter(sta == "ora" & 
           sta2 != "cot") %>% 
    filter(sta == "ora" & 
           sta2 != "int")

spi_corr <- bind_rows(spi_corr_int1, spi_corr_int2)
...
spi_corr <- spi_corr %>% 
  select(-to_lat, -to_lon, -from_lat, -from_lon, -northing, -easting)
...
rm(spi_corr_int1, spi_corr_int2, lat_to_km, lon_to_km)
...
# flatten the list to a dataframe, select values, 
# arrange into a long format, create a distance descriptor variable
#spi_corr <- flatten_dfc(corr_list) %>% 
# select(sta, sta2, starts_with("pears_r"), 
#        starts_with("spi_length")) %>% 
#  gather(key, spi_length, -sta, -sta2, -starts_with("pears")) %>%
#  select(-key) %>% 
#  gather(key, pears_r, -spi_length, -sta, -sta2) %>% 
#  select(-key) %>% 
#  group_by(sta2) %>%
#  mutate(sta3 = str_remove_all(sta2, "spi_")) %>% 
#  ungroup() %>% 
#  select(-sta2) %>% 
#  mutate(stations = str_c(sta, sta3, sep = "-")) %>% 
#  filter(pears_r < .99) %>% 
#  distinct()
```

```{r munge-joined-SCI-values}

```

```{r SCI-diagnostic-plots}
...
```{r other-plots}
# Next steps: spread or extract the spi_corr 
# set large values of SPI to the probability of the spi value 
# or max of longest station...
