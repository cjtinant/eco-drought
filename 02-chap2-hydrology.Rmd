...
_raw         imported data of monthly precip vals
...
sta_int <- full_join(sta_int, abbrev, by = "sta")
...
sta_int <- full_join(sta_int, sta_mean, by = "yr_mon") 
rm(sta_mean)
...
ggplot(sta_int, aes(as.factor(sta), depth)) + 
  geom_violin() + 
  geom_boxplot() + 
  scale_y_sqrt() + 
  theme_bw() + 
...
# plot the precip data as a monthly boxplot
ggplot(sta_int, aes(sta_abrev, depth)) + 
  geom_boxplot() +
#  geom_histogram(binwidth = 1) +
  facet_grid(cols = vars(month))

# plot the precip deviations as a monthly boxplot
ggplot(sta_int, aes(sta_abrev, deviation)) + 
  geom_boxplot() +
#  geom_histogram(binwidth = 1) +
  facet_grid(cols = vars(month))
...
# of zero events and n is the sample size. The resulting mixed 
# distribution used fro SCI transformation 
# is then: 
...
# Uncertainty in distribution parameters can cause unrealistically 
# large or small SCI values if values in x exceed the values used for 
# parameter estimation (see fitSCI). Therefore transformSCI allows for 
# a truncation of the SCI series such that abs(sci) <= sci.limit. The 
# truncation can be disabled by setting sci.limit = Inf.

# fitSCI returns an object of class "fitSCI" 
...
#Arguments for the SCI function
# x	- numeric vector, representing a monthly univariate time series.
...
# down. This stabilizes the implementation but can introduce biases in 
# the resulting SCI.
# DON'T NEED THIS BECAUSE MLE IS WORKING FOR THE DATA

# obj	- an object of class fitSCI, output from fitSCI.

# sci.limit	- Truncate absolute values of SCI that are larger than 
# sci.limit. 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
flag_int <- as.tibble(spi.int.l$dist.para.flag) # M2 flag => 3
...
# correlation plot
corrplot.mixed(spi_M,  order = "hclust", addrect = 2, upper = "ellipse", lower = "number", title = "One month SPI correlation among stations")

ggplot2::ggsave(filename = "figure/spi_corr_1mo.png", 
                width = 6, height = 6, units = "in")
