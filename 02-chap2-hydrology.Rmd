...
gage_9016 <- import("data/gage_meta_9016.csv") %>% 
  select(1:4) 
gage_9398 <- import("data/gage_meta_9398.csv") %>% 
  select(1:4)

#  combine records and prepare to look for missing years 
gage <- bind_rows(gage_9016, gage_9398) %>% 
  distinct() %>% 
...
# inital step - run 92-97 PCA with lcr_abv by dropping 1996 
#gage_yr_miss <- gage_yr_miss %>% 
#  mutate(
#    completeness = case_when(
#      sta == "lcr_abv" ~ "1 - drop96", 
#      TRUE ~ .$completeness
#    )
#) 
```

```{r PCA_exploratory_analysis -refactored}
...
# station_1991_1992_1998_neighbor
# bev_abf   y    y    y   frn_fai
# bev_pri   y    y    n   bev_buf
# wkc_wok   y    n    y   bev_buf
# blp_bel   y    n    n   brsf_co
# whi_slm   n    n    y   whi_ogl
# ros_ros   n    n    y   lwr_abv
...
```{r PCA_exploratory_analysis -refactored-old}
# Overview---- 
# ~~~~~~~~ 
...
# Examines data from 1993-1997 - wet years with Wounded Knee station  
...
# clean up Global Environment----
pca_sum <- full_join(pca_meta, gage_sum, by = "sta")

gage_9398 <- left_join(gage_aug, pca_input, 
                           by = c("sta", "date")) 

# note:looks like gage_x_raw has same info as gage_x_aug
rm(lambda_q1, lambda_q7, lambda_q30, pca_eigen, pca_meta, pca_vars, gage_aug, pca_input, pca_matrix, gage_sum, input_chk)
```
...
ggplot(gage_9398_aug, aes(q7_depth, q1_q30_diff)) +
  geom_jitter(aes(color = sta, shape = ".")) + 
  geom_density2d(aes(color = NULL)) + 
  geom_point(data = gage_9398_sum, mapping = aes(q7_mean, q1_q30_mean)) + 
  geom_text(data = gage_9398_sum, 
...
gage_clust_9398_input <- gage_9398_sum %>% 
  select(q1_mean:q30_mean)

gage_clust_9398_input_meta <- gage_9398_sum %>% 
  select(sta)

# apply & summarize model 
gage_clust_l <- Mclust(gage_clust_9398_input) # Model-based-clustering 
...
gage_clust <- bind_cols(gage_clust_9398_input_meta, gage_clust) 
...
rm(gage_clust_l, gage_clust, mc_9398_input, mc_9398_input_meta)
```
...
test <- left_join(gage_9398_sum, gage_clust)
