...
abbrev       abbreviation look-up table 
...
_mean        intermediate variable; mean monthly value
_sum         summary data 
...
_M          Matrix of correlation 
_check      Temporary variable used to check data

Variables & constants used by SCI function
...
flag_spi     records any flags from fitSPI or transformSPI

# NOT YET MAPPED Varibles
# p_zero_cm <- TRUE # ?????
# _mon         precipitation depths w/ month as variable
...
rm(spi, spi_M, flag_spi, spi_att_check)
rm(spi_att, spi_gath)
```
...
rm(att_names, att_cot, att_int, att_oel, att_ora, att_rap) 
rm(spi.cot.l, spi.int.l, spi.oel.l, spi.ora.l, spi.rap.l) 
rm(spi, spi_M, flag_spi, spi_att_check)
rm(spi_att, spi_gath)
```
...
```{r join_spi} 
# NOTE this is not yet more efficient... pmap might be the way...
...
spi_att <- flatten_dfc(att_list) %>% 
  select(sta, spi_length, property, spi1, spi2, spi3, spi4, spi5, spi6, 
...
# prepare for join- differenciate values 
spi_check1 <- mutate(spi_check1, spi1 = value) 
spi_check2 <- mutate(spi_check2, spi2 = value) 
spi_check3 <- mutate(spi_check3, spi3 = value) 
spi_check4 <- mutate(spi_check4, spi4 = value) 
spi_check5 <- mutate(spi_check5, spi5 = value) 
spi_check6 <- mutate(spi_check6, spi6 = value) 
spi_check9 <- mutate(spi_check9, spi9 = value) 
spi_check12 <- mutate(spi_check12, spi12 = value) 
spi_check18 <- mutate(spi_check18, spi18 = value) 
spi_check24 <- mutate(spi_check24, spi24 = value) 

# create a list of the spi_check dataframes
check_list <- list(spi_check1, spi_check2, spi_check3, 
                            spi_check4, spi_check5, spi_check6, 
                            spi_check9, spi_check12, spi_check18, 
                            spi_check24)

# flatten the list to a dataframe & select values
spi_check <- flatten_dfc(check_list) %>% 
  select(sta, spi_length, property, spi1, spi2, spi3, spi4, spi5, spi6, 
         spi9, spi12, spi18, spi24) %>% 
  gather(key = length, value = spi_value, -spi_length, 
         -property, -sta) %>% 
...
# create a list of the spi_att dataframes
corr_list <- list(spi_M1, spi_M2, spi_M3, 
                            spi_M4, spi_M5, spi_M6, 
                            spi_M9, spi_M12, spi_M18, 
                            spi_M24)

# flatten the list to a dataframe & select values
spi_corr <- flatten_dfc(corr_list) %>% 
 select(sta, sta2, starts_with("pears_r"), 
        starts_with("spi_length")) %>% 
    gather(key, value, -sta, -sta2) %>% 
    extract(key, "pears_r")
...
rm(check_list, gath_list, corr_list)
```
...
# correlation plot
# ~~~~~~~~~~~~~~~~
corrplot.mixed(spi_M1,  order = "hclust", addrect = 2, 
               upper = "ellipse", lower = "number", 
               title = "One month SPI correlation among stations") 

# don't know how to save the results of this! - SEE ggcorrelation?

# general attributes plot 
# ~~~~~~~~~~~~~~~~~~~~~~~
ggplot(spi_att, aes(as.factor(month), value)) + 
  geom_boxplot() +
  facet_wrap(ncol = 1, scales = "free_y", vars(property)) + 
  theme_classic() 

ggplot2::ggsave(filename = "figure/spi_att_1mo.png", 
                width = 6, height = 6, units = "in")

# result => shape parameter is interesting!
# check it out in greater detail

# attributes plot for skew 
# ~~~~~~~~~~~~~~~~~~~~~~~~


ggplot(spi_att_skew, aes(as.factor(sta), value)) + 
  geom_boxplot() + 
  theme_classic()
# Oral has a low skew in April... 

ggplot2::ggsave(filename = "figure/spi_skew_1mo.png", 
                width = 6, height = 6, units = "in") 

# SPI index values plot 
# ~~~~~~~~~~~~~~~~~~~~~
ggplot(spi_gath, aes(date, spi_index)) +
  geom_line() +
  facet_wrap(vars(sta)) + 
  theme_classic()   
...
# correlation plot
# ~~~~~~~~~~~~~~~~
corrplot.mixed(spi_M,  order = "hclust", addrect = 2, 
               upper = "ellipse", lower = "number", 
               title = "One month SPI correlation among stations") 
...
#ggplot2::ggsave(filename = "figure/spi_att_2mo.png", 
#                width = 6, height = 6, units = "in")
...
ggplot(spi_att_skew, aes(as.factor(sta), value)) + 
  geom_boxplot() + 
  theme_classic()
# cottonwoods then interior have high overall skew

#ggplot2::ggsave(filename = "figure/spi_skew_2mo.png", 
 #               width = 6, height = 6, units = "in") 
...
#ggplot2::ggsave(filename = "figure/spi_2mo.png", 
#                width = 6, height = 6, units = "in") 
...
#ggplot2::ggsave(filename = "figure/spi_att_3mo.png", 
#                width = 6, height = 6, units = "in")
...
#ggplot2::ggsave(filename = "figure/spi_skew_3mo.png", 
 #               width = 6, height = 6, units = "in") 
...
#ggplot2::ggsave(filename = "figure/spi_3mo.png", 
#                width = 6, height = 6, units = "in") 
...
#ggplot2::ggsave(filename = "figure/spi_att_4mo.png", 
#                width = 6, height = 6, units = "in")
...
ggplot(spi_att_skew, aes(as.factor(sta), value)) + 
  geom_boxplot() + 
  theme_classic()
# RAP & ORA have high skew OUTLIERS

#ggplot2::ggsave(filename = "figure/spi_skew_4mo.png", 
 #               width = 6, height = 6, units = "in") 
...
#ggplot2::ggsave(filename = "figure/spi_4mo.png", 
#                width = 6, height = 6, units = "in") 
...
# correlation plot
# ~~~~~~~~~~~~~~~~
corrplot.mixed(spi_M5,  order = "hclust", addrect = 2, 
               upper = "ellipse", lower = "number", 
               title = "One month SPI correlation among stations") 
...
# general attributes plot 
# ~~~~~~~~~~~~~~~~~~~~~~~
ggplot(spi_att5, aes(as.factor(month), value)) + 
  geom_boxplot() +
  facet_wrap(ncol = 1, scales = "free_y", vars(property)) + 
  theme_classic() 

#ggplot2::ggsave(filename = "figure/spi_att_5mo.png", 
#                width = 6, height = 6, units = "in")
...
ggplot(spi_att_skew, aes(as.factor(sta), value)) + 
  geom_boxplot() + 
  theme_classic()
# ORA haS high skew OUTLIER

#ggplot2::ggsave(filename = "figure/spi_skew_5mo.png", 
 #               width = 6, height = 6, units = "in") 
...
#ggplot2::ggsave(filename = "figure/spi_5mo.png", 
#                width = 6, height = 6, units = "in") 
...
# correlation plot
# ~~~~~~~~~~~~~~~~
corrplot.mixed(spi_M6,  order = "hclust", addrect = 2, 
               upper = "ellipse", lower = "number", 
               title = "One month SPI correlation among stations") 
...
# correlation plot
# ~~~~~~~~~~~~~~~~
corrplot.mixed(spi_M9,  order = "hclust", addrect = 2, 
               upper = "ellipse", lower = "number", 
               title = "One month SPI correlation among stations") 
...
# general attributes plot 
# ~~~~~~~~~~~~~~~~~~~~~~~
ggplot(spi_att9, aes(as.factor(month), value)) + 
  geom_boxplot() +
  facet_wrap(ncol = 1, scales = "free_y", vars(property)) + 
  theme_classic() 
...
# SPI index values plot 
# ~~~~~~~~~~~~~~~~~~~~~
ggplot(spi_gath9, aes(date, spi_index)) + 
  geom_line() +
  facet_wrap(vars(sta)) + 
  theme_classic() 
...
```{r spi-with-SCI-6mo-OLD} 
# This code chunk use the Standardized Climate Index (SCI) package
# to calculate SPI. Typical SCI are the Standardized Precipitation 
# Index (SPI), the Standardized Runoff Index (SRI) or the 
# Standardized Precipitation Evapotranspiration Index (SPEI).

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# Set SCI calculation averaging period
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
time_scale <- 6 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# create a summary
spi_summary <- spi_gath %>% 
  split(.$sta) %>%
  map(~ summary(.$spi_index)) 

# NEXT STEP TRANSPOSE THIS LIST
...
corrplot.mixed(spi_M,  order = "hclust", addrect = 2, upper = "ellipse", lower = "number", title = "Precipitation station correlation")

# NEXT STEP - SAVE THIS PLOT

# get the Pearson Type-III properties~~~~~~
# get names
att_names <- spi.cot.l$dist.para 
att_names <- rownames(att_names) 
att_names <- as_data_frame(att_names) %>% 
...
# join the rows & prepare for gather
spi_att <- bind_rows(att_cot, att_int, att_oel, att_ora, att_rap) %>% 
  rename("1" = M1) %>% 
  rename("2" = M2) %>% 
  rename("3" = M3) %>% 
...
rm(att_names, att_cot, att_int, att_oel, att_ora, att_rap) 
rm(spi.cot.l, spi.int.l, spi.oel.l, spi.ora.l, spi.rap.l) 
rm(spi, spi_M, flag_spi, spi_att)

spi_att6     <- spi_att 
spi_gath6    <- spi_gath
spi_summary6 <- spi_summary 

rm(spi_att, spi_gath, spi_summary)
```
...
#ggplot(spi_1mon_gath, aes(month, value, group = month)) + 
#  geom_boxplot() + 
#  facet_wrap(~station) + 
#  theme_classic() +  
...
ggscatter(pca_summary, x = "PC1_mean", y = "PC2_mean") + 
geom_density2d() + # Add 2D density 
    geom_text(label = pca_summary$sta) 

ggscatter(pca_au, x = ".fittedPC1", y = ".fittedPC2", 
          color = "sta", mean.point = TRUE) + 
            geom_density2d()  


ggplot() +
  geom_point(pca_summary, aes(x = "PC1_mean", y = "PC2_mean"))
...
# The plot suggests at least 3 clusters in the mixture. 
#   Interior is more negative on PCA1 & PCA2 & more positive on PCA3
#   Cottonwood is more positive on PCA1 & more positive on PCA3 
...
```{r spi-with-SCI-CAN_DROP?, eval = FALSE}
...
set.seed(101)
n.years <- 60
date <- rep(1:n.years,each = 12) + 1950 + 
...
## display test data
#matplot(date, cbind(PRECIP ,PET), t = c("h", "l"), 
#        col = c("blue", "red"), lty = 1)
#legend("topright", legend = c("PRECIPitation","temperature"), 
#       fill = c("blue","red")) # won't work in Markdown w/o 
##
## example SPI
##
spi.para <- fitSCI(PRECIP, first.mon = 1, distr = "gamma", 
                   time.scale = 6, p0 = TRUE)
...
##
spi.1.para <- fitSCI(PRECIP, first.mon = 1, time.scale = 1, 
                      distr = "gamma", p0 = TRUE)
spi.12.para <- fitSCI(PRECIP, first.mon = 1,
                      time.scale = 12,distr = "gamma", p0 = TRUE)
spi.1 <- transformSCI(PRECIP, first.mon = 1,obj = spi.1.para)
spi.12 <- transformSCI(PRECIP, first.mon = 1,obj = spi.12.para)

#matplot(date, cbind(spi.1, spi.12), t = "l", 
#        lty = 1,col = c("red", "blue"), lwd = c(1, 2))
#legend("topright",legend = c("time.scale = 1","time.scale = 12"), 
#       fill = c("red","blue"))
...
spi.ref.para <- fitSCI(PRECIP[sel.date], first.mon = 1, 
                       distr = "gamma", time.scale = 6, p0 = TRUE)
...
##
## use "start.fun.fix" in instances where maximum likelyhood 
#     estimation fails
