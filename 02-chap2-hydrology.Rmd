...
sta_mon <- sta_mon %>% 
  ...
  mutate(depth = replace(depth, depth == 0.0, 0.01)) %>%
...
```{r SPI-3month}
# clean up
rm(spi_2mon, spi_2mon_grp)

# Arrange data and calculate SPI
sta_mon <- sta_mon %>% 
  arrange(date)

spi_3mon_list <- sta_mon %>% 
  select(-(date:month)) %>%
  spi(distribution = 'PearsonIII', scale = 1, na.rm = TRUE) 

# Look at the list 
# jsonedit(spi_3mon, mode = "view", elementId = "call")

# extract the fitted data
spi_3mon <- as.data.frame(spi_3mon_list["fitted"]) # this works.  
# But, not sure how to do do the same in a Tidyverse way.  Moving fwd.
...
spi_3mon <- bind_cols(dates, spi_3mon)
rm(spi_2mon_list, dates)

spi_3mon_grp <- spi_3mon %>%
  gather(key = "station", value = "value", -date, -year, -month) %>%
  drop_na(value)
```

```{r ggplot_3month_SPI}
ggplot(spi_3mon_grp, aes(date, value)) +
  geom_line() +
  facet_grid(station~.) +
  theme_classic() + 
  labs(title = "Standardized Precipitation Index (SPI)",
       subtitle = "Three-month duration; Pine Ridge Reservation, SD") +
       xlab("") +
       ylab("SPI-value")
```
...
```{r SPI-4month}
# clean up
rm(spi_3mon, spi_3mon_grp)

# Arrange data and calculate SPI
sta_mon <- sta_mon %>% 
  arrange(date)

spi_4mon_list <- sta_mon %>% 
  select(-(date:month)) %>%
  spi(distribution = 'PearsonIII', scale = 1, na.rm = TRUE) 
...
# extract the fitted data
spi_4mon <- as.data.frame(spi_4mon_list["fitted"]) # this works.  
# But, not sure how to do do the same in a Tidyverse way.  Moving fwd.
...
spi_4mon <- bind_cols(dates, spi_4mon)
rm(spi_4mon_list, dates)

spi_4mon_grp <- spi_4mon %>%
  gather(key = "station", value = "value", -date, -year, -month) %>%
  drop_na(value)
```
