...
# drop incomplete years and noncharacteristic stations - update
gage <- gage %>%                                  # n = 269,932 
  filter(sta != "bad_mid") %>% 
  filter(sta != "che_ang") %>% 
  filter(sta != "nio_abb") %>% 
...
# add metadata to gage data - DO LATER!
...
# run01 outcome - dropped cas_hot, sna_bur
...
gage_run02 <- gage_run02 %>%  
  filter(water_year <= 1997) %>%              #       n = 48,202 
  filter(water_year >= 1993)                  # later n = 51,128
...
#  02   1993   1997  48,202    0.0053     0.0102     0.1950
...
#             "bev_pri", "blp_bel", "brsf_co", "che_was", "fal_hot", 
#             "frn_fai", "hat_edg", "hor_oel", "lcr_bel", "lcr_vet", 
#             "lwr_abv", "lwr_mar", "lwr_ros", "lwr_whi", "rap_far",  
#             "ros_ros", "spr_her", "wcc_ogl", "whi_kad", "whi_ogl", 
#             "whi_slm", "whi_sta", "wkc_wok"

# num   PC1perc PC2perc PC1cum PC2cum
#  01    0.952   0.043  0.950   0.993     
#  02    0.938   0.517  0.938   0.990     

# run  PC  q1_depth  q7_depth  q30_depth      
#   01  1    0.577      0.588     0.568 
#   01  2   -0.597     -0.171     0.784  
#   02  1    0.577      0.589     0.566   
#   02  2    0.592      0.176    -0.786 
...
gage_clust_input <- gage_pca %>% 
  select(q1_mean:q30_mean)

gage_clust_input_meta <- gage_pca %>% 
  select(sta)
...
gage_clust <- full_join(gage_pca, gage_clust, 
                             by = "sta")
...
# clean up environment
rm(gage_clust_input, gage_clust_input_meta, gage_pca)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# run02 Mclust VEV (ellipsoidal, equal shape) model with 5 components:  

# num  model  log.likeli  n   df     BIC      ICL
#  01   EEV    147.8495   22  51  138.0558 138.0555
#  02   VEV    155.2134   28  41  173.8064 173.7889
...
# prepare to estimate and fill missing data----

gage_run02 <- gage_run02 %>% 
  select(sta, everything())

gage_aug <- full_join(gage_aug, gage_clust)
...
# find group average daily depth of flow----

gage_nested <- gage_aug %>% 
   group_by(date, group) %>% 
   nest() 


# use the split assign etc.
gage_V1   <- gage %>% 
  filter(group == "V1") %>%
  group_by(date) %>% 
  summarise(q          = mean(q),
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# goal of run02 is to find groups for: 
# bev_abf -> frn_fai, V4
# bev_pri -> bev_buf, V2 
# blp_bel -> whi_kad, V1
# rap_far -> ros_ros, V4
# ros_ros -> fal_hot, V4 
# whi_slm -> (whi_ogl + whi_sta)/2, V3
# whi_sta -> whi_slm, V3
# wkc_wok -> bat_bhr, V2
...
gage      <- gage %>% 
  select(sta, date, q, q7, q30, log_q, everything()) 
...
# goal of run01 is to find neighbor for 
# ros_ros -> fal_hot
