...
# eigenvectors (incidentally, this is called spectral theorem); # corresponding eigenvalues will then be located on the diagonal. 
...
#   Scale the data, a.k.a calculate z-scores so columns have a zero 
#   mean and standard deviation of unity. 

# 5. Estimate or remove missing values in the data 
...
# Eigenvectors
gage_pc_eigen <-  tidy(gage_pc, matrix = "pcs") 
#gage_pc_eigen
...
# PCA variables
gage_pc_vars <-  tidy(gage_pc, matrix = "variables") 
gage_pc_vars <- gage_pc_vars %>% 
  filter(PC != 3) %>% 
  rename(var = column) %>% 
...
gage_au <- augment(gage_pc, data = gage_matrix) 
gage_au <- bind_cols(gage_matrix_name, gage_au)
rm(gage_matrix_name)
...
gage_eigen <- gage_au %>% 
  ...
  summarize(PC1_mean = mean(.fittedPC1),
            ...
            q1_depth = mean(log10(q1_depth)),
            q30_depth = mean(log10(q30_depth))
            ) %>% 
  mutate(ratio_q1_q30 = q1_depth - q30_depth) %>% 
  mutate(ratio_q30_q1 = q30_depth - q1_depth) %>% 
  ungroup()

ggplot(gage_eigen, aes(PC1_mean, PC2_mean)) +
  geom_point() +
...
gage_check1$sta <- as.factor(gage_check1$sta)
gage_check1$sta <- factor(gage_check1$sta, 
...
plot_gage_pca <- ggplot(gage_check1, aes(sta, q7_depth)) +
  geom_boxplot() +
  scale_y_log10() + 
  theme_classic() +
  xlab("") +
...
gage_matrix_small_name <- gage_scale %>% 
      filter(sta == "hor_oel" | 
           sta == "wcc_ogl" |
           sta == "blp_bel" |
           sta == "lwr_mar" |
...
gage_pc_small <- prcomp(gage_matrix_small, scale = TRUE)

gage_small_au <- augment(gage_pc_small, data = gage_matrix_small) 
gage_small_au <- bind_cols(gage_matrix_small_name, gage_small_au)
rm(gage_matrix_small_name)

gage_eigen_small <- gage_eigen %>% 
      filter(sta == "hor_oel" | 
           sta == "wcc_ogl" |
           sta == "blp_bel" |
           sta == "lwr_mar" |
...
ggplot() +
  geom_point(data = gage_small_au, 
             mapping = aes(x = .fittedPC1, y = .fittedPC2, 
                           color = factor(sta))) +
  geom_point(data = gage_eigen_small, 
             mapping = aes(PC1_mean, PC2_mean)) + 
    geom_text(data = gage_eigen_small,
...
# this would be nice but for the many stations
# fviz_pca_ind(prcomp(gage_pca), 
#             title = "PCA - Iris data", 
#             habillage = gage_scale$sta, 
#             palette = "jco", geom = "point", 
#             ggtheme = theme_classic(), 
#             legend = "bottom") 
