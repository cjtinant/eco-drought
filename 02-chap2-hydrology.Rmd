...
# checks for NA: gathers data, removes NA vals, & spreads data
sta_chk <- sta_raw %>% 
  gather(key = "sta", value = "depth", 
         -date, -year, -month) %>% 
  drop_na(depth)  %>% 
...
# create a matrix with SPI values - used below
spi.ora <- transformSCI(int, first.mon = first_mon_ora, 
                        obj = spi.ora.l) 
...
# create a matrix with SPI values - used below
spi.rap <- transformSCI(int, first.mon = first_mon_rap, 
                        obj = spi.rap.l) 
...
spi <- list(spi_cot, spi_int, spi_oel, spi_ora, spi_rap) %>% 
...
#spi_gath <- spi %>% 
#  select(-cot, -int, -oel, -ora, -rap) %>% # this removes
#  rename(cot = spi_cot) %>% 
#  rename(int = spi_int) %>% 
#  rename(oel = spi_oel) %>%  
#  rename(ora = spi_ora) %>%  
...
#test new way 
#test <- list(spi.cot.l, spi.int.l, spi.oel.l, 
#             spi.ora.l, spi.rap.l) 
#test2 <- test %>% 
#  flatten() 
#test3 <- pluck(test2, 1)
