...
# Method Description
# Calculate SPI with the SCI package.
# fitSCI identifies parameters for the Standardized Climate Index (SCI) 
...
spi.cot.l <- fitSCI(cot, first.mon = 1, distr = "pe3", 
                   time.scale = 6, p0 = TRUE)

spi.cot <- transformSCI(cot, first.mon = 1, obj = spi.cot.l) 
...
# calculate SPI as a list & extract results
spi.int.l <- fitSCI(int, first.mon = 1, distr = "pe3", 
                   time.scale = 6, p0 = TRUE)

spi.int <- transformSCI(int, first.mon = 1, obj = spi.int.l) 
...
spi.oel.l <- fitSCI(oel, first.mon = 1, distr = "pe3", 
                   time.scale = 6, p0 = TRUE, p0.center.mass = TRUE)

spi.oel <- transformSCI(oel, first.mon = 1, obj = spi.oel.l) 
...
# calculate SPI as a list & extract results
# It seems that the SCI package doesn't like snake_case variables
spi.ora.l <- fitSCI(ora, first.mon = 1, distr = "pe3", 
                   time.scale = 6, p0 = TRUE, p0.center.mass = TRUE)

spi.ora <- transformSCI(ora, first.mon = 1, obj = spi.ora.l) 
...
# calculate SPI as a list & extract results
# It seems that the SCI package doesn't like snake_case variables
spi.rap.l <- fitSCI(rap, first.mon = 1, distr = "pe3", 
                   time.scale = 6, p0 = TRUE, p0.center.mass = TRUE)

spi.rap <- transformSCI(rap, first.mon = 1, obj = spi.rap.l) 
spi_rap <- as.tibble(spi.rap) 
spi_rap <- spi_rap %>% 
  rename(spi_rap = value)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# combine the SPI variables & clean up

#NEED TO DO A JOIN WITH THE DATES!
#spi <- bind_cols(sta_raw, spi_cot, spi_int, spi_oel, spi_ora, spi_rap) 

rm(spi_cot, spi_int, spi_oel, spi_ora, spi_rap)
rm(sta_cot, sta_int, sta_oel, sta_ora, sta_rap) 
rm(spi.cot, spi.int, spi.oel, spi.ora, spi.rap)
rm(cot, int, oel, ora, rap)

# extract a summary as a list
spi_gath <- spi %>% 
  select(-(2:8)) %>% 
