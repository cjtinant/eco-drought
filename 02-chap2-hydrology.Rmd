...
gage <- gage %>%                                           
  ...
  filter(sta != "whi_cra")           # run02 - dropped for short rec.

# update gage summary & separate out incomplete years of record
gage_summary <- gage %>%                      # orig  n = 716 obs
...
gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>% # orig  n = 256,040 
...
gage_run02 <- gage_run02 %>%  
  filter(water_year <= 1997) %>%              ##       n = 49,302  
