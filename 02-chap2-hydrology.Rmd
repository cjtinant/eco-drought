...
# need to quit the alpha description 

# identify misclassified wet -> dry stations
gage_clust <- gage_clust %>%  
...
# taking the average is likely to smooth the data.  So, instead 
# representative stations are choosen based on complete years 
 
#    station type     q7_mean  q1_q30_mean   representitive
# 1 - EXTRA WET        0.694      0.0234       fal_hot 
...
# import gage data
gage <- import(file = "data/gage_inputs.csv")   # fix this to avoid call 
...
gage_summary <- left_join(gage_summary, gage_clust, 
                          by = "sta") %>% 
...
# plot convex hulls
  geom_polygon(data = gage_clust_hull, 
               aes(group = group,
                   fill = group)) + 
# add wet to dry arrows 
  geom_segment(data  = gage_summary, 
               arrow = arrow(length = unit(0.2, "cm"), 
                      type = "open"),     
               aes(x = PC1_wet, 
                   y = PC2_wet, 
...
#gage_clust <- gage_clust %>% 
#  select(sta, group, group_descr)
```

# SRI
```{r sri-with-SCI}   
# TO DO - SET UP CODE ABOVE TO AVOID CALLING GAGE AGAIN 
# consider best candidate for the cluster
