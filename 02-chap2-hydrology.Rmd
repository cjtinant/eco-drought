...
# check for complete days of record & years of record 
...
# remove the record of stations less than five years 
gage_dv <- gage_dv %>% 
  filter(sta != "nio_mar") 
...
gage_run_01 <- year_sum %>% 
  select(sta, yr_min:yr_max) %>% 
  drop_na() %>% 
  select(sta)  

pca_input <- semi_join(gage_dv,  gage_run_01, by = "sta") %>% 
  as.tibble() %>% 
  select(-c(contrib_drain_area_va, incomp_yr)) %>%
  filter(water_year <= as.integer(yr_min) & 
           water_year <= as.integer(yr_max))
...
pca_check <- pca_input_run_01 %>% 
  ...
  group_by(sta, water_year) %>%  
  summarise(count = n())      # note this throws a warning - it is ok 
...
pca_eigen_run_01 <- pca_eigen_run_01 %>% 
  mutate(name = "run_01") 
...
pca_vars_run_01 <- pca_vars_run_01 %>% 
  mutate(name = "run_01") 
...
# This prepares missing data --  for short-year station 'plu_hay'
# the PCA years are 1991-1995 & going to extend to 1988-1998

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 1.  prepare to join lead and lag stations using station pairs from last pca 
# create a summary table 
gage_pairs_01 <- pca_sum_run_01 %>% 
...
gage_pairs_01.01 <- left_join(gage_pairs_01, gage_pairs_01, 
                           by = c("sta" = "sta_lead")) %>% 
...
gage_pairs_01 <- left_join(gage_pairs_01.01, gage_pairs_01, 
                           by = c("sta" = "sta_lag")) %>% 
...
gage_comp_01_bkwd <- year_sum %>% 
  select(sta, '1988':'1990') %>% 
  drop_na() %>% 
  select(sta)   

gage_comp_01_fwd <- year_sum %>% 
  select(sta, '1996':'1998') %>% 
  drop_na() %>% 
  select(sta) 

# 3. find missing gages looking backward and forward & clean up
gage_miss_01_bkwd <- anti_join(gage_run_01, gage_comp_01_bkwd, 
                               by = "sta")  
gage_miss_01_fwd <- anti_join(gage_run_01, gage_comp_01_fwd, 
                              by = "sta") 

gage_miss_01 <- bind_rows(gage_miss_01_bkwd, gage_miss_01_fwd) %>% 
  distinct() 

rm(gage_comp_01_bkwd, gage_comp_01_fwd, gage_miss_01_bkwd, gage_miss_01_fwd)


# 4. select stations to fill the missing stations 
gage_miss_01 <- semi_join(gage_pairs_01, gage_miss_01, by = "sta") %>% 
...
gage_miss_01 <- gage_miss_01 %>% 
  select(sta, sta_fill) 

gage_fill_run_01 <- left_join(gage_dv, gage_miss_01, 
                                   by = c("sta" = "sta_fill")) %>% 
...
gage_fill_run_01 <- left_join(gage_fill_run_01, gage_contrib_area, 
                             by = "sta") %>% 
  mutate(contrib_drain_area_km = contrib_drain_area_va * 2.59) %>% 
...
# prepare to join 
gage_fill_run_01 <- gage_fill_run_01 %>% 
...
gage_fill_dups_01 <- semi_join(gage_dv, gage_miss_01, 
                                   by = ("sta")) %>% 
  mutate(sta_fill = "ZZ") %>% 
  filter(between(water_year, 1990, 2000)) 

# 7. remove duplicated stations
gage_fill_run_01 <- anti_join(gage_fill_run_01, gage_fill_dups_01, 
                  by = c("sta", "water_year")) %>% 
  select(-sta_fill)

gage_dv_run_01 <- bind_rows(gage_dv, gage_fill_run_01) %>% 
  distinct()
...
# 8. check results 
year_summary_01 <- gage_dv_run_01 %>%                          
  group_by(sta, water_year) %>%                  
  summarise(days_record = n()) %>%                   
  ungroup() 

year_sum_01 <- year_summary_01 %>% 
  spread(water_year, days_record) 

year_length_01 <- year_summary_01 %>% 
  group_by(sta) %>% 
  summarise(years_record = n()) %>%                   
  ungroup() 

# 9. update results
year_length <- full_join(year_length, year_length_01, 
                  by = "sta") %>% 
  rename(years_rec_orig = years_record.x) %>% 
  rename(years_rec_01 = years_record.y) %>% 
  arrange(years_rec_01) 

year_summary <- full_join(year_summary, year_summary_01, 
                  by = c("sta", "water_year")) %>% 
