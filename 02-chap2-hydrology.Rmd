...
#gage_est_9092 <- import(file = "data/gage_est_9092.csv")  #run02
                                               # n =   4,255 obs

#gage_est_9616 <- import(file = "data/gage_est_9616.csv") %>%   #run03
#  select(-contrib_drain_area_va)               # n =  15,068 obs

#gage_est_9002 <- import(file = "data/gage_est_9002.csv") %>%   #run04
#  select(-contrib_drain_area_va)               # n =   9,718 obs

# bind the gage dfs together, make min & max yr vars - clean up
gage <- bind_rows(gage_cont, gage_disc, 
                  gage_other) %>% 
  group_by(sta) %>%                        # orig  n = 286,723      
  mutate(min_yr = min(water_year)) %>%     # run02 n = 
  mutate(max_yr = max(water_year)) %>%     # run03 n =   
           ungroup() %>%                   # run04 n = 
  distinct() %>% 
  filter(!is.na(q30))   # drops some NA vals from start of record 

rm(gage_cont, gage_disc, gage_other, gage_est_9092, gage_est_9616)      

# test the data for duplicates
gage_test <- gage %>%  
...
gage_summary <- gage %>%                               # N = 44 sta
  ...
  filter(
    ...
    sta != "whi_cra" &         # run01 - dropped for short rec.
    sta != "che_was" &         # run01 - dropped for >> area    
    sta != "cas_hot" & 
    sta != "sna_bur" & 
    sta != "sna_dou")            
 
# remove stations with < 5 yr records - clean up 
gage <- semi_join(gage, gage_summary, by = "sta") # run01 n = 259,474 
                                                  # run02 n = 
                                                  # run03 n = 
                                                  # run04 n =  

# update gage summary 
gage_summary <- gage %>%                      # orig  n = 717 obs
  group_by(sta, water_year) %>%               # run02 n =  
  summarise(days_record = n()) %>%            # run03 n =      
  ungroup()                                   # run04 n = 

# find & split stations-years with incomplete days of record 
gage_incomp <- gage %>%                       # orig  n = 16 obs 
  group_by(sta, water_year) %>%               # run02 n = 12 obs
  summarise(days_record = n()) %>%            # nun03 n =  8 obs
  ungroup() %>%                               # run04 n =  6 obs    
  filter(days_record < 365)                           

gage_incomp <- semi_join(gage, gage_incomp, 
                         by = c("sta", "water_year")) %>% 
  filter(!is.na(q30))                         # run01 n = 3,433 obs 
                                              # run02 n =  obs
                                              # run03 n =  obs
                                              # run04 n =  obs
...
gage <- full_join(gage, gage_summary, 
                  by = c("sta", "water_year")) %>%  
  filter(days_record >= 365)                      # run01 n = 256,041    
                                                  # run02 n = 
                                                  # run03 n = 
                                                  # run04 n =   
...
gage <- full_join(gage, gage_meta, 
                  by = "sta") %>% 
  select(date, sta, q, log_q:q30, water_year, 
         contrib_drain_area_va, qualifier)
...
# drop NA water_year: bev_abf-1991 
#gage_run01_remove <- gage_run01 %>%           # n-remove = 0 obs 
#  filter(sta == "bev_abf" &        
#           water_year == 1991)                 

#gage_run01 <- anti_join(gage_run01, gage_run01_remove)
                                              # n-start  = 115,779 
...
#gage_run01_incomp <- bind_rows(gage_run01_incomp, gage_run01_remove) 
#                                             # n        =   2,026 obs
# ~~~~~~~~~~~~~~~~~~~~~~~ 
...
gage_input <- gage_run01                       # run01 = 115,779  obs
rm(gage_meta, gage_incomp, gage_run01, 
   gage_run01_na, gage_run01_remove, gage_test) 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
```

```{r}
# fix missing station vals - run 02
# filter the second set of complete stations - water-years post1990
#gage_run02 <- gage_summary %>% 
#  filter(water_year < 2002) %>% 
#  spread(water_year, days_record) %>% 
#  filter(!is.na(`1993`)) %>%  # 27 sta w/ complete recs [1992, 1997]
#  select(sta)                 # blp_bel & wkn_wok have na in 92

#gage_run01 <- gage_summary %>% 
#  filter(water_year < 2002) %>% 
#  spread(water_year, days_record) %>% 
#  filter(!is.na(`1990`)) %>%  # 22 sta w/ complete recs [1990, 1997]
#  select(sta)

#gage_run01 <- inner_join(gage, gage_run01, by = "sta") %>%  
#  filter(water_year <= 1997)                   #        n = 63,187

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
gage_run02 <- inner_join(gage, gage_run02, by = "sta") %>%  
  filter(water_year <= 1997) %>%              ##        
  filter(water_year >= 1993)                  #        n = 49,302

# update summary to show only stations with data in 1993
gage_run02_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(sta:'1993') %>% 
  rename(wy_1993 = '1993') %>% 
  filter(!is.na(wy_1993)) %>% 
  gather(key = water_year, val = days_record,-sta) %>% 
  mutate(water_year = as.integer(water_year)) %>% 
  ...
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))                  # n = 81 obs

gage_incomp_run02 <- semi_join(gage_incomp, gage_run02_sum, 
                  by = c("sta", "water_year")) %>%  # n = 781 obs
  filter(!is.na(date)) 

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# fix missing station vals - run 03
# use 2008-2015 to get the rest of the stations with missing vals
# show a roadmap 

gage_run03_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(-c('1990':'1995', '2016', '2017')) %>%
  filter(!is.na(`1998`)) %>%  # 24 sta w/ complete recs [1996, 2015]
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))           # 480 obs

gage_run03 <- semi_join(gage, gage_run03_sum, 
                        by = c("sta", "water_year"))   # n = 158,883

gage_run03_incomp <- semi_join(gage_incomp, gage_run03_sum, 
                  by = c("sta", "water_year")) %>%  # n = 725 obs
  filter(!is.na(date)) 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# run04 fill estimates 
gage_run04_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(-c('2003':'2017')) %>%
filter(!is.na(`1997`)) %>%             # 28 sta w/ recs [1990, 2002]
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))           # 364 obs

gage_run04 <- semi_join(gage, gage_run04_sum, 
                        by = c("sta", "water_year")) # n = 122,718

gage_run04_incomp <- semi_join(gage_incomp, gage_run04_sum, 
                  by = c("sta", "water_year")) %>%  # n = 364 obs
  filter(!is.na(date)) 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# run05 fill estimates 
gage_run05_sum <- gage_summary %>% 
  spread(water_year, days_record) %>% 
  select(-c('2008':'2017')) %>%
filter(!is.na(`2007`)) %>%            
filter(!is.na(`2006`)) %>%             # 27 sta w/ recs [1990, 2006]
  gather(key = water_year, val = days_record, -sta) %>% 
  mutate(water_year = as.integer(water_year)) %>%
  mutate(days_record = case_when(
    is.na(days_record) ~ 0,
    TRUE ~ as.numeric(days_record)))           # 485 obs

gage_run05 <- semi_join(gage, gage_run05_sum, 
                        by = c("sta", "water_year")) # n = 164,714

gage_run05_incomp <- semi_join(gage_incomp, gage_run05_sum, 
                  by = c("sta", "water_year")) %>%  # n = 448 obs
  filter(!is.na(date)) 

# show a roadmap 
gage_run05_map <- semi_join(gage_summary, gage_run05_sum,
                            by = "sta") %>% 
  spread(water_year, days_record)

#   gage_run01, gage_run02, gage_run03, 
#   gage_run04, gage_run05)
#rm(gage_incomp_run02, gage_run02_sum, 
#   gage_incomp_run03, gage_run03_sum)
```
...
lambda_q1 <- BoxCox.lambda(pca_input$q1_depth)   # q1  ==> ~ 0.015  
lambda_q7 <- BoxCox.lambda(pca_input$q7_depth)   # q7  ==> ~ 0.100  
lambda_q30 <- BoxCox.lambda(pca_input$q30_depth) # q30 ==> ~ 0.203    
...
gage_input <- gage_input %>% 
  arrange(sta)
unique(gage_input$sta)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# the series of station runs are as follows: 

# num  min_yr max_yr  n_obs  lambda_q1  lambda_q7  lambda_q30
#  01   1990   2002 115,779    0.0137     0.0110     0.2441

#old
#  01   1990   1995  48,202    0.0129     0.0110     0.2250
#  02   1993   1997  49,302    0.0083     0.0103     0.1975
#  03   1996   2002 122,713    0.0056    -0.0354    -0.1607

# run01n   "bat_bhr" "bat_her" "bat_key" "bev_abf" "bev_buf" "bev_pri" 
#          "blp_bel" "brsf_co" "fal_hot" "frn_fai" "hat_edg" "hor_oel" 
#          "lcr_abv" "lcr_bel" "lcr_vet" "lwr_abv" "lwr_mar" "lwr_ros" 
#          "lwr_whi" "rap_far" "ros_ros" "spr_her" "wcc_ogl" "whi_kad" 
#          "whi_ogl" "whi_slm" "whi_sta" "wkc_wok"

# old
# run_01 sta: bat_bhr, bat_her, bat_key, bev_buf, brsf_co, che_was, 
#             fal_hot, frn_fai, hat_edg, hor_oel, lcr_bel, lcr_vet, 
#             lwr_abv, lwr_mar, lwr_ros, lwr_whi, ros_ros, spr_her, 
#             wcc_ogl, whi_kad, whi_ogl, whi_sta

# run_02 sta: "bat_bhr", "bat_her", "bat_key", "bev_abf", "bev_buf", 
#             "bev_pri", "blp_bel", "brsf_co", "fal_hot", "frn_fai",
#             "hat_edg", "hor_oel", "lcr_bel", "lcr_vet", "lwr_abv", 
#             "lwr_mar", "lwr_ros", "lwr_whi", "rap_far", "ros_ros", 
#             "spr_her", "wcc_ogl", "whi_kad", "whi_ogl", 
#             "whi_sta", "wkc_wok", "whi_slm" 

# run_03 sta: "bat_bhr", "bat_her", "bat_key", "bev_buf",
#             "bev_pri", "blp_bel", "brsf_co", "fal_hot",  "frn_fai"
#             "hat_edg", "hor_oel", "lcr_abv", "lcr_bel", "lcr_vet" 
#             "lwr_mar", "lwr_ros", "lwr_whi", "rap_far",
#             "spr_her", "wcc_ogl", "whi_kad", "whi_ogl",
#             "whi_sta"

pca_eigen 
# num   PC1perc PC2perc PC1cum PC2cum 
#  01    0.952   0.043  0.950   0.993     
#  02    0.939   0.052  0.939   0.990     
#  03    0.946   0.047  0.949   0.994
#  01a   0.945   0.047  0.945   0.993     

pca_vars 
# run  PC  q1_depth  q7_depth  q30_depth      
#   01  1    0.577      0.589     0.568 
#   01  2    0.597      0.171    -0.784  
#   02  1    0.577      0.589     0.566    
#   02  2    0.590      0.179    -0.787   
#   03  1    0.579      0.588     0.565    
#   03  2    0.536      0.248    -0.807     
#   01a 1    0.577      0.588     0.567 
#   01a 2    0.591      0.179    -0.787  
...
#  01   EEV    147.8495   22  51  138.0558 138.0555
#  02   EEV    146.324    27  37  170.7021 169.4785
#  03   VEV    208.482    28  49  253.686  253.577
#  01a  EEV    118.869    28  23  161.098  159.912

# Run 1a clustering table:
#  cluster val:    1     2      3 
# cluster type:  mixed  gw  ephemeral
# num members:     9    16      3 


# old runs - delete
# Run 01 - clustering table:
...
#gage_clust_l$modelName # Optimal selected model ==> "VEV" 
#gage_clust_l$G # Optimal number of cluster => 5 

```

```{r fill_missing_data_wet_years_run01a, eval=FALSE} 
...
gage_pairs <- inner_join(group_lead_id, gage_pairs, 
                         by = "sta_lead") %>%              
...
gage_pairs <- inner_join(gage_pairs, group_lag_id, 
                         by = "sta_lag") %>% 
  filter(group_sta == group_lag) %>% # drops obs of different groups 
  rename(group = group_sta) %>%                              # 24 obs
...
gage_est_miss <- inner_join(gage, gage_part_miss, 
  ...
  group_by(date, sta_orig) %>%                # n = 23,370 obs
  summarise(                                   
    q          = mean(q), 
    log_q      = mean(log_q), 
    q7         = mean(q7), 
...
rm(gage_est_miss, gage_pairs, gage_part_miss, gage_run01_incomp, 
   gage_run01_map, gage_run01_sum, group_lag_id, group_lead_id)
...
test <- gage_summary %>% 
 ...
 test <- semi_join(test, gage_part_miss) 


#gage_miss <- gage_part_miss %>% 
 # filter(days_record == 0) 

#gage_part <- gage_part_miss %>% 
 # filter(days_record != 0) 
gage_est_part <- inner_join(gage, gage_part, 
                        by = c("sta", "water_year"))  %>% 
  select(date, sta_orig, q:qualifier) %>% 
  rename(sta = sta_orig) %>%                 # n = 5,117 obs
  mutate(qualifier = "est") %>% 
  distinct()                                 # n = 4,751 obs

gage_ws_area <- gage %>% 
  arrange(sta) %>% 
  group_by(sta) %>% 
  summarise(
    contrib_drain_area_va = first(contrib_drain_area_va)
  ) %>% 
  ungroup()

test <- inner_join(gage_est_part, gage_ws_area,
                            by = "sta")


gage_nearest_sum <- gage_nearest %>%        #30,678
  group_by(sta, date) %>% 
  summarise(days_record = n()) %>% 
  filter(days_record != 1)

gage_nearest <- 

# clean up
rm(group_lead_id, group_lag_id, gage_part_miss, gage_pairs)




# testing the data - check for & split duplicates


gage_est_over <- full_join(gage_9002_est, gage_9002_sum, 
                           , by = c("sta", "date")) %>% 
  filter(days_record == 2)                    # n =  1,016 obs

gage_est_under <- anti_join(gage_9002_est, gage_est_over) 
                                              # n = 9,718 obs
# filter out duplicates & join
gage_est_over <- gage_est_over %>% 
  filter(qualifier == "A")                    # n =    508 obs

gage_est_9002 <- bind_rows(gage_est_over, gage_est_under) %>% 
  select(-days_record) %>%                   #  n = 10,734 obs
  distinct()

rm(gage_9002_sum, gage_est_over, gage_est_under, gage_9616_est)

# get rid of duplicates
gage_est_9002 <- anti_join(gage_est_9002, gage_cont)
                                              # n =   10,226 obs 
gage_est_9002 <- anti_join(gage_est_9002, gage_disc)
                                              # n =    9,718 obs 
gage_est_9002 <- anti_join(gage_est_9002, gage_other)
                                              # n =    9,718 obs 



#rm(lambda_q1, lambda_q30, lambda_q7)
```
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 1. identify groups for missing & partial year-station pairs 
gage_part_miss <- gage_run04_sum %>% 
  filter(days_record < 365) %>% 
  spread(water_year, days_record)                 # n = 5 x 13 obs

gage_part_miss <- inner_join(gage_clust, gage_part_miss,  
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 28 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year))


# 2. create by-group daily flow estimates for missing yr-sta pairs
#    [1990, 2002] 
gage_group <- inner_join(gage, gage_clust, by = "sta") %>% 
  select(date:group) %>%             
  group_by(date, group) %>% 
  summarise(q          = mean(q, na.rm = TRUE), 
            log_q      = mean(log_q, na.rm = TRUE),
            q7         = mean(q7, na.rm = TRUE),
            q30        = mean(q30, na.rm = TRUE),
            water_year = mean(water_year),
            contrib_drain_area_va = 
              round(mean(contrib_drain_area_va), 
                    digits = 0)                 # summarizes by group 
            ) %>% 
  ungroup()                                      # n = 60,631 obs

# select only the bits that are useful for estimating flows
gage_group <- inner_join(gage_group, gage_part_miss, 
                  by = c("group", "water_year")         
                  ) %>%    # keeps daily flow data for miss yr-sta    
  select(sta, everything())                       # n = 10,226 obs

# split missing year data from incomplete year data
gage_group_part <- gage_group %>%                   # n = 731 obs 
  filter(days_record != 0) %>% 
  select(-c(group, days_record)) %>%           # prepares for join 
  mutate(qualifier = "g")

gage_group <- gage_group %>%                # update n after split
  filter(days_record == 0) %>%                    # n =  9,495 obs 
  select(-c(group, days_record)) %>%           # prepares for join  
    mutate(qualifier = "g")

# join together the bits & clean up
gage_9002_est <- bind_rows(gage_group, gage_group_part, 
                           gage_run04_incomp)       # n = 10,734 obs
rm(gage_group, gage_group_part, gage_run04_incomp, 
   gage_run03_sum, gage_part_miss)

# testing the data - check for & split duplicates
gage_9002_sum <- gage_9002_est %>% 
  group_by(sta, date) %>% 
  summarise(days_record = n())  
...
export(gage_est_9002, file = "data/gage_est_9002.csv") 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 2. identify groups for missing & partial year-station pairs 
gage_part_miss <- gage_run02_sum %>% 
  filter(days_record < 365) %>% 
  spread(water_year, days_record)                     # n = 15 obs

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 12 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year))
...
gage_group <- inner_join(gage_group, gage_part_miss, 
                  by = c("group", "water_year")     # n = 4382 obs     
                  ) %>%    # keeps daily flow data for miss yr-sta              
  select(sta, everything()) 

# split missing year data from incomplete year data
gage_group_part <- gage_group %>%                   # n = 1462 obs 
  filter(days_record != 0) %>% 
  select(-c(group, days_record)) %>%           # prepares for join 
  mutate(qualifier = "g")

gage_group <- gage_group %>%                # update n after split
  filter(days_record == 0) %>%                      # n = 2920 obs 
  select(-c(group, days_record)) %>%           # prepares for join  
    mutate(qualifier = "g")

# join together the bits & clean up
gage_9092_est <- bind_rows(gage_group, gage_group_part, 
                           gage_incomp_run02)       # n = 5233 obs
rm(gage_group, gage_group_part, gage_incomp_run02, 
   gage_run02_sum, gage_part_miss)

# check for & split duplicates
gage_est_9092_sum <- gage_est_9092 %>% 
  group_by(sta, date) %>% 
  summarise(days_record = n())  

gage_est_over <- full_join(gage_est_9092, gage_est_9092_sum) %>% 
  filter(days_record == 2) 

gage_est_under <- anti_join(gage_est_9092, gage_est_over)

# filter out duplicates & join
gage_est_over <- gage_est_over %>% 
  filter(qualifier == "A") 

gage_est_9092 <- bind_rows(gage_est_over, gage_est_under) %>% 
  select(-days_record) %>% 
  distinct()

rm(gage_est_9092_sum, gage_est_over, gage_est_under)


export(gage_est_9092, file = "data/gage_est_9092.csv") 
...
gage_part_miss <- gage_run03_sum %>% 
  filter(days_record < 365) %>% 
  spread(water_year, days_record)                 # n = 4 x 18 obs

gage_part_miss <- inner_join(gage_clust, gage_part_miss, 
                               by = "sta") %>% 
  select(-c(PC1_mean:prob)) %>%                       # n = 45 obs
  gather(key = water_year, value = days_record, -sta, -group) %>% 
  filter(!is.na(days_record))  %>% 
  mutate(water_year = as.integer(water_year))
...
gage_group <- inner_join(gage, gage_clust, by = "sta") %>% 
  ...
  ungroup()                                      # n = 58,074 obs

# select only the bits that are useful for estimating flows
gage_group <- inner_join(gage_group, gage_part_miss, 
                  by = c("group", "water_year")         
                  ) %>%    # keeps daily flow data for miss yr-sta    
  select(sta, everything())                     # n = 16,070 obs
...
gage_group <- gage_group %>%                # update n after split
  filter(days_record == 0) %>%                    # n = 14,608 obs 
  select(-c(group, days_record)) %>%           # prepares for join  
    mutate(qualifier = "g")

# join together the bits & clean up
gage_9616_est <- bind_rows(gage_group, gage_group_part, 
                           gage_run03_incomp)       # n = 17,244 obs
rm(gage_group, gage_group_part, gage_run03_incomp, 
   gage_run03_sum, gage_part_miss)

# testing the data - check for & split duplicates
gage_9616_sum <- gage_9616_est %>% 
  group_by(sta, date) %>% 
  summarise(days_record = n())  

gage_est_over <- full_join(gage_9616_est, gage_9616_sum, 
                           , by = c("sta", "date")) %>% 
  filter(days_record == 2)                    # n =  2,004 obs

gage_est_under <- anti_join(gage_9616_est, gage_est_over) 
                                              # n = 14,642 obs
# filter out duplicates & join
gage_est_over <- gage_est_over %>% 
  filter(qualifier == "A")                    # n =    606 obs

gage_est_9616 <- bind_rows(gage_est_over, gage_est_under) %>% 
  select(-days_record) %>%                   #  n = 15,846 obs
  distinct()

rm(gage_9616_sum, gage_est_over, gage_est_under, gage_9616_est)

# get rid of duplicates
gage_est_9616 <- anti_join(gage_est_9616, gage_cont)
                                              # n =   15,689 obs 
gage_est_9616 <- anti_join(gage_est_9616, gage_disc)
                                              # n =   15,068 obs 
gage_est_9616 <- anti_join(gage_est_9616, gage_other)
                                              # n =   15,068 obs 

export(gage_est_9616, file = "data/gage_est_9616.csv") 
...
ggplot(gage_clust,  
  ...
  geom_text(aes(label = sta, 
            ...
            check_overlap = TRUE, 
            nudge_y = 0.02) + 
  scale_x_continuous(limits = c(-2.3, 1.5)) + 
